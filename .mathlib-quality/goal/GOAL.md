...
### A mathlib lemma re-proved three times: `IsUltrametricDist.norm_sum_le_of_forall_le_of_nonneg`

`ideal_pullback_controlled` **226 -> 211**, from a task-3 finding that also moves task 2.

Three separate inductions in `FiniteJetStrictLocalization.lean` re-prove the same fact --
*in an ultrametric group, a finite sum is bounded by any common bound on its terms*:

| | was | now |
|---|---|---|
| `hsum` | 12 lines | 3 |
| `hfinal` | 18 lines | 12 (restructured; the per-term bound stays) |
| `hpartial` | 8 lines | 3 |

mathlib has it as `IsUltrametricDist.norm_sum_le_of_forall_le_of_nonneg`
(`Mathlib/Analysis/Normed/Group/Ultra.lean:256`), generated by `@[to_additive]` from
`norm_prod_le_of_forall_le_of_nonneg`.

> Searching mathlib for an *additive* lemma by name can come up empty even when it exists:
> `@[to_additive]` generates it, so only the multiplicative original appears as a
> declaration. Search for the multiplicative name, or for *uses* of the additive one. My
> first grep found only call sites in `MahlerBasis` and no definition.

**A fourth copy is genuinely irreducible.** `FiniteJetGraphKoszul:1112` runs the same
induction over `iota -> A` via `pi_norm_add_le_max`; substituting the mathlib lemma fails
with `failed to synthesize IsUltrametricDist (iota -> A)` -- the pi type carries no such
