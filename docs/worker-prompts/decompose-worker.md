...
**Self-schedule (do this first, once — it makes you autonomous).** Check your existing scheduled jobs (e.g. with `CronList`). If you do **not** already have a recurring job for the decompose lane, create one with `CronCreate`: **every 30 minutes**, whose task is exactly *"In `<DIR>`, read `docs/worker-prompts/decompose-worker.md` and do exactly what it says"* — where `<DIR>` is this worktree's absolute path (get it with `pwd`). If such a job already exists, skip it — never create a duplicate. Then continue.
