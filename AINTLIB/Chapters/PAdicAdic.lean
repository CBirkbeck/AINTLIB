...
distinguished valuation $`v : R \to \Gamma_0`. The topology on $`R` is then the valuation topology,
in which open balls around $`x` are the sets $`\{y : v(x - y) < \gamma\}` for $`\gamma \in \Gamma_0`.
:::
...
largest exponent $`k \ge 0` such that $`p^k \mid n`. By convention $`v_p(0) = 0`.
:::
...
:::proof "padic-norm"
The key step is the multiplicativity $`v_p(qr) = v_p(q) + v_p(r)`, from which
$`|qr|_p = |q|_p\,|r|_p`. The ultrametric inequality reduces to the fact that
...
respect to the $`p`-adic norm $`|\cdot|_p`. It is constructed as the quotient of Cauchy sequences
of rationals under the $`p`-adic norm by the equivalence relation of sequences converging to
zero. There is a canonical embedding $`\mathbb{Q} \hookrightarrow \mathbb{Q}_p`.
:::
...
principal ideal domain that is not a field. Equivalently, $`R` has a unique nonzero prime ideal
$`\mathfrak{m}`, every ideal is a power $`\mathfrak{m}^n`, and the maximal ideal is generated by
a single *uniformiser* $`\pi` with $`v(\pi) = 1`.
:::
