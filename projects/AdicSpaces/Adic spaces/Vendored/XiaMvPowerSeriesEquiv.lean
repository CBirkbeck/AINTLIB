...
VENDORED into AINTLIB (2026-07-04) from WilliamCoram/PhD (PhD/Bryce/Basic.lean +
...
theorem sumAlgEquiv_comp_rename_inr : (sumAlgEquiv σ τ R).toAlgHom.comp
  ...
  split_ifs with h
  · subst h
    rw [← Finsupp.embDomain_inr, coeff_embDomain_rename]
...
def optionEquivLeft : MvPowerSeries (Option σ) R ≃ₐ[R] PowerSeries (MvPowerSeries σ R) where
  ...
  commutes' := by
    simpa [MvPowerSeries.algebraMap_apply, PowerSeries.algebraMap_apply,
      PowerSeries.C_apply] using optionFunLeft_monomial (0 : Option σ →₀ ℕ)
...
lemma optionEquivRight_X_none : optionEquivRight σ R (X none) = C PowerSeries.X := by
  simpa [← X_def, PowerSeries.X_eq] using optionEquivRight_monomial (single none 1) (1 : R)
...
def toAdicCompletionAlgEquiv (σ R : Type*) [Finite σ] [CommRing R] :
  ...
  right_inv _ := by
    ext
    simpa [toAdicCompletion_apply_eq_mk_truncTotal] using mk_truncTotal_toAdicCompletionInv
