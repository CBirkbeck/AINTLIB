...
/-- The norm half of `exists_evalBI_approx_bloc`: every monomial of the approximating polynomial
has `wIRPS` bounded by its per-term estimate rescaled by `σ₁ᵐ · (ρ₁ᵐ)⁻¹`, so the sum is too. -/
private theorem wIRPS_monomialSum_le
    (hρσ : ρ₁ ≤ σ₁) (hσρ : σ₁ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₁ ^ m)
    ...
    (hw2 : wLoc p F ϖ hρ₂0 hρ₂1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)) ≤ W)
    {ε : NNReal} (hε : 0 < ε)
    (N : ℕ) (Jf Ef : ℕ → ℕ) (Cf : ℕ → OF F)
    (hbnd1 : ∀ i, wLoc p F ϖ hρ₁0 hρ₁1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i)) (sPow p F ϖ k))
      ≤ σ₁ ^ m * ((ρ₁ ^ m)⁻¹) * wLoc p F ϖ hσ₁0 hσ₁1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)))
    (hbnd2 : ∀ i, wLoc p F ϖ hρ₂0 hρ₂1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i)) (sPow p F ϖ k))
      ≤ max
        (wLoc p F ϖ hσ₁0 hσ₁1 (IsLocalization.mk' (Bloc p F ϖ)
          ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
...
include hφb in
/-- Each monomial of the approximating polynomial evaluates to the corresponding `BIProd`
fraction: `evalBI` on a monomial is `φ` of its coefficient, and `hfact` identifies that
coefficient with the `i`-th term of `w`. -/
private theorem evalBI_monomial_eq_BIProd
    (hρσ : ρ₁ ≤ σ₁) (hσρ : σ₁ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₁ ^ m)
    {b : (hatK p F hσ₁0 hσ₁1) × (hatK p F hρ₂0 hρ₂1)}
    ...
    {ε : NNReal} (hε : 0 < ε)
    (N : ℕ) (Jf Ef : ℕ → ℕ) (Cf : ℕ → OF F)
    (hfact : ∀ i, IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)
      = teichPowGen p F ϖ zb m ^ (Jf i) * IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i)) (sPow p F ϖ k)) :
    ∀ i ∈ Finset.Iic N,
...
include hφb in
/-- The residual half of `exists_evalBI_approx_bloc`: the head sum reproduces `w`'s first
`max N₁ N₂` Teichmüller terms, so the difference is the tail, and the two coordinate norms of the
tail are cut below `ε` by the defining choices of `N₁` and `N₂`. -/
private theorem evalBI_residual_le
    (hρσ : ρ₁ ≤ σ₁) (hσρ : σ₁ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₁ ^ m)
    {b : (hatK p F hσ₁0 hσ₁1) × (hatK p F hρ₂0 hρ₂1)}
    ...
    (hw2 : wLoc p F ϖ hρ₂0 hρ₂1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)) ≤ W)
    {ε : NNReal} (hε : 0 < ε)
    (N₁ N₂ : ℕ) (Jf Ef : ℕ → ℕ) (Cf : ℕ → OF F) (w' : Ainf p F)
    (hsplit : IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)
      = (∑ i ∈ Finset.Iic (max N₁ N₂), IsLocalization.mk' (Bloc p F ϖ)
          ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
          (sPow p F ϖ k))
        + IsLocalization.mk' (Bloc p F ϖ)
            ((p : Ainf p F) ^ (max N₁ N₂ + 1) * w') (sPow p F ϖ k))
    (hN₁ : σ₁ ^ N₁ < ε * ((σ₁ * perfectoidValuation p F
      ((PseudoUniformizer.toOF F ϖ : OF F) : F)) ^ k))
    (hN₂ : ρ₂ ^ N₂ < ε * ((ρ₂ * perfectoidValuation p F
      ((PseudoUniformizer.toOF F ϖ : OF F) : F)) ^ k))
    (hfact : ∀ i, IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)
    ...
    wI p F hσ₁0 hσ₁1 hρ₂0 hρ₂1
        ...
        - evalBI p F ϖ φ hφ hbmem hb (∑ i ∈ Finset.Iic (max N₁ N₂),
        ⟨MvPowerSeries.monomial (Finsupp.single (0 : Fin 1) (Jf i))
          (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
            (IsLocalization.mk' (Bloc p F ϖ)
              ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i))
              (sPow p F ϖ k))),
          isRestricted_monomial_BI p F ϖ _⟩ :
        ↥(restrictedMvPowerSeriesSubring 1 ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)))) ≤ ε := by
  set N := max N₁ N₂ with hNdef
  rw [evalBI_finset_sum p F ϖ φ hφ]
  have hterm := evalBI_monomial_eq_BIProd p F ϖ φ hφ hφb hρσ hσρ zb m hm hgen hbmem hb
    hbg w k hWle hw1 hw2 hε N Jf Ef Cf hfact
...
theorem exists_evalBI_approx_bloc
  ...
  · exact evalBI_residual_le p F ϖ φ hφ hφb hρσ hσρ zb m hm hgen hbmem hb hbg w k hWle
      hw1 hw2 hε N₁ N₂ Jf Ef Cf w' hsplit hN₁ hN₂ hfact
  · exact wIRPS_monomialSum_le p F ϖ hρσ hσρ zb m hm hgen hbmem hb hbg w k hWle hw1 hw2 hε
      N Jf Ef Cf hbnd1 hbnd2
end Assembly
...
/-- The norm half of `exists_evalBI_approx_bloc₂`. -/
private theorem wIRPS_monomialSum_le₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₂ ^ m)
    ...
    (hbg : b = BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (teichPowGen₂ p F ϖ zb m))
    (w : Ainf p F) (k : ℕ) {W : NNReal}
    (hw1 : wLoc p F ϖ hρ₁0 hρ₁1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)) ≤ W)
    (hw2 : wLoc p F ϖ hσ₂0 hσ₂1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)) ≤ W)
    {ε : NNReal} (hε : 0 < ε)
    (N : ℕ) (Jf Ef : ℕ → ℕ) (Cf : ℕ → OF F)
    (hbnd2K : ∀ i, wLoc p F ϖ hρ₂0 hρ₂1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i)) (sPow p F ϖ k))
      ≤ ρ₂ ^ m * ((σ₂ ^ m)⁻¹) * wLoc p F ϖ hσ₂0 hσ₂1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)))
    (hbnd1M : ∀ i, wLoc p F ϖ hρ₁0 hρ₁1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ (Ef i) * WittVector.teichmuller p (Cf i)) (sPow p F ϖ k))
      ≤ max
        (wLoc p F ϖ hσ₂0 hσ₂1 (IsLocalization.mk' (Bloc p F ϖ)
          ((p : Ainf p F) ^ i * WittVector.teichmuller p (teichCoeff p F w i))
  ...
  · refine le_trans (hbnd1M i) ?_
    ...
    have hm2 : wLoc p F ϖ hρ₁0 hρ₁1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i
          * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)) ≤ W :=
      le_trans (wLoc_mk'_monomial_le p F ϖ hρ₁0 hρ₁1 w k i) hw1
    refine le_trans (max_le hm1 hm2) ?_
    exact le_mul_of_one_le_left zero_le hK1
  · refine le_trans (hbnd2K i) ?_
    have hm1 : wLoc p F ϖ hσ₂0 hσ₂1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i
          * WittVector.teichmuller p (teichCoeff p F w i))
        (sPow p F ϖ k)) ≤ W :=
...
include hφb in
/-- Each monomial of the `bloc₂` approximating polynomial evaluates to its `BIProd` fraction. -/
private theorem evalBI_monomial_eq_BIProd₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₂ ^ m)
    ...
    intro i _
    rw [evalBI_monomial p F ϖ φ hφ hbmem hb]
    rw [hφb, hbg, ← map_pow (BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1),
...
include hφb in
/-- The residual half of `exists_evalBI_approx_bloc₂`. -/
private theorem evalBI_residual_le₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    (hgen : perfectoidValuation p F (zb : F) = σ₂ ^ m)
  ...
  rw [evalBI_finset_sum p F ϖ φ hφ]
  have hterm := evalBI_monomial_eq_BIProd₂ p F ϖ φ hφ hφb hρσ hσρ zb m hm hgen hbmem hb
    hbg w k hw1 hw2 hε N Jf Ef Cf hfact
  rw [Finset.sum_congr rfl hterm]
  rw [← map_sum (BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1)]
  rw [show BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
        (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k))
      - BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
        (∑ i ∈ Finset.Iic N, IsLocalization.mk' (Bloc p F ϖ)
          ((p : Ainf p F) ^ i
...
include hφb in
/-- **The case-2 first approximation** (the mirrored Kedlaya lift on the
dense layer). -/
theorem exists_evalBI_approx_bloc₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m : ℕ) (hm : 0 < m)
    ...
    ∃ f : ↥(restrictedMvPowerSeriesSubring 1
      ...
      ∧ wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
          (f : MvPowerSeries (Fin 1)
            ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        ≤ ρ₂ ^ m * ((σ₂ ^ m)⁻¹) * W := by
  have hzb : perfectoidValuation p F (zb : F) ≠ 0 := by
    rw [hgen]
    exact (pow_pos hσ₂0 m).ne'
  -- the tail cutoffs, one per target radius
  have hD1 : (0 : NNReal) < ε * ((ρ₁ * perfectoidValuation p F
      ((PseudoUniformizer.toOF F ϖ : OF F) : F)) ^ k) :=
    mul_pos hε (pow_pos (mul_pos hρ₁0 (vpi_pos p F ϖ)) k)
  have hD2 : (0 : NNReal) < ε * ((σ₂ * perfectoidValuation p F
      ((PseudoUniformizer.toOF F ϖ : OF F) : F)) ^ k) :=
    mul_pos hε (pow_pos (mul_pos hσ₂0 (vpi_pos p F ϖ)) k)
  obtain ⟨N₁, hN₁⟩ := NNReal.exists_pow_lt_of_lt_one hD1 hρ₁1
  obtain ⟨N₂, hN₂⟩ := NNReal.exists_pow_lt_of_lt_one hD2 hσ₂1
  set N := max N₁ N₂ with hNdef
  -- the head/tail split
  obtain ⟨w', hwd⟩ :=
    WittVector.dvd_sub_sum_teichmuller_iterateFrobeniusEquiv_coeff w N
  have hsplit := mk'_sPow_split p F ϖ w k N w' hwd
  -- per-monomial data
  choose Jf Ef Cf hfact hbnd2K hbnd1M using fun i : ℕ =>
    exists_monomial_lift_package₂ p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1 hρ₂0 hρ₂1
      hρσ hσρ zb hzb m hm hgen i k (teichCoeff p F w i)
  refine ⟨∑ i ∈ Finset.Iic N,
    ⟨MvPowerSeries.monomial (Finsupp.single (0 : Fin 1) (Jf i))
      (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
        (IsLocalization.mk' (Bloc p F ϖ)
          ...
          (sPow p F ϖ k))),
      isRestricted_monomial_BI p F ϖ _⟩, ?_, ?_⟩
  · exact evalBI_residual_le₂ p F ϖ φ hφ hφb hρσ hσρ zb m hm hgen hbmem hb hbg w k
      hw1 hw2 hε N₁ N₂ Jf Ef Cf w' hsplit hN₁ hN₂ hfact
  · exact wIRPS_monomialSum_le₂ p F ϖ hρσ hσρ zb m hm hgen hbmem hb hbg w k hw1 hw2 hε
      N Jf Ef Cf hbnd2K hbnd1M
...
include hφb in
/-- **The correction round** (case 2): a residual of size `W·2⁻ᵐ` is reduced
to `W·2⁻⁽ᵐ⁺¹⁾` by a lift of `K`-controlled round norm. -/
theorem exists_correction_step_BI₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m₀ : ℕ) (hm₀ : 0 < m₀)
    ...
    (hrmem : r ∈ BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1)
    (hrbnd : wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 r ≤ W * (2⁻¹ : NNReal) ^ n) :
    ∃ f : ↥(restrictedMvPowerSeriesSubring 1
      ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)),
      wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
        (f : MvPowerSeries (Fin 1) ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        ≤ ρ₂ ^ m₀ * ((σ₂ ^ m₀)⁻¹) * (W * (2⁻¹ : NNReal) ^ n)
      ∧ (r - evalBI p F ϖ φ hφ hbmem hb f)
          ∈ BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
      ∧ wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 (r - evalBI p F ϖ φ hφ hbmem hb f)
        ≤ W * (2⁻¹ : NNReal) ^ (n + 1) := by
  have hhalf0 : (0 : NNReal) < 2⁻¹ := by norm_num
  have hhalf1 : ((2 : NNReal)⁻¹) ≤ 1 := by norm_num
  have hε : (0 : NNReal) < W * (2⁻¹ : NNReal) ^ (n + 1) :=
  ...
  rw [wI_BIProd p F ϖ, valued_BlocToHatK p F ϖ, valued_BlocToHatK p F ϖ]
    at hxw
  have hw1 : wLoc p F ϖ hρ₁0 hρ₁1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k))
      ≤ W * (2⁻¹ : NNReal) ^ n := le_trans (le_max_left _ _) hxw
  have hw2 : wLoc p F ϖ hσ₂0 hσ₂1
      (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k))
      ≤ W * (2⁻¹ : NNReal) ^ n := le_trans (le_max_right _ _) hxw
  -- the round-budget is ≤ 1
  have hWn : W * (2⁻¹ : NNReal) ^ n ≤ 1 := by
  ...
  -- the first approximation of the approximant
  obtain ⟨f, hfres, hfnorm⟩ := exists_evalBI_approx_bloc₂ p F ϖ φ hφ hφb
    hρσ hσρ zb m₀ hm₀ hgen hbmem hb hbg w k hw1 hw2 hε
  refine ⟨f, hfnorm, ?_, ?_⟩
  · exact sub_mem hrmem (evalBI_mem p F ϖ φ hφ hbmem hb f)
  · have hsplit : r - evalBI p F ϖ φ hφ hbmem hb f
        = (r - BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
            (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k)))
          + (BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
              (IsLocalization.mk' (Bloc p F ϖ) w (sPow p F ϖ k))
            - evalBI p F ϖ φ hφ hbmem hb f) := by ring
...
theorem exists_correction_chain_BI₂
    ...
    ∃ (u : ℕ → ↥(restrictedMvPowerSeriesSubring 1
        ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)))
      (r : ℕ → (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)),
      r 0 = z
      ∧ (∀ m, r (m + 1) = r m - evalBI p F ϖ φ hφ hbmem hb (u m))
      ∧ (∀ l, wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
        ((u l : ↥(restrictedMvPowerSeriesSubring 1
          ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)))
          : MvPowerSeries (Fin 1) ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        ≤ ρ₂ ^ m₀ * ((σ₂ ^ m₀)⁻¹) * (W * (2⁻¹ : NNReal) ^ l))
      ∧ ∀ m, wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 (r m)
            ≤ W * (2⁻¹ : NNReal) ^ m := by
  have hK0 : (0 : NNReal) < ρ₂ ^ m₀ * ((σ₂ ^ m₀)⁻¹) :=
    mul_pos (pow_pos hρ₂0 m₀) (inv_pos.mpr (pow_pos hσ₂0 m₀))
  set K : NNReal := ρ₂ ^ m₀ * ((σ₂ ^ m₀)⁻¹) with hKdef
...
include hφb in
/-- **The correction sequence, telescoped** (case 2): partial-sum residuals
shrink geometrically with `K`-scaled round norms. -/
theorem exists_correction_sequence_BI₂
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ ≤ ρ₂)
    (zb : OF F) (m₀ : ℕ) (hm₀ : 0 < m₀)
...
include hφ in
/-- **The correction limit** (case 2): a `K`-normed correction sequence with
geometrically vanishing residuals sums to an exact preimage of `K`-controlled
norm. -/
theorem exists_evalBI_eq_of_correction_BI₂
    {b : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)}
    ...
    (hCbnd : ∀ l, wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
      ((u l : ↥(restrictedMvPowerSeriesSubring 1
        ...
        : MvPowerSeries (Fin 1) ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
      ≤ K * (W * (2⁻¹ : NNReal) ^ l))
    (hres : ∀ m, wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (z - evalBI p F ϖ φ hφ hbmem hb (∑ l ∈ Finset.range m, u l))
        ≤ W * (2⁻¹ : NNReal) ^ m) :
    ∃ U : ↥(restrictedMvPowerSeriesSubring 1
      ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)),
  ...
  have hUnorm : wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
    ...
    have h0 := hU 0 (K * W) (mul_pos hK0 hW) (fun l _ => by
      calc K * (W * (2⁻¹ : NNReal) ^ l)
        ...
        _ = K * W := by rw [mul_one])
    rwa [Finset.sum_range_zero, sub_zero] at h0
  have hdiff : ∀ m : ℕ, wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (z - evalBI p F ϖ φ hφ hbmem hb U)
        ≤ K * (W * (2⁻¹ : NNReal) ^ m) := by
    intro m
    have hε : (0 : NNReal) < K * (W * (2⁻¹ : NNReal) ^ m) :=
    ...
    have h := wI_z_sub_evalBI_add_le p F ϖ φ hφ hbmem hb z
      ...
      (le_trans (hres m) (le_mul_of_one_le_left zero_le hK1)) htail
    exact le_trans (le_of_eq (congrArg
      (fun t => wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
        (z - evalBI p F ϖ φ hφ hbmem hb t)) hcancel.symm)) h
  have hle0 : wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (z - evalBI p F ϖ φ hφ hbmem hb U) ≤ 0 :=
    ge_of_tendsto hC0 (Filter.Eventually.of_forall hdiff)
  have h0 : wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (z - evalBI p F ϖ φ hφ hbmem hb U) = 0 := le_antisymm hle0 zero_le
  exact ⟨U, (sub_eq_zero.mp ((wI_eq_zero_iff p F _).mp h0)).symm, hUnorm⟩

include hφb in
/-- **Strict surjectivity of the case-2 presentation on the unit ball**
(Kedlaya Lemma 4.9, case 1, surjectivity with (4.9.1)-norm control): every
`≤ 1`-normalized element of the cut interval ring is the value of a
restricted series over `B^I` of norm at most `K = σ₂^m(ρ₁^m)⁻¹`. -/
theorem exists_evalBI_eq_of_le_one₂
    ...
    {z : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)}
    (hz : z ∈ BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1)
    (hzle : wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z ≤ 1) :
    ∃ U : ↥(restrictedMvPowerSeriesSubring 1
      ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)),
      evalBI p F ϖ φ hφ hbmem hb U = z
      ∧ wIRPS p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
...
theorem evalBI_GeltEltM0_eq₂
    ...
    (c : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)) :
    evalBI p F ϖ φ hφ hbmem hb (GeltEltM0 p F ϖ c)
      = ((φ c : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1))
...
theorem exists_p_scaling₂
    (z : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)) :
    ∃ k : ℕ, wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (pImage p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1 ^ k * z) ≤ 1 := by
  have hbound : ∀ k : ℕ, wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (pImage p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1 ^ k * z)
      ≤ (max ρ₁ σ₂) ^ k * wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z := by
    intro k
    refine le_trans (wI_mul_le p F _ _) ?_
    rw [wI_pow, pImage, wI_p_image]
  rcases eq_or_ne (wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z) 0 with h0 | hne
  · exact ⟨0, le_trans (hbound 0) (by rw [h0, mul_zero]; exact zero_le_one)⟩
  · have hr1 : max ρ₁ σ₂ < 1 := max_lt hρ₁1 hσ₂1
    have hipos : (0 : NNReal) < (wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z)⁻¹ :=
      inv_pos.mpr (pos_of_ne_zero hne)
    obtain ⟨k, hk⟩ := NNReal.exists_pow_lt_of_lt_one hipos hr1
    refine ⟨k, le_trans (hbound k) ?_⟩
    refine le_of_lt ?_
    calc (max ρ₁ σ₂) ^ k * wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z
        < (wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z)⁻¹
          * wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1 z := by
          exact mul_lt_mul_of_pos_right hk (pos_of_ne_zero hne)
      _ = 1 := inv_mul_cancel₀ hne
...
theorem surjective_evalBIHom₂
  ...
  exact (congrArg (fun t =>
      ((φ (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
          (((↑(isUnit_p_image p F ϖ).unit⁻¹ : Bloc p F ϖ)) ^ k))
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)) * t) hU'eq).trans
    ((congrArg (fun s => s * (pImage p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1 ^ k
        * ((zsub : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1))
          : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1))))
      (hφb (((↑(isUnit_p_image p F ϖ).unit⁻¹ : Bloc p F ϖ)) ^ k))).trans
      hfinal)
include φ hφ hφb in
/-- **The case-2 Robba presentation** (Kedlaya Lemma 4.9, case 1): the cut
interval ring is the quotient of the restricted power series ring over
`B^I` by the principal ideal on `T - g`. -/
theorem nonempty_case2_quotient_equiv
    (hφfst : ∀ z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1),
      ((φ z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₂0 hσ₂1)).1
      = ((z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
  ...
  have hker := ker_evalBIHom_eq_span₂ p F ϖ φ hφ hφfst hbmem hb
    gB hgu hφgB hg1 hg2
  have hsurj := surjective_evalBIHom₂ p F ϖ φ hφ hφb
    hρσ hσρ zb m₀ hm₀ hgen hbmem hb hbg
  exact ⟨(Ideal.quotEquivOfEq hker.symm).trans
...
/-- **The bottom-anchored restriction pair lands in the sub-interval ring**:
the first coordinate is kept literally. -/
theorem resI_bot_pair_mem {θ : ℝ} (hθ0 : 0 ≤ θ) (hθ1 : θ ≤ 1)
    (hσ₁0 : 0 < ρ₁ ^ θ * ρ₂ ^ (1 - θ)) (hσ₁1 : ρ₁ ^ θ * ρ₂ ^ (1 - θ) < 1)
    {z : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)}
    (hz : z ∈ BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1) :
  ...
  have h1 : Filter.Tendsto (fun x => BlocToHatK p F ϖ hρ₁0 hρ₁1 x)
    ...
    have hb : Filter.Tendsto
        ...
        (Filter.comap (BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1) (nhds z))
        (nhds z.1) :=
      (continuous_fst.tendsto z).comp Filter.tendsto_comap
    refine hb.congr fun x => ?_
    exact BIProd_fst p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x
  have hpair : Filter.Tendsto (fun x => (BlocToHatK p F ϖ hρ₁0 hρ₁1 x,
        BlocToHatK p F ϖ hσ₁0 hσ₁1 x))
      (Filter.comap (BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1) (nhds z))
      (nhds (z.1, resI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 hσ₁0 hσ₁1 z)) :=
    Filter.Tendsto.prodMk_nhds h1 h2
  have hmem : ∀ᶠ x in Filter.comap (BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)
      (nhds z),
      (BlocToHatK p F ϖ hρ₁0 hρ₁1 x, BlocToHatK p F ϖ hσ₁0 hσ₁1 x)
        ∈ (Set.range (BIProd p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1)) :=
    Filter.Eventually.of_forall fun x => ⟨x, rfl⟩
  exact mem_closure_of_tendsto hpair hmem

/-- **The bottom-anchored restriction hom** `B^I → B^{[ρ₁,σ]}`: bottom
coordinate kept literally, top coordinate restricted (so the shared-bottom
kernel machinery applies without radius transport). -/
def resIHomBot {θ : ℝ} (hθ0 : 0 ≤ θ) (hθ1 : θ ≤ 1)
    (hσ₁0 : 0 < ρ₁ ^ θ * ρ₂ ^ (1 - θ)) (hσ₁1 : ρ₁ ^ θ * ρ₂ ^ (1 - θ) < 1) :
    ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)
      →+* ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1) where
  toFun z := ⟨(((z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1,
      resI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 hσ₁0 hσ₁1 (z : _)),
    resI_bot_pair_mem p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 z.2⟩
  map_one' := by
    have hone : ((1 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1))
        = BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 1 := by
    ...
    have e2 : resI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 hσ₁0 hσ₁1
        ((1 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
          : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)) = 1 := by
      rw [hone, resI_BIProd p F ϖ hθ0 hθ1 hσ₁0 hσ₁1, map_one]
    have e1 : ((1 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1 = 1 := rfl
    refine Subtype.ext ?_
    rw [show ((1 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1))
      : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₁0 hσ₁1)) = (1, 1) from rfl]
    exact Prod.ext e1 e2
  map_mul' := fun z z' => by
    have e2 := resI_mul p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 z.2 z'.2
    have e1 := congrArg Prod.fst (BISub_coe_mul p F ϖ z z')
    refine Subtype.ext ?_
    exact Prod.ext e1 e2
  map_zero' := by
    have hzero : ((0 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
    ...
    have e2 : resI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 hσ₁0 hσ₁1
        ((0 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
          : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)) = 0 := by
      rw [hzero, resI_BIProd p F ϖ hθ0 hθ1 hσ₁0 hσ₁1, map_zero]
    have e1 : ((0 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1 = 0 := rfl
    refine Subtype.ext ?_
    rw [show ((0 : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1))
      : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₁0 hσ₁1)) = (0, 0) from rfl]
    exact Prod.ext e1 e2
  map_add' := fun z z' => by
    have e2 := resI_add p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 z.2 z'.2
    have e1 := congrArg Prod.fst (BISub_coe_add p F ϖ z z')
    refine Subtype.ext ?_
    exact Prod.ext e1 e2

/-- The bottom-anchored restriction contracts the interval norm. -/
theorem wI_resIHomBot_le {θ : ℝ} (hθ0 : 0 ≤ θ) (hθ1 : θ ≤ 1)
    (hσ₁0 : 0 < ρ₁ ^ θ * ρ₂ ^ (1 - θ)) (hσ₁1 : ρ₁ ^ θ * ρ₂ ^ (1 - θ) < 1)
    (z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)) :
    wI p F hρ₁0 hρ₁1 hσ₁0 hσ₁1
        ((resIHomBot p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 z
          : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1))
          : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₁0 hσ₁1))
      ≤ wI p F hρ₁0 hρ₁1 hρ₂0 hρ₂1
          ((z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
...
/-- The bottom-anchored restriction fixes the first coordinate. -/
theorem resIHomBot_fst {θ : ℝ} (hθ0 : 0 ≤ θ) (hθ1 : θ ≤ 1)
    (hσ₁0 : 0 < ρ₁ ^ θ * ρ₂ ^ (1 - θ)) (hσ₁1 : ρ₁ ^ θ * ρ₂ ^ (1 - θ) < 1)
    (z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)) :
    ((resIHomBot p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 z
      : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1))
      : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₁0 hσ₁1)).1
      = ((z : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1 := rfl

/-- The bottom-anchored restriction carries `Bloc`-images to `Bloc`-images. -/
theorem resIHomBot_blocToBI {θ : ℝ} (hθ0 : 0 ≤ θ) (hθ1 : θ ≤ 1)
    (hσ₁0 : 0 < ρ₁ ^ θ * ρ₂ ^ (1 - θ)) (hσ₁1 : ρ₁ ^ θ * ρ₂ ^ (1 - θ) < 1)
    (x : Bloc p F ϖ) :
    ((resIHomBot p F ϖ hθ0 hθ1 hσ₁0 hσ₁1
        (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x)
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hσ₁0 hσ₁1))
      = BIProd p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1 x := by
  refine Prod.ext ?_ ?_
  · show ((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1
      = (BIProd p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1 x).1
    rw [show ((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1))
      = BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x from rfl]
    rw [BIProd_fst p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x,
      BIProd_fst p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1 x]
  · show resI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 hσ₁0 hσ₁1
        ((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x
          : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
          : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1))
      = (BIProd p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1 x).2
    rw [show ((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1))
      = BIProd p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 x from rfl]
    rw [resI_BIProd p F ϖ hθ0 hθ1 hσ₁0 hσ₁1 x,
      BIProd_snd p F ϖ hρ₁0 hρ₁1 hσ₁0 hσ₁1 x]

/-- **The case-2 generator's valuation at the bottom endpoint.** -/
theorem valued_blocToBI_teichPowGen₂_fst (zb : OF F)
    (hzb : perfectoidValuation p F (zb : F) ≠ 0) (m₀ : ℕ) :
    Valued.v (((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
        (teichPowGen₂ p F ϖ zb m₀)
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
...
/-- **The case-2 generator's valuation at the top endpoint.** -/
theorem valued_blocToBI_teichPowGen₂_snd (zb : OF F)
    (hzb : perfectoidValuation p F (zb : F) ≠ 0) (m₀ : ℕ) :
    Valued.v (((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
        (teichPowGen₂ p F ϖ zb m₀)
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
...
theorem isUnit_teichPowGen₂ (zb : OF F)
  ...
  · rw [map_pow]
    exact (isUnit_p_image p F ϖ).pow m₀
  · have hu := isUnit_teichmuller_image p F ϖ zb hzb
    rw [← hu.unit_spec, Ring.inverse_unit]
    exact hu.unit⁻¹.isUnit

/-- **The case-2 Robba presentation, instantiated** (Kedlaya Lemma 4.9,
case 2): for a strict interior cut `ρ₁ ≤ σ₂ < ρ₂` realized by an inverse
Teichmüller generator (`|z̄| = σ₂^{m₀}`), the cut interval ring is the
quotient of the restricted power series ring over `B^I` by the principal
ideal on `T - p^{m₀}/[z̄]`. -/
theorem robba_case2_presentation
    {σ₂ : NNReal} (hσ₂0 : 0 < σ₂) (hσ₂1 : σ₂ < 1)
    (hρσ : ρ₁ ≤ σ₂) (hσρ : σ₂ < ρ₂)
    (zb : OF F) (m₀ : ℕ) (hm₀ : 0 < m₀)
    (hgen : perfectoidValuation p F (zb : F) = σ₂ ^ m₀) :
      ...
      ((↥(restrictedMvPowerSeriesSubring 1
        ...
        ⧸ Ideal.span {GeltElt p F ϖ
            (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
              (teichPowGen₂ p F ϖ zb m₀))})
      ≃+* ↥(BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1)) := by
  refine (exists_interpolant hρ₁0 hρσ hσρ.le hρ₂1).elim
    fun θ h => h.elim fun hθ0 h' => h'.elim fun hθ1 hθeq => ?_
  subst hθeq
  have hzb : perfectoidValuation p F (zb : F) ≠ 0 := by
    rw [hgen]
  ...
  have hgu : IsUnit (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1
      (teichPowGen₂ p F ϖ zb m₀)) :=
    (isUnit_teichPowGen₂ p F ϖ zb hzb m₀).map
      (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)
  have hg1 : Valued.v
      (((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 (teichPowGen₂ p F ϖ zb m₀)
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).1) ≤ 1 := by
    rw [valued_blocToBI_teichPowGen₂_fst p F ϖ zb hzb, hgen]
    exact gen_val_le_one hσ₂0 hρσ m₀
  have hg2 : 1 < Valued.v
      (((blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 (teichPowGen₂ p F ϖ zb m₀)
        : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1))
        : (hatK p F hρ₁0 hρ₁1) × (hatK p F hρ₂0 hρ₂1)).2) := by
    rw [valued_blocToBI_teichPowGen₂_snd p F ϖ zb hzb, hgen]
    exact one_lt_gen_val hσ₂0 hσρ m₀ hm₀
  have hbmem : BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (teichPowGen₂ p F ϖ zb m₀) ∈ BISub p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1 :=
    BIProd_mem_BISub p F ϖ _
  have hb : wI p F hρ₁0 hρ₁1 hσ₂0 hσ₂1
      (BIProd p F ϖ hρ₁0 hρ₁1 hσ₂0 hσ₂1
        (teichPowGen₂ p F ϖ zb m₀)) ≤ 1 := by
    refine max_le ?_ ?_
    · rw [BIProd_fst, valued_BlocToHatK,
        wLoc_teichPowGen₂ p F ϖ hρ₁0 hρ₁1 zb hzb, hgen]
      exact gen_val_le_one hσ₂0 hρσ m₀
    · rw [BIProd_snd, valued_BlocToHatK,
        wLoc_teichPowGen₂ p F ϖ hσ₂0 hσ₂1 zb hzb, hgen]
      exact le_of_eq (mul_inv_cancel₀ (pow_pos hσ₂0 m₀).ne')
  exact nonempty_case2_quotient_equiv p F ϖ
    (resIHomBot p F ϖ hθ0 hθ1 hσ₂0 hσ₂1)
    (wI_resIHomBot_le p F ϖ hθ0 hθ1 hσ₂0 hσ₂1)
    (resIHomBot_blocToBI p F ϖ hθ0 hθ1 hσ₂0 hσ₂1)
    (resIHomBot_fst p F ϖ hθ0 hθ1 hσ₂0 hσ₂1)
    hρσ hσρ.le zb m₀ hm₀ hgen hbmem hb rfl
    (blocToBI p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1 (teichPowGen₂ p F ϖ zb m₀))
    hgu (resIHomBot_blocToBI p F ϖ hθ0 hθ1 hσ₂0 hσ₂1
      (teichPowGen₂ p F ϖ zb m₀))
    hg1 hg2
...
/-- The coefficient bound forced by a normalized monomial fraction
(`≤`-version). -/
theorem valuation_coeff_of_wLoc_le_one {σ₁ : NNReal} (hσ₁0 : 0 < σ₁)
    (hσ₁1 : σ₁ < 1) (i k : ℕ) (hik : i ≤ k) (c : OF F)
    (hval : wLoc p F ϖ hσ₁0 hσ₁1 (IsLocalization.mk' (Bloc p F ϖ)
        ((p : Ainf p F) ^ i * WittVector.teichmuller p c) (sPow p F ϖ k))
      ≤ 1) :
  ...
  have hpos : (0 : NNReal) < (σ₁ * V) ^ k := pow_pos (mul_pos hσ₁0 hV0) k
  have h2 : σ₁ ^ i * perfectoidValuation p F (c : F) ≤ (σ₁ * V) ^ k := by
    have h3 : σ₁ ^ i * perfectoidValuation p F (c : F)
        * (((σ₁ * V) ^ k)⁻¹) * ((σ₁ * V) ^ k) ≤ 1 * ((σ₁ * V) ^ k) :=
      mul_le_mul_of_nonneg_right h1 zero_le
    rwa [one_mul, mul_assoc, inv_mul_cancel₀ hpos.ne', mul_one] at h3
  have h4 : σ₁ ^ i * perfectoidValuation p F (c : F)
      ≤ σ₁ ^ i * (σ₁ ^ (k - i) * V ^ k) := by
    ...
    rw [show σ₁ ^ k = σ₁ ^ i * σ₁ ^ (k - i) by
      ...
      omega]
    ring
  exact le_of_mul_le_mul_left h4 (pow_pos hσ₁0 i)

/-- **The normalized power lift, ball form** (T910-M M2c): a monomial
fraction of `σ₁`-value at most `1` has, at the `m`-th power, an evaluation
preimage inside the unit ball of restricted series. -/
theorem exists_evalBI_pow_mem_image_of_le
    {ρ₁ ρ₂ σ₁ : NNReal} {hρ₁0 : 0 < ρ₁} {hρ₁1 : ρ₁ < 1} {hρ₂0 : 0 < ρ₂}
    {hρ₂1 : ρ₂ < 1} {hσ₁0 : 0 < σ₁} {hσ₁1 : σ₁ < 1}
    (φ : ↥(BISub p F ϖ hρ₁0 hρ₁1 hρ₂0 hρ₂1)
      →+* ↥(BISub p F ϖ hσ₁0 hσ₁1 hρ₂0 hρ₂1))
  ...
  have hV0 : 0 < V := vpi_pos p F ϖ
  have hV1 : V < 1 := perfectoidValuation_toOF_lt_one p F ϖ
  have hc := valuation_coeff_of_wLoc_le_one p F ϖ hσ₁0 hσ₁1 i k hik c hval
  have hdvd : perfectoidValuation p F (c : F) ^ m
      ≤ perfectoidValuation p F (zb : F) ^ (k - i) := by
    refine le_trans (pow_le_pow_left' hc m) ?_
    rw [hgen, mul_pow, ← pow_mul, ← pow_mul, ← pow_mul]
    rw [show (k - i) * m = m * (k - i) from Nat.mul_comm _ _]
    refine le_trans (mul_le_mul_of_nonneg_left
      (pow_le_one₀ zero_le hV1.le) zero_le) ?_
    rw [mul_one]
  ...
  have hc'val : perfectoidValuation p F (c' : F) ≤ V ^ (k * m) := by
    have h1 : perfectoidValuation p F ((c : F)) ^ m
      ...
      have h2 := congrArg (fun t : OF F =>
        perfectoidValuation p F (t : F)) hc'
      simpa [map_mul, map_pow] using h2
    have h3 : σ₁ ^ (m * (k - i)) * perfectoidValuation p F (c' : F)
        ≤ σ₁ ^ (m * (k - i)) * V ^ (k * m) :=
      calc σ₁ ^ (m * (k - i)) * perfectoidValuation p F (c' : F)
          = perfectoidValuation p F (zb : F) ^ (k - i)
            * perfectoidValuation p F (c' : F) := by
            rw [hgen, ← pow_mul]
        _ = perfectoidValuation p F (c : F) ^ m := h1.symm
        _ ≤ (σ₁ ^ (k - i) * V ^ k) ^ m := pow_le_pow_left' hc m
        _ = σ₁ ^ (m * (k - i)) * V ^ (k * m) := by
            rw [mul_pow, ← pow_mul, ← pow_mul,
              Nat.mul_comm (k - i) m, Nat.mul_comm k m]
...
/-- **Power membership gives integrality** (generic): an element whose `m`-th
power lies in a subring is integral over it. -/
theorem isIntegral_of_pow_mem {S : Type*} [CommRing S] (B : Subring S)
    (z : S) (m : ℕ) (hm : 0 < m) (h : z ^ m ∈ B) :
    IsIntegral (↥B) z := by
  refine IsIntegral.of_pow hm ?_
  rw [show z ^ m = algebraMap (↥B) S ⟨z ^ m, h⟩ from rfl]
...
/-- **The image of the unit ball** (T910-M): the subring of `B^{I'}`
generated by evaluating unit-ball restricted series — Kedlaya's
`B^{I,+}\{T/ρ\}`-image. -/
noncomputable def evalBallSubring
    {b : (hatK p F hσ₁0 hσ₁1) × (hatK p F hσ₂0 hσ₂1)}
    (hbmem : b ∈ BISub p F ϖ hσ₁0 hσ₁1 hσ₂0 hσ₂1)
    (hb : wI p F hσ₁0 hσ₁1 hσ₂0 hσ₂1 b ≤ 1) :
    Subring ↥(BISub p F ϖ hσ₁0 hσ₁1 hσ₂0 hσ₂1) :=
  (wIRPSBall p F ϖ).map (evalBIHom p F ϖ φ hφ hbmem hb)
