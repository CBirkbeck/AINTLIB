...
variable (F : Type u) [Field F] [TopologicalSpace F] [NonarchimedeanRing F]
...
abbrev OF (F : Type u) [CommRing F] [TopologicalSpace F] [NonarchimedeanRing F] : Type u :=
...
def PseudoUniformizer.toOF (F : Type u) [CommRing F] [TopologicalSpace F]
    [NonarchimedeanRing F] (ϖ : PseudoUniformizer F) : OF F :=
...
instance instCharPOF [CharP F p] : CharP (OF F) p where
...
its perfectness". -/
theorem frobenius_surjective_OF [UniformSpace F] [IsPerfectoidField p F] [CharP F p] :
    Function.Surjective (frobenius (OF F) p) := by
  intro x
...
instance instPerfectRingOF [UniformSpace F] [IsPerfectoidField p F] [CharP F p] :
    PerfectRing (OF F) p :=
...
theorem span_toOF_pow_mem_nhds_zero [UniformSpace F] [IsPerfectoidField p F]
    (ϖ : PseudoUniformizer F) (n : ℕ) :
...
theorem exists_span_toOF_pow_subset_of_mem_nhds [UniformSpace F] [IsPerfectoidField p F]
    (ϖ : PseudoUniformizer F) {U : Set (OF F)} (hU : U ∈ nhds (0 : OF F)) :
...
/-- **Adic precision gives Cauchy differences.** If `f m - f n ∈ I ^ m` whenever `m ≤ n`,
for `I` the ideal generated by a pseudo-uniformizer, then for every neighbourhood `W` of
`0` in `F` the differences `f m - f n` eventually lie in `W`. The bound combines
topological nilpotence of `ϖ` with boundedness of the power-bounded subring. -/
private theorem exists_forall_sub_mem_of_adicPrecision [UniformSpace F]
    [IsPerfectoidField p F] (ϖ : PseudoUniformizer F)
...
theorem isAdicComplete_span_toOF [UniformSpace F] [IsPerfectoidField p F]
  ...
  have hprec : IsPrecomplete I (OF F) := by
    ...
    have hsmall := exists_forall_sub_mem_of_adicPrecision p F ϖ I hI f hf'
