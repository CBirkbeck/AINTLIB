...
This module imports only the **definition layer** — `FJP.FiniteJetNoetherianVertices` (which
re-exports `FJP.FiniteJetRings`, defining `JetA` and its instances, and declares the base class
`IsFJPNoetherianBase`) and `Uniform` (defines `IsUniform` / `IsStablyUniform`). Their combined
...
theorem fjp_1_3_isSheafy [IsFJPNoetherianBase F] : IsSheafy (JetA F) := sorry
