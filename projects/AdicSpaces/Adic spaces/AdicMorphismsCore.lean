...
omit [IsHuberRing A] in
/-- The subring generated by the image of `PA.I ^ m` is open: it contains the image of
`PA.I ^ (2 * m)`, which is a neighbourhood of `0`. -/
private theorem isOpen_closure_pow_image (PA : PairOfDefinition A) (m : ℕ) {S : Set A}
    (S_def : S = PA.A₀.subtype '' ((PA.I ^ m : Ideal PA.A₀) : Set PA.A₀)) :
    IsOpen ((Subring.closure S : Subring A) : Set A) := by
  change IsOpen ((Subring.closure S).toAddSubgroup : Set A)
  exact AddSubgroup.isOpen_of_mem_nhds _
    (Filter.mem_of_superset
...
hypothesis `ih` places the result in `I' ^ n`. -/
private theorem span_mul_pow_mem_pow_gen (PA : PairOfDefinition A) (m n : ℕ)
    {A₀' : Subring A} [DecidableEq ↥A₀'] (hA₀'_le_PA : A₀' ≤ PA.A₀)
    (ι : ↥A₀' →+* ↥PA.A₀) (ι_def : ι = Subring.inclusion hA₀'_le_PA)
    (F : Finset ↥PA.A₀) (hF_sub : ∀ g ∈ F, (PA.A₀.subtype g : A) ∈ A₀')
    (I' : Ideal ↥A₀')
    (hI'_def : I' = Ideal.span
      ((F.attach.image (fun g ↦ (⟨PA.A₀.subtype g.1, hF_sub g.1 g.2⟩ : A₀'))) : Set ↥A₀'))
    (hlift : ∀ x ∈ PA.I ^ m, (PA.A₀.subtype x : A) ∈ A₀')
    (ih : Ideal.comap ι (PA.I ^ ((n + 2) * m)) ≤ I' ^ n)
    (f : ↥PA.A₀) (hf : f ∈ F) :
    ∀ (w' : PA.A₀), w' ∈ PA.I ^ ((n + 2) * m) →
      (PA.A₀.subtype (f * w') : A) ∈ A₀' ∧
      ∀ (h' : (PA.A₀.subtype (f * w') : A) ∈ A₀'),
      (⟨PA.A₀.subtype (f * w'), h'⟩ : A₀') ∈ (I' ^ (n + 1) : Ideal A₀') := by
  subst ι_def
  set ι := Subring.inclusion hA₀'_le_PA with ι_def
  intro w' hw'
  have hf'_mem :
      (⟨PA.A₀.subtype f, hF_sub f hf⟩ : A₀') ∈ I' := by
    rw [hI'_def]
    exact Ideal.subset_span (Finset.mem_image.mpr
...
private theorem span_mul_pow_mem_pow_smul (PA : PairOfDefinition A) (m n : ℕ)
    {A₀' : Subring A} (I' : Ideal ↥A₀') (c x' : ↥PA.A₀)
    (hx'_ih :
    ∀ (w' : PA.A₀), w' ∈ PA.I ^ ((n + 2) * m) →
        (PA.A₀.subtype (x' * w') : A) ∈ A₀' ∧
        ∀ (h' : (PA.A₀.subtype (x' * w') : A) ∈ A₀'),
        (⟨PA.A₀.subtype (x' * w'), h'⟩ : A₀') ∈ (I' ^ (n + 1) : Ideal A₀')) :
    ∀ (w' : PA.A₀), w' ∈ PA.I ^ ((n + 2) * m) →
      (PA.A₀.subtype (c • x' * w') : A) ∈ A₀' ∧
      ∀ (h' : (PA.A₀.subtype (c • x' * w') : A) ∈ A₀'),
      (⟨PA.A₀.subtype (c • x' * w'), h'⟩ : A₀') ∈ (I' ^ (n + 1) : Ideal A₀') := by
  intro w' hw'
  have hcw'_mem :
      c * w' ∈ PA.I ^ ((n + 2) * m) :=
...
Proved by `Submodule.span_induction` on `g'`; the `smul` and `add` cases reduce to the outer
induction hypothesis `ih`. -/
private theorem span_mul_pow_mem_pow (PA : PairOfDefinition A) (m n : ℕ)
    {A₀' : Subring A} [DecidableEq ↥A₀'] (hA₀'_le_PA : A₀' ≤ PA.A₀)
    (ι : ↥A₀' →+* ↥PA.A₀) (ι_def : ι = Subring.inclusion hA₀'_le_PA)
    (F : Finset ↥PA.A₀) (hF_sub : ∀ g ∈ F, (PA.A₀.subtype g : A) ∈ A₀')
    (I' : Ideal ↥A₀')
 ...
 := by
  ...
  | mem f hf =>
    exact span_mul_pow_mem_pow_gen PA m n hA₀'_le_PA ι ι_def F hF_sub I' hI'_def
      hlift ih f hf
  | zero =>
    intro w' _
...
The `+ 2` shift absorbs two things at once — one factor of `PA.I ^ m` to rewrite a product over
the generators `F`, and one to keep the induction hypothesis applicable at `n`. -/
private theorem comap_pow_le_pow_of_generators (PA : PairOfDefinition A) (m : ℕ)
    {A₀' : Subring A} [DecidableEq ↥A₀'] (hA₀'_le_PA : A₀' ≤ PA.A₀)
    (ι : ↥A₀' →+* ↥PA.A₀) (hι_def : ι = Subring.inclusion hA₀'_le_PA)
    (F : Finset ↥PA.A₀) (hF : Ideal.span (F : Set ↥PA.A₀) = PA.I ^ m)
    (hF_sub : ∀ g ∈ F, (PA.A₀.subtype g : A) ∈ A₀')
    (F' : Finset ↥A₀')
    (hF'_def : F' = F.attach.image (fun g ↦ ⟨PA.A₀.subtype g.1, hF_sub g.1 g.2⟩))
    (I' : Ideal ↥A₀') (hI'_def : I' = Ideal.span (F' : Set ↥A₀'))
    (hlift : ∀ x ∈ PA.I ^ m, (PA.A₀.subtype x : A) ∈ A₀') :
  ...
  | succ n ih =>
    ...
    · exact span_mul_pow_mem_pow PA m n hA₀'_le_PA ι ι_def F hF_sub I' hI'_def hlift ih
        g (hF ▸ hg) w hw
...
theorem exists_pairOfDefinition_le_subring (PA : PairOfDefinition A) {U : Subring A}
  ...
  have hA₀'_open : IsOpen (A₀' : Set A) := isOpen_closure_pow_image PA m S_def
  set ι := Subring.inclusion hA₀'_le_PA with ι_def
  ...
  have comap_le_pow :
      ∀ n, Ideal.comap ι (PA.I ^ ((n + 2) * m)) ≤ I' ^ n :=
    comap_pow_le_pow_of_generators PA m hA₀'_le_PA ι ι_def F hF hF_sub F' rfl I' I'_def hlift
