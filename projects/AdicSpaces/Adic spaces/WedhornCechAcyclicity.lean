...
/- The `B = 𝒪_X(D₀)` ring-theoretic facts are `theorem`s upstream, so every proof in this
file used to re-introduce them with a five-line `haveI` preamble (~250 lines in total).
Registering them as *local* instances lets typeclass inference find them instead; the blast
radius stays in this file rather than changing global resolution for every consumer of
`Wedhorn828`. `IsHuberRing` needs no entry: `IsTateRing extends IsHuberRing`.

NOT included: `presheafValue_completeSpace_rightUniformSpace`, which is stated at the
non-canonical `IsTopologicalAddGroup.rightUniformSpace`, so it must stay an explicit
...
/-! ### Shared restriction/separation idioms

Four packagings of chases that the Prop A.3 arguments (`..._propA3_part1_gluing`,
`..._propA3_part2_project_gluing`, `genRestrictedCover_gluing`) run at every overlap.
Stating them once keeps those proofs at the level of the mathematics — "the sections
agree on the intersection", "separate over the acyclic cover" — instead of the
...
`IsOXAcyclic.separation` is stated in "restricts to zero ⇒ is zero" form on `K.base`; the
callers invariably have two sections on an `I` that is only *equal* to `K.base`, so each
use otherwise repeats the same `sub_eq_zero` → `Eq.rec`-transport → `separation` →
`map_sub` chase. -/
private theorem eq_of_restrictionMap_eq_of_isOXAcyclic [HasLocLiftPowerBounded A]
    (I : RationalLocData A) (K : RationalCoveringData A) (hK : K.base = I)
    (hK_acyclic : K.IsOXAcyclic) (u v : presheafValue I)
    (h : ∀ (W : RationalLocData A), W ∈ K.covers →
      ∀ (hW : rationalOpen W.T W.s ⊆ rationalOpen I.T I.s),
      restrictionMap I W hW u = restrictionMap I W hW v) :
    u = v := by
  rw [← sub_eq_zero,
    ← RationalCoveringData.presheafValue_eqRec_eq_zero_iff I K.base hK.symm (u - v)]
  apply hK_acyclic.separation
  intro W hW
  have hWI : rationalOpen W.T W.s ⊆ rationalOpen I.T I.s := by
    rw [← hK]; exact K.hsubset W hW
  rw [RationalCoveringData.eqRec_restrictionMap_direct I K.base hK.symm W hWI
    (K.hsubset W hW) (u - v)]
  show (restrictionMapHom I W hWI) _ = 0
...
This is what reduces an arbitrary common refinement to the single canonical intersection,
which is where the `q = 1` standing hypothesis of Prop A.3(1) applies. -/
private theorem restrictionMap_eq_of_eq_on_inter [HasLocLiftPowerBounded A]
    (X₁ X₂ I Y : RationalLocData A)
    (hI : rationalOpen I.T I.s =
      rationalOpen X₁.T X₁.s ∩ rationalOpen X₂.T X₂.s)
    (u : presheafValue X₁) (v : presheafValue X₂)
    (hI₁ : rationalOpen I.T I.s ⊆ rationalOpen X₁.T X₁.s)
    (hI₂ : rationalOpen I.T I.s ⊆ rationalOpen X₂.T X₂.s)
    (h₁ : rationalOpen Y.T Y.s ⊆ rationalOpen X₁.T X₁.s)
    (h₂ : rationalOpen Y.T Y.s ⊆ rationalOpen X₂.T X₂.s)
    (hI_eq : restrictionMap X₁ I hI₁ u = restrictionMap X₂ I hI₂ v) :
    restrictionMap X₁ Y h₁ u = restrictionMap X₂ Y h₂ v := by
  have hYI : rationalOpen Y.T Y.s ⊆ rationalOpen I.T I.s := by
    rw [hI]; exact Set.subset_inter h₁ h₂
  have hL : restrictionMap X₁ Y h₁ u =
      restrictionMap I Y hYI (restrictionMap X₁ I hI₁ u) :=
    (restrictionMap_restrictionMap X₁ I Y hI₁ hYI u).symm
  have hR : restrictionMap X₂ Y h₂ v =
      restrictionMap I Y hYI (restrictionMap X₂ I hI₂ v) :=
...
needs a `show … from rfl` to fold it back into the `presheafValueCast` the section was
built with. -/
private theorem restrictionMap_presheafValueCast [HasLocLiftPowerBounded A]
    (C V : RationalCoveringData A) (hVC : V.base = C.base) (x : presheafValue C.base)
    (Y : RationalLocData A)
    (hYV : rationalOpen Y.T Y.s ⊆ rationalOpen V.base.T V.base.s)
    (hYC : rationalOpen Y.T Y.s ⊆ rationalOpen C.base.T C.base.s) :
    restrictionMap V.base Y hYV
        (RationalCoveringData.presheafValueCast (C := C) (C' := V) hVC x) =
      restrictionMap C.base Y hYC x :=
  RationalCoveringData.presheafValueCast_restrictionMap C.base V.base hVC Y hYC hYV x
...
private theorem mem_covers_interProd (Uf V : RationalCoveringData A)
    (hbase : V.base = Uf.base)
    (hUfP : ∀ P ∈ Uf.covers, P.P = Uf.base.P)
    (hVP : ∀ Q ∈ V.covers, Q.P = Uf.base.P)
    {P Q : RationalLocData A} (hP : P ∈ Uf.covers) (hQ : Q ∈ V.covers) :
    P.interSamePair Q ((hVP Q hQ).trans (hUfP P hP).symm) ∈
      (Uf.interProd V hbase hUfP hVP).covers := by
  rw [RationalCoveringData.interProd_covers, Finset.mem_image]
  exact ⟨⟨(P, Q), Finset.mem_product.mpr ⟨hP, hQ⟩⟩, Finset.mem_attach _ _, rfl⟩

omit [IsHuberRing A] in
/-- The forward direction of `exists_of_mem_interProdOn`: an intersection of a
`C₁`-piece with a `C₂`-piece is a piece of the product cover. -/
private theorem mem_covers_interProdOn (C₁ C₂ : RationalCoveringData A)
    (I : RationalLocData A)
    (hI : rationalOpen I.T I.s =
      rationalOpen C₁.base.T C₁.base.s ∩ rationalOpen C₂.base.T C₂.base.s)
    (hP' : ∀ D₁ ∈ C₁.covers, ∀ D₂ ∈ C₂.covers, D₂.P = D₁.P)
    {D₁ D₂ : RationalLocData A} (hD₁ : D₁ ∈ C₁.covers) (hD₂ : D₂ ∈ C₂.covers) :
    D₁.interSamePair D₂ (hP' D₁ hD₁ D₂ hD₂) ∈
      (C₁.interProdOn C₂ I hI hP').covers := by
  ...
  exact ⟨⟨(D₁, D₂), Finset.mem_product.mpr ⟨hD₁, hD₂⟩⟩, Finset.mem_attach _ _, rfl⟩

omit [IsHuberRing A] [DecidableEq (RationalLocData A)] in
open Classical in
/-- For a cover generated by `units`, each piece's `insert s T` collapses back to
`units`: `T` is `units` and `s` is one of its elements. -/
private theorem insert_s_T_eq_of_generated {C : RationalCoveringData A}
    {units : Finset A} {φ : ↥units → ↥C.covers}
    (hφ : ∀ t : ↥units, (φ t).1.T = units ∧ (φ t).1.s = (t : A)) (u : ↥units) :
    insert (φ u).1.s (φ u).1.T = units := by
  rw [(hφ u).2, (hφ u).1]
  exact Finset.insert_eq_self.mpr u.2

omit [IsHuberRing A] in
open Classical in
/-- Surjectivity of the piece map of `interProdOn`: every intersection piece `E` is
`D₁ ∩ D₂` for pieces of the two covers, each of which is `φᵢ tᵢ`, and `t₁ * t₂` is the
unit that maps to it. -/
private theorem interProdOn_pieceMap_surjective (C₁ C₂ : RationalCoveringData A)
    (I : RationalLocData A)
    (hI : rationalOpen I.T I.s =
      rationalOpen C₁.base.T C₁.base.s ∩ rationalOpen C₂.base.T C₂.base.s)
    (P₀ : PairOfDefinition A) (hP₁ : ∀ D₁ ∈ C₁.covers, D₁.P = P₀)
    (hP' : ∀ D₁ ∈ C₁.covers, ∀ D₂ ∈ C₂.covers, D₂.P = D₁.P)
    {units₁ units₂ : Finset A}
    (φ₁ : ↥units₁ → ↥C₁.covers) (φ₂ : ↥units₂ → ↥C₂.covers)
    (hφ₁_bij : Function.Bijective φ₁) (hφ₂_bij : Function.Bijective φ₂)
    (hφ₁ : ∀ t : ↥units₁, (φ₁ t).1.T = units₁ ∧ (φ₁ t).1.s = (t : A))
    (hφ₂ : ∀ t : ↥units₂, (φ₂ t).1.T = units₂ ∧ (φ₂ t).1.s = (t : A))
    (a : ↥(units₁ * units₂) → A) (ha : ∀ p, a p ∈ units₁)
    (b : ↥(units₁ * units₂) → A) (hb : ∀ p, b p ∈ units₂)
    (hab : ∀ p, a p * b p = (p : A)) :
    Function.Surjective (fun p : ↥(units₁ * units₂) =>
      (⟨(φ₁ ⟨a p, ha p⟩).1.interSamePair (φ₂ ⟨b p, hb p⟩).1
          (hP' _ (φ₁ ⟨a p, ha p⟩).2 _ (φ₂ ⟨b p, hb p⟩).2),
        mem_covers_interProdOn C₁ C₂ I hI hP'
          (φ₁ ⟨a p, ha p⟩).2 (φ₂ ⟨b p, hb p⟩).2⟩ :
        ↥(C₁.interProdOn C₂ I hI hP').covers)) := by
  rintro ⟨E, hE⟩
  ...
  · rw [insert_s_T_eq_of_generated hφ₁ ⟨a q, ha q⟩, hs₁, hT₁,
      Finset.insert_eq_self.mpr t₁.2]
  · rw [insert_s_T_eq_of_generated hφ₂ ⟨b q, hb q⟩, hs₂, hT₂,
      Finset.insert_eq_self.mpr t₂.2]
...
theorem RationalCoveringData.interProdOn_isGeneratedBy (C₁ C₂ : RationalCoveringData A)
  ...
  · -- the piece bijection
    ...
    refine ⟨fun p => ⟨(φ₁ ⟨a p, ha p⟩).1.interSamePair (φ₂ ⟨b p, hb p⟩).1
      (hP' _ (φ₁ ⟨a p, ha p⟩).2 _ (φ₂ ⟨b p, hb p⟩).2),
      mem_covers_interProdOn C₁ C₂ I hI hP'
        (φ₁ ⟨a p, ha p⟩).2 (φ₂ ⟨b p, hb p⟩).2⟩, ⟨?_, ?_⟩, ?_⟩
    ...
    · -- surjectivity: every intersection piece is hit
      exact interProdOn_pieceMap_surjective C₁ C₂ I hI P₀ hP₁ hP' φ₁ φ₂
        hφ₁_bij hφ₂_bij hφ₁ hφ₂ a ha b hb hab
    · -- field computation on each piece
      ...
      · have hT : ((φ₁ ⟨a p, ha p⟩).1.interSamePair (φ₂ ⟨b p, hb p⟩).1
        ...
        rw [hT, insert_s_T_eq_of_generated hφ₁ ⟨a p, ha p⟩,
          insert_s_T_eq_of_generated hφ₂ ⟨b p, hb p⟩]
...
theorem isOXAcyclic_interProd [HasLocLiftPowerBounded A] (Uf V : RationalCoveringData A)
  ...
  · -- SEPARATION: `x` zero on every `P ∩ Q` ⇒ (via `hV0`) zero on every `P` ⇒
    ...
    rw [RationalCoveringData.restrictTo_covers, Finset.mem_image] at hE
    obtain ⟨Q, -, rfl⟩ := hE
    have hE' := mem_covers_interProd Uf V hbase hUfP hVP hP Q.2
    simp only [RationalCoveringData.restrictTo_base]
    rw [restrictionMap_restrictionMap Uf.base P
  ...
  · -- GLUING (the A.3(3) cocycle): per-`P` glue via `hV0`, `Uf`-cocycle via `hV1`,
    ...
    rw [← restrictionMap_restrictionMap Uf.base P D₀ (Uf.hsubset P hP)
...
Derived from `tate_backward_exists`, which establishes exactly this
proof's backward data; only the final kernel step is specific to this
statement. -/
    ...
    (hT_mod : ∀ t ∈ D.T, ∃ w : ↥(restrictedMvPowerSeriesSubring m A),
      ...
      Ideal.Quotient.mk aI (algebraMap A ↥(restrictedMvPowerSeriesSubring m A) t) =
        Ideal.Quotient.mk aI (algebraMap A ↥(restrictedMvPowerSeriesSubring m A) D.s) *
          Ideal.Quotient.mk aI w) :
    RingHom.ker Φ ≤ aI := by
  obtain ⟨_, _, _, hext, _, _⟩ :=
    tate_backward_exists D Φ hΦ_cont hΦ_alg gen hΦ_X aI haI_closed hUnit hgen_mod hT_mod
  intro h hh
  have hh2 : Φ h = 0 := hh
  have hfun := congrFun hext h
  simp only [Function.comp_apply, hh2, map_zero] at hfun
...
private theorem unitCover_witness_case_pq
    ...
    (p q : A) (c_p c_q : presheafValue D₀)
    (hc_p : c_p ∈ OD.P.A₀) (hc_q : c_q ∈ OD.P.A₀)
    (hone : (1 : presheafValue D₀) ∈ OD.T)
    (hbb : D₀.canonicalMap f * D₀.canonicalMap f ∈ OD.T)
    (hLHS : ∀ _ : A, F (algebraMap A (Localization.Away DII.s) (p * q)) =
      OD.canonicalMap (D₀.canonicalMap D₀.s) *
        OD.canonicalMap (D₀.coeRingHom (divByS p D₀.s)) *
        (OD.canonicalMap (D₀.canonicalMap D₀.s) *
          OD.canonicalMap (D₀.coeRingHom (divByS q D₀.s))))
    (hRHS : F (algebraMap A (Localization.Away DII.s) DII.s) =
      OD.canonicalMap (D₀.canonicalMap D₀.s) * OD.canonicalMap (D₀.canonicalMap D₀.s) *
        OD.canonicalMap (D₀.canonicalMap f))
    (hinvO : OD.canonicalMap (D₀.canonicalMap f) *
      OD.coeRingHom (divByS (1 : presheafValue D₀) OD.s) = 1)
    (haMbb : OD.coeRingHom (divByS (D₀.canonicalMap f * D₀.canonicalMap f) OD.s) =
      OD.canonicalMap (D₀.canonicalMap f))
    (hk_p : D₀.coeRingHom (divByS p D₀.s) = c_p) (hk_q : D₀.coeRingHom (divByS q D₀.s) = c_q) :
  ...
  refine ⟨algebraMap (presheafValue D₀) (Localization.Away OD.s) c_p *
    ...
    (locSubring OD.P OD.T OD.s).mul_mem
      ...
      (divByS_mem_locSubring OD.P OD.T OD.s (hone)), ?_⟩
...
private theorem unitCover_witness_case_pFq
    ...
    (haMbb : OD.coeRingHom (divByS (D₀.canonicalMap f * D₀.canonicalMap f) OD.s) =
      OD.canonicalMap (D₀.canonicalMap f))
    (hk_p : D₀.coeRingHom (divByS p D₀.s) = c_p) (hk_q : D₀.coeRingHom (divByS q D₀.s) = c_q * D₀.canonicalMap f) :
    ∃ y : Localization.Away OD.s, y ∈ locSubring OD.P OD.T OD.s ∧
      F (divByS (p * q) DII.s) = OD.coeRingHom y := by
  refine ⟨algebraMap (presheafValue D₀) (Localization.Away OD.s) c_p *
      algebraMap (presheafValue D₀) (Localization.Away OD.s) c_q,
...
private theorem unitCover_witness_case_pqF
    ...
    (haMbb : OD.coeRingHom (divByS (D₀.canonicalMap f * D₀.canonicalMap f) OD.s) =
      OD.canonicalMap (D₀.canonicalMap f))
    (hk_p : D₀.coeRingHom (divByS p D₀.s) = c_p * D₀.canonicalMap f) (hk_q : D₀.coeRingHom (divByS q D₀.s) = c_q) :
    ∃ y : Localization.Away OD.s, y ∈ locSubring OD.P OD.T OD.s ∧
      F (divByS (p * q) DII.s) = OD.coeRingHom y := by
  refine ⟨algebraMap (presheafValue D₀) (Localization.Away OD.s) c_p *
      algebraMap (presheafValue D₀) (Localization.Away OD.s) c_q,
...
private theorem unitCover_witness_case_pFqF
    ...
    (haMbb : OD.coeRingHom (divByS (D₀.canonicalMap f * D₀.canonicalMap f) OD.s) =
      OD.canonicalMap (D₀.canonicalMap f))
    (hk_p : D₀.coeRingHom (divByS p D₀.s) = c_p * D₀.canonicalMap f) (hk_q : D₀.coeRingHom (divByS q D₀.s) = c_q * D₀.canonicalMap f) :
    ∃ y : Localization.Away OD.s, y ∈ locSubring OD.P OD.T OD.s ∧
      F (divByS (p * q) DII.s) = OD.coeRingHom y := by
  refine ⟨algebraMap (presheafValue D₀) (Localization.Away OD.s) c_p *
      algebraMap (presheafValue D₀) (Localization.Away OD.s) c_q *
    ...
    (locSubring OD.P OD.T OD.s).mul_mem
      ((locSubring OD.P OD.T OD.s).mul_mem
        (algebraMap_mem_locSubring OD.P OD.T OD.s hc_p)
        (algebraMap_mem_locSubring OD.P OD.T OD.s hc_q))
      (divByS_mem_locSubring OD.P OD.T OD.s (hbb)), ?_⟩
  refine forwardLoc_div_eq DII F hu _ _ ?_
  rw [hLHS 0, hRHS, hk_p, hk_q]
  rw [show OD.coeRingHom (algebraMap (presheafValue D₀) (Localization.Away OD.s) c_p *
      algebraMap (presheafValue D₀) (Localization.Away OD.s) c_q *
...
private theorem divByS_mul_eq (D₀ : RationalLocData A) : ∀ p' q' : A,
    divByS (p' * q') D₀.s = divByS p' D₀.s * algebraMap A (Localization.Away D₀.s) q' := by
  intro p' q'
...
private theorem unitCover_overlapDatum_B_s_eq
    ...
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
    (D₀ : RationalLocData A) (f : A) :
    ((unitCover_overlapDatum_B D₀ f).s : presheafValue D₀) = D₀.canonicalMap f := by
...
private theorem unitCover_relOverlap_forward_witness
  ...
  · -- K = 0: witness `aM c_p · aM c_q · (1/b)`
    exact unitCover_witness_case_pq D₀ f DII OD F hu p q c_p c_q hc_p hc_q
      (unitCoUnit_inter_one_mem _ _) (unitCoUnit_inter_bb_mem _ _)
      hLHS hRHS hinvO haMbb hk_p hk_q
  · -- K = 1 (q-side `b`): witness `aM c_p · aM c_q`
    exact unitCover_witness_case_pFq D₀ f DII OD F hu p q c_p c_q hc_p hc_q
      (unitCoUnit_inter_one_mem _ _) (unitCoUnit_inter_bb_mem _ _)
      hLHS hRHS hinvO haMbb hk_p hk_q
  · -- K = 1 (p-side `b`): witness `aM c_p · aM c_q`
    exact unitCover_witness_case_pqF D₀ f DII OD F hu p q c_p c_q hc_p hc_q
      (unitCoUnit_inter_one_mem _ _) (unitCoUnit_inter_bb_mem _ _)
      hLHS hRHS hinvO haMbb hk_p hk_q
  · -- K = 2: witness `aM c_p · aM c_q · (b²/b)`
    exact unitCover_witness_case_pFqF D₀ f DII OD F hu p q c_p c_q hc_p hc_q
      (unitCoUnit_inter_one_mem _ _) (unitCoUnit_inter_bb_mem _ _)
      hLHS hRHS hinvO haMbb hk_p hk_q
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- On the annulus `U₁∩U₂`, `canonicalMap f` is a unit: the denominator factors as
`(D₀.s * 1) * (D₀.s * f)`, and a factor of a unit is a unit. -/
private theorem unitCover_annulus_canonicalMap_isUnit
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- LHS of the minus `X`-case: restricting `backward (invS)` into the annulus and
multiplying by `canonicalMap f` gives `1`. -/
private theorem unitCover_minus_restriction_backward_invS_one
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- `canonicalMap f` on the annulus is the overlap-backward image of `canonicalMap f`
in the overlap datum. -/
private theorem unitCover_annulus_canonicalMap_eq_backward
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- In the overlap datum, `canonicalMap f * coeRingHom (1/s) = 1`: the denominator is
`1 * f`, so this is the unit cancellation `algebraMap s * (1/s) = 1`. -/
private theorem unitCover_overlapDatum_canonicalMap_mul_divByS_one
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- `C`-case of the minus square: on constants both composites are restriction
functoriality for `U₂ ⊇ U₁∩U₂`. -/
private theorem unitCover_sq_minus_comp_C
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
  ...
  rw [show LaurentTateAlgebra.negIncl (algebraMap (presheafValue D₀)
      ↥(TateAlgebra (presheafValue D₀)) c) =
    algebraMap (presheafValue D₀) ↥(TateAlgebra₂ (presheafValue D₀)) c
    from LaurentTateAlgebra.negIncl_algebraMap c]
  erw [mvEvalHomBounded_algebraMap]
  rw [show (unitCover_overlapDatum_B D₀ f).canonicalMap c =
    (unitCover_overlapDatum_B D₀ f).coeRingHom (algebraMap (presheafValue D₀)
      (Localization.Away ((unitCover_overlapDatum_B D₀ f).s)) c) from rfl]
...
set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- `X`-case of the minus square: both composites send the variable to the inverse of
`canonicalMap f` on the annulus. -/
private theorem unitCover_sq_minus_comp_X
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
    (D₀ : RationalLocData A) (f : A)
    (hb : TopologicalRing.IsPowerBounded (invS (unitCover_minusDatum_B D₀ f))) (i : Fin 1) :
    (((restrictionMapHom (D₀.interSamePair (coUnitDatum D₀.P f) rfl)
        ((D₀.interSamePair (unitDatum D₀.P f) rfl).interSamePair
          (D₀.interSamePair (coUnitDatum D₀.P f) rfl) (unitCover_annulus_pair_eq D₀ f))
        (RationalLocData.interSamePair_subset_right _ _ _)).comp
  ...
  refine (unitCover_annulus_canonicalMap_isUnit D₀ f).mul_left_cancel ?_
  ...
  rw [show (unitCover_overlapEval D₀ f)
    ...
    rfl]
  rw [unitCover_annulus_canonicalMap_eq_backward D₀ f, ← map_mul]
  rw [unitCover_overlapDatum_canonicalMap_mul_divByS_one D₀ f]
  rw [map_one]

set_option linter.unusedSectionVars false in
set_option backward.isDefEq.respectTransparency false in
/-- The two backward-composites agree as functions on `B⟨X⟩` (minus column). -/
private theorem unitCover_sq_minus_funext
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
  ...
  have hb : TopologicalRing.IsPowerBounded (invS (unitCover_minusDatum_B D₀ f)) :=
    isPowerBounded_invS_of_one_mem_T _ h1T
  exact RingHom.congr_fun
    (MvPolynomial.ringHom_ext (unitCover_sq_minus_comp_C D₀ f hb)
      (unitCover_sq_minus_comp_X D₀ f hb)) q
...
private theorem unitCover_sq_minus_dense
    ...
    unitCover_relOverlap_backward D₀ f
      (unitCover_overlapEval D₀ f (LaurentTateAlgebra.negIncl z)) := by
  intro z
  exact congrFun (unitCover_sq_minus_funext D₀ f) z
...
This is step 2 of the Prop A.3(1) transfer, but nothing in it is specific to that proof:
it uses only that `f` is compatible and that `choose D'` names a `C`-piece containing
`D'`. -/
private theorem restrictionMap_chosenPiece_compat
    [HasLocLiftPowerBounded A]
    (C : RationalCoveringData A)
    (f : ∀ (D : ↥C.covers), presheafValue D.1)
    (h_compat : ∀ (D₁ D₂ : ↥C.covers) (D₃ : RationalLocData A)
      ...
      (h₃₂ : rationalOpen D₃.T D₃.s ⊆ rationalOpen D₂.1.T D₂.1.s),
      restrictionMap D₁.1 D₃ h₃₁ (f D₁) = restrictionMap D₂.1 D₃ h₃₂ (f D₂))
    (K : RationalCoveringData A)
    (choose : ∀ D' : ↥K.covers,
      { D : ↥C.covers // rationalOpen D'.1.T D'.1.s ⊆ rationalOpen D.1.T D.1.s })
    (D'₁ D'₂ : ↥K.covers) (D'₃ : RationalLocData A)
    (h₃₁ : rationalOpen D'₃.T D'₃.s ⊆ rationalOpen D'₁.1.T D'₁.1.s)
    (h₃₂ : rationalOpen D'₃.T D'₃.s ⊆ rationalOpen D'₂.1.T D'₂.1.s) :
    restrictionMap D'₁.1 D'₃ h₃₁
        (restrictionMap (choose D'₁).1.1 D'₁.1 (choose D'₁).2 (f (choose D'₁).1)) =
      restrictionMap D'₂.1 D'₃ h₃₂
        (restrictionMap (choose D'₂).1.1 D'₂.1 (choose D'₂).2 (f (choose D'₂).1)) := by
  rw [restrictionMap_restrictionMap (choose D'₁).1.1 D'₁.1 D'₃ (choose D'₁).2 h₃₁,
    restrictionMap_restrictionMap (choose D'₂).1.1 D'₂.1 D'₃ (choose D'₂).2 h₃₂]
...
`hy` says only that on each piece of `C|Vⱼ` the section `y Vⱼ` is *some* `f`-value; that
is exactly the content of the step-3 gluing, and it is all step 5 needs.  Compatibility on
an arbitrary common refinement `Vj₃` reduces to the canonical intersection `I_at`, where
the `q = 1` instance of A.3's standing hypothesis supplies the acyclic cover `W_at` to
separate over — on each `W`-piece both sides become restrictions of `f`-values, and
`h_compat` closes. -/
private theorem propA3_part1_gluing_family_compat
    [HasLocLiftPowerBounded A]
    (C V : RationalCoveringData A)
    (f : ∀ (D : ↥C.covers), presheafValue D.1)
    (h_compat : ∀ (D₁ D₂ : ↥C.covers) (D₃ : RationalLocData A)
      (h₃₁ : rationalOpen D₃.T D₃.s ⊆ rationalOpen D₁.1.T D₁.1.s)
      (h₃₂ : rationalOpen D₃.T D₃.s ⊆ rationalOpen D₂.1.T D₂.1.s),
    ...
    (y : ∀ Vj : ↥V.covers, presheafValue Vj.1)
    (hy :
      ∀ (Vj : ↥V.covers) (D' : RationalLocData A), D' ∈ (C_restr_at Vj).covers →
        ∀ (hsub : rationalOpen D'.T D'.s ⊆ rationalOpen Vj.1.T Vj.1.s),
        ∃ (Dc : ↥C.covers) (hc : rationalOpen D'.T D'.s ⊆ rationalOpen Dc.1.T Dc.1.s),
          restrictionMap Vj.1 D' hsub (y Vj) = restrictionMap Dc.1 D' hc (f Dc))
    (Vj₁ Vj₂ : ↥V.covers) (Vj₃ : RationalLocData A)
    (h₃₁ : rationalOpen Vj₃.T Vj₃.s ⊆ rationalOpen Vj₁.1.T Vj₁.1.s)
    (h₃₂ : rationalOpen Vj₃.T Vj₃.s ⊆ rationalOpen Vj₂.1.T Vj₂.1.s) :
    restrictionMap Vj₁.1 Vj₃ h₃₁ (y Vj₁) = restrictionMap Vj₂.1 Vj₃ h₃₂ (y Vj₂) := by
  ...
  refine restrictionMap_eq_of_eq_on_inter Vj₁.1 Vj₂.1 (I_at Vj₁ Vj₂) Vj₃
    (hI_open Vj₁ Vj₂) (y Vj₁) (y Vj₂) hI_sub₁ hI_sub₂ h₃₁ h₃₂ ?_
  refine eq_of_restrictionMap_eq_of_isOXAcyclic (I_at Vj₁ Vj₂) (W_at Vj₁ Vj₂)
    (hW_base Vj₁ Vj₂) (hW_acyclic Vj₁ Vj₂) _ _ (fun W' hW' hsub_I => ?_)
  ...
  have hD'₁_sub : rationalOpen D'₁.T D'₁.s ⊆ rationalOpen Vj₁.1.T Vj₁.1.s := by
    ...
    rw [hC_restr_base Vj₁] at this
    exact this
  have hD'₂_sub : rationalOpen D'₂.T D'₂.s ⊆ rationalOpen Vj₂.1.T Vj₂.1.s := by
    have := (C_restr_at Vj₂).hsubset D'₂ hD'₂_in
    rw [hC_restr_base Vj₂] at this
    exact this
  obtain ⟨Dc₁, hc₁, h_inner₁⟩ := hy Vj₁ D'₁ hD'₁_in hD'₁_sub
  obtain ⟨Dc₂, hc₂, h_inner₂⟩ := hy Vj₂ D'₂ hD'₂_in hD'₂_sub
  -- Collapse the I-leg on each side, then re-factor through the containing piece.
  have e₁ : restrictionMap (I_at Vj₁ Vj₂) W' hsub_I
        (restrictionMap Vj₁.1 (I_at Vj₁ Vj₂) hI_sub₁ (y Vj₁)) =
      restrictionMap D'₁ W' hW'_sub_D'₁ (restrictionMap Vj₁.1 D'₁ hD'₁_sub (y Vj₁)) := by
    rw [restrictionMap_restrictionMap, restrictionMap_restrictionMap]
  have e₂ : restrictionMap (I_at Vj₁ Vj₂) W' hsub_I
        (restrictionMap Vj₂.1 (I_at Vj₁ Vj₂) hI_sub₂ (y Vj₂)) =
      restrictionMap D'₂ W' hW'_sub_D'₂ (restrictionMap Vj₂.1 D'₂ hD'₂_sub (y Vj₂)) := by
    rw [restrictionMap_restrictionMap, restrictionMap_restrictionMap]
  rw [e₁, e₂, h_inner₁, h_inner₂, restrictionMap_restrictionMap,
...
Existentially quantifying `y` is what keeps the choice function `chooseC` and the glued
sections `yVⱼ` local to this proof: every later step consumes only the two conclusions
below, never the constructions behind them. -/
private theorem propA3_part1_gluing_exists_family
    [HasLocLiftPowerBounded A]
    (C V : RationalCoveringData A)
    (f : ∀ (D : ↥C.covers), presheafValue D.1)
    (h_compat : ∀ (D₁ D₂ : ↥C.covers) (D₃ : RationalLocData A)
    ...
    ∃ y : ∀ Vj : ↥V.covers, presheafValue Vj.1,
      ...
      ∀ (Vj₁ Vj₂ : ↥V.covers) (Vj₃ : RationalLocData A)
        ...
        (h₃₂ : rationalOpen Vj₃.T Vj₃.s ⊆ rationalOpen Vj₂.1.T Vj₂.1.s),
        restrictionMap Vj₁.1 Vj₃ h₃₁ (y Vj₁) = restrictionMap Vj₂.1 Vj₃ h₃₂ (y Vj₂) := by
  -- For each piece of `C|Vⱼ`, a `C`-piece containing it, and `f` restricted from there.
  let chooseC : ∀ (Vj : ↥V.covers) (D' : ↥(C_restr_at Vj).covers),
      { D : ↥C.covers //
        rationalOpen D'.1.T D'.1.s ⊆ rationalOpen D.1.T D.1.s } := fun Vj D' =>
    ⟨⟨(hC_restr_pieces Vj D'.1 D'.2).choose,
       (hC_restr_pieces Vj D'.1 D'.2).choose_spec.1⟩,
     (hC_restr_pieces Vj D'.1 D'.2).choose_spec.2⟩
  ...
  have h_gVj_compat : ∀ (Vj : ↥V.covers)
      ...
      restrictionMap D'₁.1 D'₃ h₃₁ (gVj Vj D'₁) =
        restrictionMap D'₂.1 D'₃ h₃₂ (gVj Vj D'₂) := fun Vj =>
    restrictionMap_chosenPiece_compat C f h_compat (C_restr_at Vj) (chooseC Vj)
  -- Glue on each `C|Vⱼ`, then transport along `(C_restr_at Vⱼ).base = Vⱼ`.
  let yVj : ∀ (Vj : ↥V.covers), presheafValue (C_restr_at Vj).base := fun Vj =>
    ((hC_restr_acyclic Vj).gluing (gVj Vj) (h_gVj_compat Vj)).choose
  let yV : ∀ (Vj : ↥V.covers), presheafValue Vj.1 := fun Vj =>
    hC_restr_base Vj ▸ yVj Vj
  have hyV :
      ∀ (Vj : ↥V.covers) (D' : RationalLocData A), D' ∈ (C_restr_at Vj).covers →
        ∀ (hsub : rationalOpen D'.T D'.s ⊆ rationalOpen Vj.1.T Vj.1.s),
        ∃ (Dc : ↥C.covers) (hc : rationalOpen D'.T D'.s ⊆ rationalOpen Dc.1.T Dc.1.s),
          restrictionMap Vj.1 D' hsub (yV Vj) = restrictionMap Dc.1 D' hc (f Dc) := by
    intro Vj D' hD'_in hsub
    refine ⟨(chooseC Vj ⟨D', hD'_in⟩).1, (chooseC Vj ⟨D', hD'_in⟩).2, ?_⟩
    have hspec := ((hC_restr_acyclic Vj).gluing (gVj Vj)
      (h_gVj_compat Vj)).choose_spec ⟨D', hD'_in⟩
    show restrictionMap Vj.1 D' hsub
        (@Eq.rec (RationalLocData A) (C_restr_at Vj).base
          (fun b _ => presheafValue b) (yVj Vj) Vj.1 (hC_restr_base Vj)) = _
    rw [RationalCoveringData.eqRec_restrictionMap_direct (C_restr_at Vj).base Vj.1
      (hC_restr_base Vj) D' ((C_restr_at Vj).hsubset D' hD'_in) hsub (yVj Vj)]
    exact hspec
  exact ⟨yV, hyV, fun Vj₁ Vj₂ Vj₃ h₃₁ h₃₂ =>
...
Separate over the mixed acyclic cover `M_at`: each piece `E` lies in a single piece of
`C|Vⱼ`, where `hy` turns `y Vⱼ` into an `f`-value and `h_compat` matches it against
`f U`. -/
private theorem propA3_part1_gluing_mixed_trace
    [HasLocLiftPowerBounded A]
    (C V : RationalCoveringData A)
    (f : ∀ (D : ↥C.covers), presheafValue D.1)
    (h_compat : ∀ (D₁ D₂ : ↥C.covers) (D₃ : RationalLocData A)
    ...
    (hI_Vj : rationalOpen (M_at U Vj).base.T (M_at U Vj).base.s ⊆
      rationalOpen Vj.1.T Vj.1.s)
    (hI_U : rationalOpen (M_at U Vj).base.T (M_at U Vj).base.s ⊆
      rationalOpen U.1.T U.1.s) :
    restrictionMap Vj.1 (M_at U Vj).base hI_Vj (y Vj) =
      restrictionMap U.1 (M_at U Vj).base hI_U (f U) := by
  refine eq_of_restrictionMap_eq_of_isOXAcyclic (M_at U Vj).base (M_at U Vj) rfl
    (hM_acyclic U Vj) _ _ (fun E hE hE_sub_I => ?_)
  obtain ⟨D'', hD''_in, hE_sub_D''⟩ := hM_pieces U Vj E hE
  have hD''_sub_Vj : rationalOpen D''.T D''.s ⊆ rationalOpen Vj.1.T Vj.1.s := by
    have := (C_restr_at Vj).hsubset D'' hD''_in
    rw [hC_restr_base Vj] at this
    exact this
  obtain ⟨Dc, hc, h_inner⟩ := hy Vj D'' hD''_in hD''_sub_Vj
  have e : restrictionMap (M_at U Vj).base E hE_sub_I
        (restrictionMap Vj.1 (M_at U Vj).base hI_Vj (y Vj)) =
      restrictionMap D'' E hE_sub_D'' (restrictionMap Vj.1 D'' hD''_sub_Vj (y Vj)) := by
    rw [restrictionMap_restrictionMap, restrictionMap_restrictionMap]
...
Separate over the trace cover `V|U`.  Each trace piece `V'` sits inside a single `V`-piece
`Vⱼ`, so on `V'` the section becomes `y Vⱼ`; the mixed identity on `U ∩ Vⱼ` then equates
that with `f U`, and `V' ⊆ U ∩ Vⱼ` carries it down. -/
private theorem propA3_part1_gluing_restrict_eq
    [HasLocLiftPowerBounded A]
    ...
    (x : presheafValue C.base)
    (hx : ∀ (Vj : ↥V.covers)
      (h : rationalOpen Vj.1.T Vj.1.s ⊆ rationalOpen C.base.T C.base.s),
      restrictionMap C.base Vj.1 h x = y Vj)
    (U : ↥C.covers) :
    restrictionMap C.base U.1 (C.hsubset U.1 U.2) x = f U := by
  refine eq_of_restrictionMap_eq_of_isOXAcyclic U.1 (V_restr_at U) (hV_restr_base U)
    (hV_restr_acyclic U) _ _ (fun V' _ hV'_sub_U => ?_)
  obtain ⟨Vj, hVj, hV'_sub_Vj⟩ := hV_restr_pieces U V' ‹_›
  have hVj_sub_C : rationalOpen Vj.T Vj.s ⊆ rationalOpen C.base.T C.base.s := by
    rw [← hV_base]; exact V.hsubset Vj hVj
  -- On `V'` the glued section is `y Vⱼ`: factor through the containing `V`-piece.
  have e_x : restrictionMap U.1 V' hV'_sub_U
        (restrictionMap C.base U.1 (C.hsubset U.1 U.2) x) =
      restrictionMap Vj V' hV'_sub_Vj (y ⟨Vj, hVj⟩) := by
    rw [restrictionMap_restrictionMap, ← hx ⟨Vj, hVj⟩ hVj_sub_C,
      restrictionMap_restrictionMap]
  rw [e_x]
  have hI_Vj : rationalOpen (M_at U ⟨Vj, hVj⟩).base.T (M_at U ⟨Vj, hVj⟩).base.s ⊆
      rationalOpen Vj.T Vj.s := by
    rw [hM_base_open U ⟨Vj, hVj⟩]; exact Set.inter_subset_right
  have hI_U : rationalOpen (M_at U ⟨Vj, hVj⟩).base.T (M_at U ⟨Vj, hVj⟩).base.s ⊆
      rationalOpen U.1.T U.1.s := by
    rw [hM_base_open U ⟨Vj, hVj⟩]; exact Set.inter_subset_left
  exact (restrictionMap_eq_of_eq_on_inter U.1 Vj (M_at U ⟨Vj, hVj⟩).base V'
    (hM_base_open U ⟨Vj, hVj⟩) (f U) (y ⟨Vj, hVj⟩) hI_U hI_Vj hV'_sub_U hV'_sub_Vj
    (propA3_part1_gluing_mixed_trace C V f h_compat C_restr_at hC_restr_base M_at
...
theorem wedhorn_lemma_834_propA3_part1_gluing
    ...
    ∀ (f : ∀ (D : ↥C.covers), presheafValue D.1),
      ...
      ∃ x : presheafValue C.base, ∀ (D : ↥C.covers),
        restrictionMap C.base D.1 (C.hsubset D.1 D.2) x = f D := by
  intro f h_compat
  obtain ⟨yV, hyV, h_yV_compat⟩ :=
    propA3_part1_gluing_exists_family C V f h_compat C_restr_at _hC_restr_base
      _hC_restr_pieces _hC_restr_acyclic I_at hI_open W_at hW_base hW_pieces₁
      hW_pieces₂ hW_acyclic
  obtain ⟨x', hx'⟩ := _hV_acyclic.gluing yV h_yV_compat
  refine ⟨(RationalCoveringData.presheafValueCast (C := C) (C' := V) _hV_base).symm x',
    fun D => ?_⟩
  refine propA3_part1_gluing_restrict_eq C V _hV_base f h_compat V_restr_at
    _hV_restr_base _hV_restr_pieces _hV_restr_acyclic C_restr_at _hC_restr_base M_at
    hM_base_open hM_pieces hM_acyclic yV hyV _ (fun Vj h => ?_) D
  rw [← restrictionMap_presheafValueCast C V _hV_base
...
set_option linter.unusedSectionVars false in
/-- A factor of `D.s` is a unit in `Loc D.s`. -/
private theorem isUnit_algebraMap_of_mul_eq (D : RationalLocData A) {a c : A}
    (h : a * c = D.s) : IsUnit (algebraMap A (Localization.Away D.s) a) := by
  have h2 : IsUnit (algebraMap A (Localization.Away D.s) a *
      algebraMap A (Localization.Away D.s) c) := by
    rw [← map_mul, h]
    exact IsLocalization.Away.algebraMap_isUnit (S := Localization.Away D.s) D.s
  exact isUnit_of_mul_isUnit_left h2

set_option linter.unusedSectionVars false in
/-- On the image of `Loc D₀.s`, restriction `𝒪(D₀) → 𝒪(D)` is the localization lift `ρ`:
both carry `algebraMap a` to `D.canonicalMap a`, and `Loc D₀.s` is generated by those. -/
private theorem restrictionMapHom_comp_coeRingHom
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
    (D₀ D : RationalLocData A) (h : rationalOpen D.T D.s ⊆ rationalOpen D₀.T D₀.s)
    (ρ : Localization.Away D₀.s →+* Localization.Away D.s)
    (hρ_alg : ∀ a : A, ρ (algebraMap A (Localization.Away D₀.s) a) =
      algebraMap A (Localization.Away D.s) a) :
    (restrictionMapHom D₀ D h).comp D₀.coeRingHom = D.coeRingHom.comp ρ := by
  letI : UniformSpace (Localization.Away D₀.s) := D₀.uniformSpace
  letI : IsTopologicalRing (Localization.Away D₀.s) := D₀.isTopologicalRing
  letI : IsUniformAddGroup (Localization.Away D₀.s) := D₀.isUniformAddGroup
  letI : UniformSpace (Localization.Away D.s) := D.uniformSpace
...
private theorem genPiece_relOverlap_forward_restriction
  ...
  -- The two overlap data this proof lives on; spelled out they are 30 of its lines.
  set DII := (D₀.interSamePair (genPieceDatum D₀.P T t₁ hspan) rfl).interSamePair
    (genPieceDatum D₀.P T t₂ hspan) rfl
  set EII := (imagePieceDatum D₀ T t₁ hspan).interSamePair
    (imagePieceDatum D₀ T t₂ hspan) rfl
  have hsub : rationalOpen DII.T DII.s ⊆ rationalOpen D₀.T D₀.s :=
    (RationalLocData.interSamePair_subset_left _ _ _).trans
      (RationalLocData.interSamePair_subset_left _ _ _)
  letI : UniformSpace (Localization.Away D₀.s) := D₀.uniformSpace
  letI : IsTopologicalRing (Localization.Away D₀.s) := D₀.isTopologicalRing
  ...
  have h3 := isUnit_algebraMap_of_mul_eq DII (a := D₀.s * t₁) (c := t₂) rfl
  rw [map_mul] at h3
  have hbase : IsUnit (algebraMap A (Localization.Away DII.s) D₀.s) :=
    isUnit_of_mul_isUnit_left h3
  set ρ : Localization.Away D₀.s →+* Localization.Away DII.s :=
    IsLocalization.Away.lift (x := D₀.s)
      (g := algebraMap A (Localization.Away DII.s)) hbase with hρ
  have hρ_alg : ∀ a : A, ρ (algebraMap A (Localization.Away D₀.s) a) =
      algebraMap A (Localization.Away DII.s) a := by
    intro a
    rw [hρ, IsLocalization.Away.lift_eq]
  have hfactor := restrictionMapHom_comp_coeRingHom D₀ DII hsub ρ hρ_alg
  have hRT2 : (genPiece_relOverlap_forwardLocHom D₀ T hspan t₁ t₂).comp ρ =
      (algebraMap (presheafValue D₀) (Localization.Away EII.s)).comp D₀.coeRingHom := by
    refine IsLocalization.ringHom_ext (Submonoid.powers D₀.s) ?_
    ext a
    simp only [RingHom.comp_apply]
    ...
    rfl
  refine @UniformSpace.Completion.ext' (Localization.Away D₀.s) D₀.uniformSpace
    (presheafValue EII) _ _ _ _
    (UniformSpace.Completion.continuous_extension.comp
      UniformSpace.Completion.continuous_extension)
    (canonicalMap_continuous EII) ?_ x
  intro z
  show genPiece_relOverlap_forward D₀ T hspan t₁ t₂
      (restrictionMapHom D₀ DII hsub (D₀.coeRingHom z)) =
    EII.canonicalMap (D₀.coeRingHom z)
  rw [show restrictionMapHom D₀ DII hsub (D₀.coeRingHom z) =
    DII.coeRingHom (ρ z) from RingHom.congr_fun hfactor z]
...
set_option linter.unusedSectionVars false in
/-- Step of `genPiece_relative_overlap_square₂` (hcomp). -/
private theorem genPiece_relOverlap_equiv_restriction_canonicalMap
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
...
set_option linter.unusedSectionVars false in
/-- Step of `genPiece_relative_overlap_square₂` (hhom). -/
private theorem genPiece_relOverlap_equiv_comp_restrictionMapHom
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
  ...
  exact genPiece_relOverlap_equiv_restriction_canonicalMap D₀ T hspan t₁ t₂ a
  ...
  · rintro _ ⟨y, rfl⟩
    show (genPiece_relative_overlap_equiv D₀ T hspan t₁ t₂)
      ...
      restrictionMapHom (imagePieceDatum D₀ T t₂ hspan)
        ...
        ((genPiece_relative_equiv D₀ T t₂ hspan)
          ((D₀.interSamePair (genPieceDatum D₀.P T t₂ hspan) rfl).coeRingHom y))
    have hcomp := genPiece_relOverlap_equiv_restriction_canonicalMap D₀ T hspan t₁ t₂
    have hhom := genPiece_relOverlap_equiv_comp_restrictionMapHom D₀ T hspan t₁ t₂
...
private theorem ratio_pairwise_comparable_on_leaf [DecidableEq A]
    ...
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A]
    (C : RationalCoveringData A) (units : Finset A)
    (h_units : ∀ f ∈ units, IsUnit f) (ratios : List A)
    (hratios : ratios = (units.attach.toList.product units.attach.toList).map
      (fun p => (p.1 : A) * ↑((h_units p.2 p.2.2).unit⁻¹)))
    {V' : RationalLocData A} (hV'' : V' ∈ laurentProdLeaves C.base ratios) :
    ∀ f ∈ units, ∀ g ∈ units,
...
private theorem exists_max_vle_of_pairwise_comparable {V' : RationalLocData A}
    (units : Finset A)
    (hcmp : ∀ f ∈ units, ∀ g ∈ units,
      (∀ v ∈ rationalOpen V'.T V'.s, v.vle f g) ∨
      (∀ v ∈ rationalOpen V'.T V'.s, v.vle g f)) :
    ∀ (S : Finset A), S.Nonempty → S ⊆ units →
...
theorem ratio_laurent_refines_unitGen_cover [DecidableEq A]
  ...
  have hcmp := ratio_pairwise_comparable_on_leaf C units h_units ratios hratios hV''
...
set_option linter.unusedSectionVars false in
/-- The degenerate half of `genRestrictedCover_isOXAcyclic_of_units_or_empty`: when *every*
piece-trace of the generated cover is empty, so is the base trace, every section ring in
sight is the zero ring, and both acyclicity legs are `Subsingleton.elim`. -/
private theorem genRestrictedCover_isOXAcyclic_of_all_empty
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
...
set_option linter.unusedSectionVars false in
/-- σ₋-emptiness transported from `A` to `B = 𝒪_X(D₀)`: a `Spa B`-point dominated by a
σ₋-generator `t` would comap into `t`'s piece-trace over `A`, which is empty. -/
private theorem spa_presheafValue_empty_of_notMem
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    (D₀ : RationalLocData A) (T : Finset A)
    (hspan : Ideal.span (T : Set A) = ⊤) (Tpos : Finset A)
    (h_empty : ∀ t ∈ T, t ∉ Tpos →
      rationalOpen (D₀.interSamePair (genPieceDatum D₀.P T t hspan) rfl).T
        (D₀.interSamePair (genPieceDatum D₀.P T t hspan) rfl).s = ∅) :
    ∀ t ∈ T, t ∉ Tpos →
  ...
  have htrace : comap D₀.canonicalMap w ∈
    ...
    · rw [genPieceDatum_s]
      exact hnz
  rw [h_empty t ht htn] at htrace
  exact htrace

set_option linter.unusedSectionVars false in
/-- The `Tpos`-indexed rational covering of `Spa 𝒪_X(D₀)` whose pieces are cut out by the
image of `Sfin ⊆ T`.

`Sfin := Tpos` gives the `Tpos`-native cover, whose pieces are generated by the unit images
alone; `Sfin := T` gives the `Tpos`-indexed subfamily of the *full* image cover. The two
were written out separately and are the same object apart from which finset defines each
piece — hence one definition, parameterised by `Sfin`. -/
@[reducible] private noncomputable def imageSubCover
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    ...
    [DecidableEq (RationalLocData (presheafValue D₀))]
    (T Tpos Sfin : Finset A) (hSsub : Sfin ⊆ T)
    (hspanS : Ideal.span ((Sfin.image D₀.canonicalMap :
      Finset (presheafValue D₀)) : Set (presheafValue D₀)) = ⊤)
    (h_units : ∀ t ∈ Tpos, IsUnit (D₀.canonicalMap t))
    (hkey : ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
  ...
  { base := globalLocData (presheafValue_concretePair D₀)
    covers := (Tpos.image D₀.canonicalMap).image (fun u =>
      genPieceDatum (presheafValue_concretePair D₀)
        (Sfin.image D₀.canonicalMap) u hspanS)
    hsubset := by
      intro D hD v hv
      exact ⟨hv.1, fun x hx => by
        rw [Finset.mem_singleton.mp hx]
    ...
    hcover := by
      ...
      refine ⟨genPieceDatum (presheafValue_concretePair D₀)
          (Sfin.image D₀.canonicalMap) (D₀.canonicalMap t) hspanS,
      ...
      · intro k hk
        ...
        rw [genPieceDatum_s]
        exact hcond i (hSsub hi)
      · rw [genPieceDatum_s]
        exact not_vle_zero_of_isUnit (h_units t htpos) v }

set_option linter.unusedSectionVars false in
/-- The `T`-image and `Tpos`-image rational opens agree at every `Tpos`-image generator: the
σ₋-conditions are automatic on `Spa 𝒪_X(D₀)`, because every point's dominant generator
already lies in `Tpos`. -/
private theorem rationalOpen_image_eq_of_dominant_mem
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    (D₀ : RationalLocData A) [DecidableEq (presheafValue D₀)]
    (T Tpos : Finset A) (hSsub : Tpos ⊆ T)
    (hkey : ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
      ∃ t ∈ Tpos, ∀ i ∈ T, w.vle (D₀.canonicalMap i) (D₀.canonicalMap t)) :
    ∀ u ∈ Tpos.image D₀.canonicalMap,
  ...
  · rintro w ⟨hw, hcond, hnz⟩
    exact ⟨hw, fun k hk =>
      hcond k (Finset.image_subset_image hSsub hk), hnz⟩
  · rintro w ⟨hw, hcond, hnz⟩
    refine ⟨hw, ?_, hnz⟩
    intro k hk
...
set_option linter.unusedSectionVars false in
/-- The `Tpos`-native cover is `O_X`-acyclic: it is generated by the `Tpos`-image, every
member of which is a ring unit of `𝒪_X(D₀)`, so 8.34(iii)+(i) apply directly. -/
private theorem imageSubCover_isOXAcyclic_of_units
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    ...
    [DecidableEq (RationalLocData (presheafValue D₀))]
    (T Tpos : Finset A) (_hsub : Tpos ⊆ T) (hTne : Tpos.Nonempty)
    (hspanB : Ideal.span ((Tpos.image D₀.canonicalMap :
      Finset (presheafValue D₀)) : Set (presheafValue D₀)) = ⊤)
    (_h_units : ∀ t ∈ Tpos, IsUnit (D₀.canonicalMap t))
    (hkey : ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
      ∃ t ∈ Tpos, ∀ i ∈ T, w.vle (D₀.canonicalMap i) (D₀.canonicalMap t)) :
    (imageSubCover D₀ T Tpos Tpos _hsub hspanB _h_units hkey).IsOXAcyclic := by
  set C_gen := imageSubCover D₀ T Tpos Tpos _hsub hspanB _h_units hkey with C_gen_def
  ...
  have hC_gen_acyclic : C_gen.IsOXAcyclic :=
    isOXAcyclic_of_isGeneratedBy_ring_units C_gen
      ...
      (fun D hD => by
        obtain ⟨u, hu, rfl⟩ := Finset.mem_image.mp hD
        rfl)
  exact hC_gen_acyclic

set_option linter.unusedSectionVars false in
/-- The `Tpos`-indexed subfamily of the full image cover is `O_X`-acyclic: it is open-level
congruent to the `Tpos`-native cover, since the σ₋-conditions are automatic on `Spa 𝒪_X(D₀)`. -/
private theorem imageSubCover_isOXAcyclic_of_subset
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    ...
    (hspanB : Ideal.span ((Tpos.image D₀.canonicalMap :
      Finset (presheafValue D₀)) : Set (presheafValue D₀)) = ⊤)
    (hspanBT : Ideal.span ((T.image D₀.canonicalMap :
      Finset (presheafValue D₀)) : Set (presheafValue D₀)) = ⊤)
    (_h_units : ∀ t ∈ Tpos, IsUnit (D₀.canonicalMap t))
    (hkey : ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
      ∃ t ∈ Tpos, ∀ i ∈ T, w.vle (D₀.canonicalMap i) (D₀.canonicalMap t)) :
    (imageSubCover D₀ T Tpos T subset_rfl hspanBT _h_units hkey).IsOXAcyclic := by
  set C_gen := imageSubCover D₀ T Tpos Tpos _hsub hspanB _h_units hkey with C_gen_def
  set C_sub := imageSubCover D₀ T Tpos T subset_rfl hspanBT _h_units hkey with C_sub_def
  have hC_gen_acyclic : C_gen.IsOXAcyclic :=
    imageSubCover_isOXAcyclic_of_units D₀ T Tpos _hsub hTne hspanB _h_units hkey
  have hopens := rationalOpen_image_eq_of_dominant_mem D₀ T Tpos _hsub hkey
  have hC_sub_acyclic : C_sub.IsOXAcyclic := by
    ...
    · obtain ⟨u, hu, rfl⟩ := Finset.mem_image.mp hD
      ...
      rw [genPieceDatum_T, genPieceDatum_s, genPieceDatum_T, genPieceDatum_s]
      exact hopens u hu
  exact hC_sub_acyclic

set_option linter.unusedSectionVars false in
/-- The full image cover of `Spa 𝒪_X(D₀)` is `O_X`-acyclic under the σ₊-dichotomy: pad the
`Tpos`-indexed subfamily with the σ₋-pieces, which are empty at `B`.

Takes no `DecidableEq` instance binders on purpose — `imageGenCover` installs
`Classical.decEq` in its own body, so a binder here would be a *different* instance and the
`Finset.mem_image` steps against `(imageGenCover …).covers` would stop unifying. -/
private theorem imageGenCover_isOXAcyclic_of_units_or_empty
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    (D₀ : RationalLocData A)
    (T Tpos : Finset A) (hspan : Ideal.span (T : Set A) = ⊤)
    (_hsub : Tpos ⊆ T) (hTne : Tpos.Nonempty)
    (_h_units : ∀ t ∈ Tpos, IsUnit (D₀.canonicalMap t))
    (hkey : ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
      ∃ t ∈ Tpos, ∀ i ∈ T, w.vle (D₀.canonicalMap i) (D₀.canonicalMap t))
    (hB_empty : ∀ t ∈ T, t ∉ Tpos →
      ∀ w ∈ Spa (presheafValue D₀) (presheafValue D₀)⁺,
      (∀ i ∈ T, w.vle (D₀.canonicalMap i) (D₀.canonicalMap t)) → False) :
    (imageGenCover D₀ T hspan).IsOXAcyclic := by
  letI : DecidableEq (presheafValue D₀) := Classical.decEq _
  letI : DecidableEq (RationalLocData (presheafValue D₀)) := Classical.decEq _
  ...
  set C_sub := imageSubCover D₀ T Tpos T subset_rfl hspanBT _h_units hkey with C_sub_def
  have hC_sub_acyclic : C_sub.IsOXAcyclic :=
    imageSubCover_isOXAcyclic_of_subset D₀ T Tpos _hsub hTne hspanB hspanBT _h_units hkey
...
theorem genRestrictedCover_isOXAcyclic_of_units_or_empty
  ...
  case neg =>
    -- ALL piece-traces are empty, so every section ring in sight is the zero ring.
    rw [Finset.not_nonempty_iff_eq_empty] at hTne
    exact genRestrictedCover_isOXAcyclic_of_all_empty D₀ T hspan
  ...
  case pos =>
    ...
    have hB_empty := spa_presheafValue_empty_of_notMem D₀ T hspan Tpos _h_empty
    ...
    have hC_full_acyclic := imageGenCover_isOXAcyclic_of_units_or_empty D₀ T Tpos hspan
...
set_option linter.unusedSectionVars false in
/-- Trace inclusion for a doubly-restricted piece: cutting `U' ∩ Vj'` down to the `t`-piece of
the generated cover lands inside the `t`-piece of `Vj'` alone. Both sides unfold to the same
intersection of `rationalOpen`s; the left simply carries the extra `U'` constraint. -/
private theorem interSamePair_genPiece_trace_subset
    [IsTateRing A] [IsNoetherianRing A] [IsStronglyNoetherian A] [T2Space A]
    [NonarchimedeanRing A] [HasLocLiftPowerBounded A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A;
      CompleteSpace A] [DecidableEq A]
    (U' Vj' : RationalLocData A) (hP : Vj'.P = U'.P) (T : Finset A)
    (hspan : Ideal.span (T : Set A) = ⊤) (t : A) :
    rationalOpen ((U'.interSamePair Vj' hP).interSamePair
        (genPieceDatum (U'.interSamePair Vj' hP).P T t hspan) rfl).T
      ((U'.interSamePair Vj' hP).interSamePair
        (genPieceDatum (U'.interSamePair Vj' hP).P T t hspan) rfl).s ⊆
    rationalOpen (Vj'.interSamePair (genPieceDatum Vj'.P T t hspan) rfl).T
      (Vj'.interSamePair (genPieceDatum Vj'.P T t hspan) rfl).s := by
  rw [RationalLocData.interSamePair_rationalOpen Vj'
      (genPieceDatum Vj'.P T t hspan) rfl,
    RationalLocData.interSamePair_rationalOpen
      (U'.interSamePair Vj' (hP))
      (genPieceDatum (U'.interSamePair Vj' (hP)).P T t
        hspan) rfl,
    RationalLocData.interSamePair_rationalOpen U' Vj' (hP),
    genPieceDatum_T, genPieceDatum_s, genPieceDatum_T, genPieceDatum_s]
