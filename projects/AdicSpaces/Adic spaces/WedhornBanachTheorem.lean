...
open scoped Pointwise in
/-- In a Tate ring, `0` has a neighbourhood consisting entirely of topologically nilpotent
elements: dilate the (open) power-bounded subring by a topologically nilpotent unit. -/
private theorem exists_nhds_zero_forall_isTopologicallyNilpotent
    (A : Type u) [CommRing A] [UniformSpace A] [IsUniformAddGroup A]
  ...
  refine ⟨(ϖ : A) • (TopologicalRing.powerBoundedSubring A : Set A),
    (ϖ.isUnit.isOpenMap_smul _ P.isOpen_powerBoundedSubring).mem_nhds
      ⟨0, (TopologicalRing.powerBoundedSubring.toSubring A).zero_mem, smul_zero _⟩, ?_⟩
  rintro _ ⟨b, hb, rfl⟩
  simp only [smul_eq_mul]
  rw [mul_comm]
  exact hb.isTopologicallyNilpotent_mul hϖ_nil

/-- The linear map `a ↦ ∑ᵢ aᵢ • gᵢ` attached to a spanning family of a complete Tate-module is
open, by the faithful open mapping theorem `wedhorn_6_16_of_topNilpUnit`. -/
private theorem exists_isOpenMap_linearCombination
    {A : Type u} [CommRing A] [UniformSpace A] [IsUniformAddGroup A]
      [CompleteSpace A] [(uniformity A).IsCountablyGenerated] [T2Space A]
      [IsTateRing A]
    {V : Type*} [AddCommGroup V] [Module A V] [UniformSpace V] [IsUniformAddGroup V]
      [CompleteSpace V] [(uniformity V).IsCountablyGenerated] [T2Space V] [ContinuousSMul A V]
    {n : ℕ} (g : Fin n → V) (hg_span : Submodule.span A (Set.range g) = ⊤) :
    ∃ π : (Fin n → A) →ₗ[A] V, (∀ a, π a = ∑ i, a i • g i) ∧ IsOpenMap π := by
  let π : (Fin n → A) →ₗ[A] V :=
  ...
  have hπ_surj : Function.Surjective π := by
    ...
    exact ⟨c, hc⟩
  obtain ⟨ϖ, hϖ_nil⟩ := ‹IsTateRing A›.exists_topologicallyNilpotent_unit
  exact ⟨π, fun _ => rfl, wedhorn_6_16_of_topNilpUnit hϖ_nil ϖ.isUnit π hπ_cont hπ_surj⟩

/-- **BGR §3.7.2/1, intrinsic form**: a dense submodule of a module-finite complete Tate-module is
everything. The open mapping theorem dilates a topologically nilpotent neighbourhood of `0` in `Aⁿ`
onto one in `V`, so density writes each generator as `gᵥ = m'ᵥ + ∑ⱼ aᵥⱼ • gⱼ` with `m'ᵥ ∈ N'` and
every `aᵥⱼ` topologically nilpotent; matrix Nakayama in `V ⧸ N'` then forces every `gᵥ ∈ N'`. -/
theorem eq_top_of_dense_of_finite
    {A : Type u} [CommRing A] [UniformSpace A] [IsUniformAddGroup A]
      [CompleteSpace A] [(uniformity A).IsCountablyGenerated] [T2Space A]
      [IsTateRing A]
    {V : Type*} [AddCommGroup V] [Module A V] [UniformSpace V] [IsUniformAddGroup V]
      [CompleteSpace V] [(uniformity V).IsCountablyGenerated] [T2Space V] [ContinuousSMul A V]
      [Module.Finite A V]
    (N' : Submodule A V) (hN'_dense : Dense (N' : Set V)) : N' = ⊤ := by
  classical
  obtain ⟨n, g, hg_span⟩ := Module.Finite.exists_fin (R := A) (M := V)
  obtain ⟨π, hπ_apply, hπ_open⟩ := exists_isOpenMap_linearCombination g hg_span
  ...
  have hextract : ∀ v : Fin n, ∃ a : Fin n → A, (∀ j, a j ∈ W) ∧
    ...
    intro v
    have hnb : (fun z : V => g v - z) ⁻¹' Ω ∈ nhds (g v) := by
      have hcont : Continuous (fun z : V => g v - z) := continuous_const.sub continuous_id
    ...
    have hpa : (∑ j, a j • g j) = g v - w := by rw [← hπ_apply]; exact ha_eq
    rw [hpa]; abel
  choose a haW m' hm'N' hrel using hextract
  -- Matrix Nakayama in `V ⧸ N'`: the relation becomes `ȳᵥ = ∑ⱼ Ãᵥⱼ • ȳⱼ`, so every `ȳᵥ = 0`.
  have hy : ∀ v, N'.mkQ (g v) = ∑ j, Matrix.of a v j • N'.mkQ (g j) := by
    intro v
    have hq0 : N'.mkQ (m' v) = 0 := by
      simpa using (Submodule.Quotient.mk_eq_zero N').2 (hm'N' v)
    simp only [Matrix.of_apply]
    rw [hrel v, map_add, hq0, map_sum, zero_add]
    simp only [map_smul]
  have hzero := eq_zero_of_forall_eq_sum_topNilp_smul (B := Matrix.of a)
    (fun i j => hW_tn _ (haW i j)) hy
  refine eq_top_iff.2 ?_
  rw [← hg_span]
  exact Submodule.span_le.2 (Set.range_subset_iff.2 fun v =>
    (Submodule.Quotient.mk_eq_zero N').1 (by simpa using hzero v))
...
theorem fg_topologicalClosure_isClosed
  ...
  have hN'_dense : Dense (N' : Set ↥Nbar) := by
    ...
    exact x.2
  -- So `N' = ⊤`, i.e. `N̄ ≤ N`; with `N ≤ N̄` this exhibits `N` as the closed set `N̄`.
  have hN'_top : N' = ⊤ := eq_top_of_dense_of_finite N' hN'_dense
  have hNbar_le_N : Nbar ≤ N := by
    intro m hm
    have hmem : (⟨m, hm⟩ : ↥Nbar) ∈ N' := by rw [hN'_top]; exact Submodule.mem_top
    rw [hN'_def, Submodule.mem_comap] at hmem
    simpa using hmem
  have hNbar_eq : Nbar = N := le_antisymm hNbar_le_N hN_le_Nbar
  rw [← hNbar_eq]
  exact hNbar_closed
...
/-- If a neighbourhood of `0` in `M_∞` lands inside `chain k₀`, then all of
`M_∞` does: scale any element into the neighbourhood by a topologically
nilpotent unit, then scale back. -/
private theorem le_chain_of_nhds_preimage_subset {A : Type u} [CommRing A]
    [TopologicalSpace A] [IsTopologicalRing A] [IsTateRing A]
    {M : Type*} [AddCommGroup M] [Module A M] [UniformSpace M]
    [IsUniformAddGroup M] [ContinuousSMul A M]
    {chain : ℕ → Submodule A M} {M_inf : Submodule A M} {k₀ : ℕ}
    {V₀ : Set ↥M_inf} (hV₀_nhds : V₀ ∈ nhds (0 : ↥M_inf))
    (hV₀_sub : V₀ ⊆ (Subtype.val : ↥M_inf → M) ⁻¹' (chain k₀ : Set M)) :
    M_inf ≤ chain k₀ := by
...
theorem _sub_lemma_L3_2_baire_chain_submodule
  ...
  have h_inf_sub := le_chain_of_nhds_preimage_subset hV₀_nhds hV₀_sub
