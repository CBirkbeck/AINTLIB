...
precomposition with `E.coeRingHom`, because on the `algebraMap`-range both carry `a` to
`(imagePieceDatum …).canonicalMap (D₀.canonicalMap a)`. The square itself is this identity
