...
theorem mul_mem_addClosure_mul_range_pow (B : Subring A) (a : A) {x y : A}
  ...
  | zero_left _ _ => simp
  | zero_right _ _ => simp
  | add_left _ _ _ _ _ _ ih₁ ih₂ => rw [add_mul]; exact (AddSubgroup.closure _).add_mem ih₁ ih₂
  | add_right _ _ _ _ _ _ ih₁ ih₂ => rw [mul_add]; exact (AddSubgroup.closure _).add_mem ih₁ ih₂
  | neg_left _ _ _ _ ih₁ => rw [neg_mul]; exact (AddSubgroup.closure _).neg_mem ih₁
  | neg_right _ _ _ _ ih₁ => rw [mul_neg]; exact (AddSubgroup.closure _).neg_mem ih₁
...
theorem isBounded_closure_finset_of_isPowerBounded [IsTopologicalRing A]
  ...
  | @insert a S ha ih =>
    ...
    have hC_mul : ∀ x ∈ AddSubgroup.closure BM, ∀ y ∈ AddSubgroup.closure BM,
      ...
      rw [hBM]
      exact fun _ hx _ hy ↦ mul_mem_addClosure_mul_range_pow B_old a hx hy
