...
/-! ### Generalities: valid data present rational subsets -/
...
theorem RationalLocData.isRationalSubset_rationalOpen {B : Type*} [CommRing B] [TopologicalSpace B]
    [IsTopologicalRing B] [PlusSubring B] {D : RationalLocData B} (hD : D.IsRational) :
    IsRationalSubset (rationalOpen D.T D.s) := ⟨D.T, D.s, hD, rfl⟩

/-! ### The clean membership theorem for the image datum -/

omit [T2Space A]
  [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A; CompleteSpace A]
  [IsRingOfIntegralElements (A⁺ : Subring A)] in
...
theorem imgDatum_mem_rationalOpen_iff (D E : RationalLocData A)
  ...
  -- the conditions transport verbatim through `comap_vle`
  constructor
  · rintro ⟨hspa, hcond, hnz⟩
    refine ⟨hspa, comap_mem_spa (canonicalMap_continuous D)
      D.canonicalMap_integral hspa, fun t ht ↦ ?_, fun h0 ↦ ?_⟩
    · simpa only [comap_vle, imgDatum_s] using
        hcond (D.canonicalMap t) (by rw [imgDatum_T]; exact Finset.mem_image_of_mem _ ht)
    · exact hnz (by simpa only [imgDatum_s, comap_vle, map_zero] using h0)
  · rintro ⟨hspa, -, hcond, hnz⟩
    refine ⟨hspa, fun x hx ↦ ?_, fun h0 ↦ ?_⟩
    · rw [imgDatum_T, Finset.mem_image] at hx
      obtain ⟨t, ht, rfl⟩ := hx
      simpa only [imgDatum_s, comap_vle] using hcond t ht
    · exact hnz (by simpa only [imgDatum_s, comap_vle, map_zero] using h0)
...
private theorem exists_coeRingHom_eq_add_pow_mul (D : RationalLocData A) (u : (presheafValue D)ˣ)
    (n : ℕ) (x : presheafValue D) : ∃ l : Localization.Away D.s,
      ∃ b ∈ ((presheafValue D)⁺ : Subring (presheafValue D)),
        D.coeRingHom l = x + (u : presheafValue D) ^ n * b := by
  obtain ⟨u', hu'⟩ := (u.isUnit.pow n).exists_left_inv
  have hO_open : IsOpen ((fun y => u' * (y - x)) ⁻¹'
      (((presheafValue D)⁺ : Subring (presheafValue D)) : Set (presheafValue D))) :=
    ((inferInstance : IsRingOfIntegralElements
        ((presheafValue D)⁺ : Subring (presheafValue D))).isOpen).preimage
      (continuous_const.mul (continuous_id.sub continuous_const))
  have hxO : x ∈ (fun y => u' * (y - x)) ⁻¹'
      (((presheafValue D)⁺ : Subring (presheafValue D)) : Set (presheafValue D)) := by
    simpa only [Set.mem_preimage, sub_self, mul_zero, SetLike.mem_coe] using
      Subring.zero_mem _
  ...
  exact ⟨l, u' * (D.coeRingHom l - x), hl, by
    linear_combination (x - D.coeRingHom l) * hu'⟩

omit [T2Space A] [NonarchimedeanRing A] [IsRingOfIntegralElements (A⁺ : Subring A)]
  [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A; CompleteSpace A] in
private theorem exists_canonicalMap_indexedRationalSet_eq (D : RationalLocData A) {ι : Type*}
    [Fintype ι] (t : Finset ι) (lf : ι → Localization.Away D.s) (lg : Localization.Away D.s) :
    ∃ (a : ι → A) (q : A), indexedRationalSet (presheafValue D) t (fun i => D.canonicalMap (a i))
      (D.canonicalMap q) = indexedRationalSet (presheafValue D) t (fun i => D.coeRingHom (lf i))
      (D.coeRingHom lg) := by
  obtain ⟨b, hb⟩ := IsLocalization.exist_integer_multiples (Submonoid.powers D.s)
    (Finset.univ : Finset (Option ι)) (fun o => o.elim lg lf)
  choose a ha using fun i : ι =>
    (hb (some i) (Finset.mem_univ _) :
      ∃ a : A, algebraMap A (Localization.Away D.s) a = (b : A) • lf i)
  obtain ⟨q, hq⟩ : ∃ q : A, algebraMap A (Localization.Away D.s) q = (b : A) • lg :=
    hb none (Finset.mem_univ _)
  set U : presheafValue D := D.coeRingHom (algebraMap A (Localization.Away D.s) (b : A))
  have hqparam : D.canonicalMap q = U * D.coeRingHom lg := by
    have hstep := congrArg D.coeRingHom hq
    rwa [Algebra.smul_def, map_mul] at hstep
  have hparam : (fun i => D.canonicalMap (a i)) = fun i => U * D.coeRingHom (lf i) :=
    funext fun i => by
      have hstep := congrArg D.coeRingHom (ha i)
      rwa [Algebra.smul_def, map_mul] at hstep
  refine ⟨a, q, ?_⟩
  rw [hparam, hqparam]
  exact indexedRationalSet_unit_mul_eq
    ((IsLocalization.map_units (Localization.Away D.s) b).map D.coeRingHom) t _ _

omit [T2Space A] [NonarchimedeanRing A] [IsRingOfIntegralElements (A⁺ : Subring A)]
  [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A; CompleteSpace A] in
private theorem rationalOpen_eq_imgDatum_of_vle (D : RationalLocData A)
    (E : RationalLocData (presheafValue D)) (a : ↥E.T → A) (q x : A) (W : RationalLocData A)
    (hspanW : Ideal.span (W.T : Set A) = ⊤) (hWT : W.T = insert x (E.T.attach.image a))
    (hWs : W.s = q) (hchain : rationalOpen E.T E.s = indexedRationalSet (presheafValue D) E.T.attach
      (fun i => D.canonicalMap (a i)) (D.canonicalMap q)) (hx : ∀ w ∈ spaOpen E,
      (w.1 : Spv (presheafValue D)).vle (D.canonicalMap x) (D.canonicalMap q)) :
    rationalOpen E.T E.s = rationalOpen (imgDatum D W hspanW).T (imgDatum D W hspanW).s := by
  rw [imgDatum_T, imgDatum_s, hWT, hWs]
  ext w
  constructor
  · intro hw
    have hw' := hw
    ...
    refine ⟨hspa, fun t' ht' => ?_, h0⟩
    obtain ⟨t, ht, rfl⟩ := Finset.mem_image.mp ht'
    rcases Finset.mem_insert.mp ht with rfl | ht₀
    · -- the padded parameter: dominated by the denominator on the upstairs subset
      exact hx ⟨w, hspa⟩ hw
  ...
  · intro hw
    obtain ⟨hspa, hT, h0⟩ := hw
    rw [hchain]
    refine ⟨hspa, fun i _ => ?_, h0⟩
    exact hT (D.canonicalMap (a i)) (Finset.mem_image_of_mem _
      (Finset.mem_insert_of_mem (Finset.mem_image_of_mem a (Finset.mem_attach _ i))))
...
theorem exists_downstairs_rationalDatum (D : RationalLocData A) (_hD : D.IsRational)
    (E : RationalLocData (presheafValue D)) (hE : E.IsRational) :
    ∃ (W : RationalLocData A) (hW : W.IsRational), rationalOpen E.T E.s =
      rationalOpen (imgDatum D W hW.span_eq_top).T (imgDatum D W hW.span_eq_top).s := by
  ...
  -- denominators with a unit of the localization, then pad the downstairs numerator
  -- family with `π^N` — a topologically nilpotent unit power uniformly dominated by
  ...
  have hins : Ideal.span ((insert E.s (E.T.attach.image Subtype.val) :
    ...
    exact top_unique (hE.span_eq_top.symm.trans_le
      (Ideal.span_mono (Finset.coe_subset.mpr (Finset.subset_insert _ _))))
  ...
  choose lf bf hbf hlf using fun i : ↥E.T =>
    exists_coeRingHom_eq_add_pow_mul D u (M + 1) i.val
  obtain ⟨lg, bg, hbg, hlg⟩ := exists_coeRingHom_eq_add_pow_mul D u (M + 1) E.s
  ...
  obtain ⟨a, q, hclear⟩ := exists_canonicalMap_indexedRationalSet_eq D E.T.attach lf lg
  have hchain : rationalOpen E.T E.s =
      indexedRationalSet (presheafValue D) E.T.attach
        (fun i => D.canonicalMap (a i)) (D.canonicalMap q) :=
    (hbase.trans (indexedRationalSet_perturb_eq u.isUnit hu hM
      (fun i _ => ⟨bf i, hbf i, hlf i⟩) ⟨bg, hbg, hlg⟩)).trans hclear.symm
  ...
  have hq0 : ∀ w ∈ spaOpen E,
    ...
    rw [hchain] at hwE
    exact hwE.2.2
  obtain ⟨N, hN⟩ := exists_uniform_pow_vle_on_compact (isCompact_spaOpen E hE)
    (hπ.map (canonicalMap_continuous D)) hq0
  -- Steps 9–11: pad the downstairs family with the unit power
  have hspanW : Ideal.span ((insert ((π : A) ^ N) (E.T.attach.image a) :
      Finset A) : Set A) = ⊤ :=
    Ideal.eq_top_of_isUnit_mem _
      (Ideal.subset_span (Finset.mem_insert_self _ _)) (π.isUnit.pow N)
  exact ⟨genPieceDatum D.P (insert ((π : A) ^ N) (E.T.attach.image a)) q hspanW,
    RationalLocData.isRational_of_span_eq_top hspanW,
    rationalOpen_eq_imgDatum_of_vle D E a q ((π : A) ^ N) _ _ rfl rfl hchain
      (fun w hw => by simpa using hN w hw)⟩
...
theorem comap_image_imgDatum_rationalOpen (D : RationalLocData A) (W : RationalLocData A)
    (hspanW : Ideal.span (W.T : Set A) = ⊤) :
    comap D.canonicalMap '' rationalOpen (imgDatum D W hspanW).T (imgDatum D W hspanW).s =
  ...
  · rintro ⟨hvD, hvW⟩
    ...
    exact ⟨w, (imgDatum_mem_rationalOpen_iff D W hspanW w).mpr ⟨hwspa, hwcomap ▸ hvW⟩, hwcomap⟩
...
theorem setOf_comap_mem_eq_imgDatum_rationalOpen (D : RationalLocData A) (W : RationalLocData A)
    (hspanW : Ideal.span (W.T : Set A) = ⊤) :
    {w : Spv (presheafValue D) | w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
...
private theorem forward_subset (V : RationalSubset (presheafValue D)) :
    comap D.canonicalMap '' V.1 ⊆ rationalOpen D.T D.s :=
  Set.image_subset_iff.mpr fun w hw ↦ (comap_canonicalMap_mem_rationalOpen_inter_spa D
    ⟨w, V.2.hasRationalPresentation.subset_spa hw⟩).1
...
private theorem forward_rational (V : RationalSubset (presheafValue D)) :
  ...
  have hspanT : Ideal.span (T : Set (presheafValue D)) = ⊤ := span_eq_top_of_isOpen_span hopen
  obtain ⟨W, hW, hWeq⟩ := exists_downstairs_rationalDatum D hD
    (genPieceDatum (presheafValue_concretePair D) T s hspanT)
    (RationalLocData.isRational_of_span_eq_top hspanT)
  rw [hpres.trans hWeq, comap_image_imgDatum_rationalOpen D W hW.span_eq_top,
...
private theorem backward_rational (V : {V : RationalSubset A // V.1 ⊆ rationalOpen D.T D.s}) :
    IsRationalSubset {w : Spv (presheafValue D) | w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
  ...
  -- ascribed `have`: its `exact` bridges `rationalOpen T s` ≡ genPieceDatum-projections by
  -- defeq; a goal-level `rw` with the setOf lemma would miss the match (reducibility)
  have hcarrier : {w : Spv (presheafValue D) | w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
    ...
    rw [hpres]
    exact setOf_comap_mem_eq_imgDatum_rationalOpen D (genPieceDatum D.P T s hspanT) hspanT
  rw [hcarrier]
  exact RationalLocData.isRationalSubset_rationalOpen
...
def spaPresheafValueRationalSubsetEquiv : RationalSubset (presheafValue D) ≃
  ...
  left_inv V := by
    ...
    show {w : Spv (presheafValue D) | w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
        comap D.canonicalMap w ∈ comap D.canonicalMap '' V.1} = V.1
    ext w
    constructor
    · rintro ⟨hspa, w', hw', hcomap⟩
      exact comap_canonicalMap_injOn_spa D (V.2.hasRationalPresentation.subset_spa hw') hspa
        hcomap ▸ hw'
    · exact fun hw ↦ ⟨V.2.hasRationalPresentation.subset_spa hw, w, hw, rfl⟩
  right_inv V := by
    refine Subtype.ext (Subtype.ext ?_)
    show comap D.canonicalMap '' {w : Spv (presheafValue D) |
        w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
        comap D.canonicalMap w ∈ (V : RationalSubset A).1} = (V : RationalSubset A).1
    ext v
    constructor
    · rintro ⟨w, ⟨hspa, hcomap⟩, rfl⟩
      exact hcomap
    · intro hv
      obtain ⟨w, hwspa, hwcomap⟩ := exists_spa_presheafValue_of_rationalOpen D (V.2 hv)
      exact ⟨w, ⟨hwspa, hwcomap ▸ hv⟩, hwcomap⟩
...
@[simp] theorem spaPresheafValueRationalSubsetEquiv_apply_coe
    ...
    ((spaPresheafValueRationalSubsetEquiv D hD V).1).1 = comap D.canonicalMap '' V.1 := rfl
...
@[simp] theorem spaPresheafValueRationalSubsetEquiv_symm_apply_coe
    ...
    ((spaPresheafValueRationalSubsetEquiv D hD).symm V).1 = {w : Spv (presheafValue D) |
      w ∈ Spa (presheafValue D) (presheafValue D)⁺ ∧
