...
/-- **Every valid datum carries a power certificate at any pair**: the open
tray span contains an open image of any pair's ideal power. -/
theorem exists_pow_le_of_isRational_pair (P : PairOfDefinition A)
    (D : RationalLocData A) (hD : D.IsRational) :
    ∃ M : ℕ, (Ideal.span (P.A₀.subtype '' (P.I : Set P.A₀))) ^ M
      ≤ Ideal.span (D.T : Set A) := by
  have hbasis := P.hasBasis_nhds_zero
...
theorem exists_pow_le_of_isRational (D : RationalLocData A)
    ...
    ∃ M : ℕ, (Ideal.span (D.P.A₀.subtype '' (D.P.I : Set D.P.A₀))) ^ M
      ≤ Ideal.span (D.T : Set A) :=
  exists_pow_le_of_isRational_pair D.P D hD
