...
This is the whole content of `mem_plus_of_forall_spa_vle_one`, which is that statement's
contrapositive plus one line. Extracted so the construction has a name of its own. -/
private theorem exists_spa_point_not_vle_one
    [IsTateRing A] [T2Space A] [NonarchimedeanRing A]
    [letI : UniformSpace A := IsTopologicalAddGroup.rightUniformSpace A; CompleteSpace A]
    [IsRingOfIntegralElements (A⁺)]
    (D' : RationalLocData A) (x : presheafValue D') (hxnot : x ∉ (presheafValue D')⁺) :
...
theorem mem_plus_of_forall_spa_vle_one
  ...
  -- contradicting `hx`.
  by_contra hxnot
  obtain ⟨w, hw_spa, hw⟩ := exists_spa_point_not_vle_one D' x hxnot
