...
- **Status**: done (2026-08-11 — CechRationalCover.lean complete: def-layer + cechD_comp_cechD + cechD_comp_cechAug, 0 warnings. AUDIT NOTE, binding for T633-T637: `CechCohomology.lean` (1067 L) ALREADY HAS the abstract all-degree layer — `AbPresheaf X`/`FiniteCover X ι`, `CechCochain`/`cechDiff` + **`cechDiff_comp_cechDiff` PROVEN** (Finset.sum_involution double-sum), `cechAug` + `cechDiff_comp_cechAug`, **`IsAcyclic` = Definition A.1 all-degree** (:463), refinement `cochainMap` + `cochainMap_comm_diff`, `isAcyclic_of_components`, single-cover lemmas, and **A.3(3) at degree ≤ 0 only** (`isDegreeZeroAcyclic_prod` :996). REVISED PLAN: (i) my CechRationalCover.lean def-layer stays as the RationalLocData-native carrier (its cechD/cechAug mirror the abstract ones; the dd=0/d-aug=0 identities should be obtained by MIRRORING the :382/:656 proofs verbatim — same combinatorics, restrictionMapHom instead of F.res — OR by an adapter iso to the abstract complex); (ii) T634 (A.3 all-degree engine) should be built AT THE ABSTRACT (AbPresheaf, FiniteCover) LEVEL in/next to CechCohomology.lean, extending isDegreeZeroAcyclic_prod's pattern to all degrees (the cited Ȟq(U)=Ȟq(V) double-complex comparison); (iii) NEXT ENTRY POINT: grep how WedhornCechAcyclicity instantiates AbPresheaf/FiniteCover at the structure presheaf (the :12766 note says the IsOXAcyclic↔IsAcyclic bridge is NOT stated — decide whether T637's SES lives at the abstract complex with the PresheafIdentification values or at CechRationalCover's native complex). def-layer LANDED in CechRationalCover.lean (tupleDatum/cechO/cechD/cechAug/IsCechAcyclicFull, 0 warnings); REMAINING: the simplicial identities cechD_comp_cechD = 0 (via restrictionMap_comp + Fin.succAbove_succAbove-swap double-sum cancellation) and cechD_comp_aug = 0 (two-face agreement via restrictionMap_comp)) · **Depends on**: T631 · **Type**: def-layer
...
- **Status**: open (ENTRY NOTE 2026-08-11: build at the abstract (AbPresheaf X, FiniteCover X ι) level next to CechCohomology.lean. Bridges that already exist: `RationalCoveringData.toFiniteCover` (WedhornCech:12725, carrier = ↥(rationalOpen base)) + a `Refinement` bridge (:12751); the structure-presheaf-as-AbPresheaf wrap was deliberately NOT done (:12766 note) — decide there whether to wrap (making CechCohomology's complex directly usable for rational covers, with an iso to CechRationalCover's native complex) or to state A.3 abstractly and instantiate. The engine: Čech double complex of two covers, acyclic-columns/rows ⟹ the two edge augmentations are quasi-isos ⟹ Ȟq(U) ≅ Ȟq(V) under the A.3 bi-acyclicity hypotheses; then A.3(1)(2)(3) + Remark A.2 as corollaries (isDegreeZeroAcyclic_prod :996 is the deg-0 template).) · **Depends on**: T632 · **Type**: theorem stack
