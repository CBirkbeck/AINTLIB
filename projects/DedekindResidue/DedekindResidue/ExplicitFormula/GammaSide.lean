...
/-- **Pointwise integral decomposition** for the truncated Fourier integral of `gammaFT`:
for `n ≥ 1` the integral over `(-n, n]` splits into the unit-window piece, an `F 0`-scaled
sine window, and the outer `F`-tail. -/
...
/-- **Limit of the decomposed truncated integral**: the pieces converge to `gammaFT F t`
(unit window constant, sine window via `tendsto_integral_cexp_sub_div_window'`, `F`-tail via
monotone convergence over the exhausting union). -/
