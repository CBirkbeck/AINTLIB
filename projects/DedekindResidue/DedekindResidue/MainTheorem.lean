...
/-- The key algebraic identity behind the `(√L + 2)²` zero-sum bound: at `σ = 1 + 1/s` and
`L = s²`, the Landau–Stark master expression `(2σ − 1)·(L + 2/(σ − 1))` collapses to `(s + 2)²`. -/
...
/-- **SP2 zero-sum bound.** The combined explicit-formula zero sums of `ζ_K` and `ζ_ℚ` at `s = 1`
are bounded by `(√(log Δ_K) + 2)²`. This packages the `σ = 1 + 1/√L` instantiation of the
Landau–Stark estimate (`landau_stark_estimate`, `dSigma_ge`) together with the rational zero-sum
bound (`zeroSumSigma_rat_le`). -/
private lemma zeroSum_le_sq {K : Type*} [Field K] [NumberField K]
...
/-- **Archimedean β-piece bound.** For `X ≥ 69` the truncated archimedean kernel factor
`(1/2 + 1/log(X/9))·e^{log(X/9)/2}·L(log(X/9))` is bounded by `6/√X`. This unfolds
`e^{log(X/9)/2} = √(X/9) = √X/3` and applies `beta_bound_half`. -/
...
/-- **Right-hand side normal form.** Clearing denominators, the scaled target bound
`(4/3)·√X·log(3X)·RHS` equals the sum of the zero-sum coefficient term (a multiple of `(s+2)²`)
and the archimedean term. Stated with `L = s²` already substituted. -/
private lemma rhs_expand {X s nR : ℝ} (hs0 : 0 < s)
    (hsX : (0:ℝ) < Real.sqrt X) (hl3X : (0:ℝ) < Real.log (3*X))
    (hT'0 : (0:ℝ) < Real.log (X/9)) :
...
theorem belabas_friedman_thm1 {K : Type*} [Field K] [NumberField K]
  ...
  have hs0 : (0:ℝ) < s := by linarith
  have hs2 : s^2 = L := Real.sq_sqrt hL0.le
  -- ### the zero sums are bounded by `(s+2)²` (SP2, via the σ-instantiated Landau–Stark estimate)
  have hZS : zeroSumSigma K 1 + zeroSumSigma ℚ 1 ≤ (s+2)^2 := by
    rw [hs_def, hL_def]; exact zeroSum_le_sq hn hGRH hRH
  -- ### the β-piece: `(1/2 + 1/T')·e^{T'/2}·L(T') ≤ 6/√X`
  have hβ6 : (1/2 + 1/Real.log (X/9)) * Real.exp (Real.log (X/9)/2)
      * archKernelL (Real.log (X/9)) ≤ 6 / Real.sqrt X := beta_piece_le hX
  ...
  have hRHSexpand : (4/3) * Real.sqrt X * Real.log (3*X)
      * (2.324 * L / (Real.sqrt X * Real.log (3 * X))
      ...
      = (4/3) * 2.324 * (1 + 3.88 * (1/Real.log (X/9))) * (s+2)^2
        + (4/3) * 2.324 * 4.26 * (nR - 1) / Real.sqrt X := by
    rw [← hs2]; exact rhs_expand hs0 hsX hl3X hT'0
  -- ### chain and cancel
