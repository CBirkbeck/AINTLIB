...
/-- **The affine ↔ Jacobian transport of `[m]`**: scalar multiplication of an affine point
commutes with the `Jacobian.Point` round-trip, so `m • (x₀, y₀)` is the affine lift of
`m • fromAffine (x₀, y₀)`.  Pure round-trip identity (no division polynomials): push `[m]`
through the additive equivalence `toAffineAddEquiv` (`map_zsmul`) and cancel
`toAffineLift ∘ fromAffine = id` via the right inverse of `toAffineAddEquiv`. -/
private theorem zsmul_some_eq_toAffineLift_zsmul_fromAffine (m : ℤ) {x₀ y₀ : F₀}
...
/-- **The Jacobian lift of `[m] • fromAffine` is `toAffine ∘ smulEval`**: the underlying
Jacobian point of `m • fromAffine (x₀, y₀)` is the division-polynomial evaluation
`smulEval V x₀ y₀ m` (`zsmul_eq_smulEval`), so its affine lift equals `toAffine` of that
representative. -/
private theorem toAffineLift_zsmul_fromAffine_eq_toAffine_smulEval (m : ℤ) {x₀ y₀ : F₀}
    (h_ns : V.toAffine.Nonsingular x₀ y₀) :
    WeierstrassCurve.Jacobian.Point.toAffineLift
        (m • WeierstrassCurve.Jacobian.Point.fromAffine
          (Affine.Point.some x₀ y₀ h_ns)) =
...
theorem zsmul_affine_point_eq_general (m : ℤ) {x₀ y₀ : F₀}
  ...
  rw [zsmul_some_eq_toAffineLift_zsmul_fromAffine V m h_ns,
    toAffineLift_zsmul_fromAffine_eq_toAffine_smulEval V m h_ns,
