...
has `pointValuation < 1` at `P`. The first factor has valuation `≤ 1` (sum of a
slope with valuation `≤ 1` and a constant), the second is the regular vanishing
of the `x`-coordinate evaluation (`…translateX_xy_sub_alg_addX…_of_doubling`).
Tangent counterpart of
`pointValuation_slope_add_a₁_mul_translateX_diff_lt_one_of_X_ne`. -/
private theorem pointValuation_slope_add_a₁_mul_translateX_diff_lt_one_of_doubling
    (P : (W_smooth W).SmoothPoint) (xk yk : F) (h_eq_x : P.x = xk) (h_eq_y : P.y = yk)
    (h_not_2_tor : yk ≠ W.toAffine.negY xk yk) :
    (W_smooth W).pointValuation P
      ((translateSlope_xy W xk yk + (W.map (algebraMap F KE)).a₁) *
...
`pointValuation < 1` at `P`. The slope has valuation `≤ 1`; the generator
difference `x_gen − alg P.x` vanishes at `P`
(`pointValuation_x_gen_sub_alg_x_self_lt_one`). Tangent counterpart of
`pointValuation_slope_mul_x_gen_diff_lt_one_of_X_ne`. -/
private theorem pointValuation_slope_mul_x_gen_diff_lt_one_of_doubling
    (P : (W_smooth W).SmoothPoint) (xk yk : F) (h_eq_x : P.x = xk) (h_eq_y : P.y = yk)
    (h_not_2_tor : yk ≠ W.toAffine.negY xk yk) :
    (W_smooth W).pointValuation P
      (translateSlope_xy W xk yk * (x_gen W - algebraMap F KE P.x)) < 1 := by
...
`< 1`; the constant `addX − P.x` lifts from `F`, so its valuation is `≤ 1`.
Tangent counterpart of
`pointValuation_slope_diff_mul_alg_addX_diff_lt_one_of_X_ne`. -/
private theorem pointValuation_slope_diff_mul_alg_addX_diff_lt_one_of_doubling
    (P : (W_smooth W).SmoothPoint) (xk yk : F) (h_eq_x : P.x = xk) (h_eq_y : P.y = yk)
    (h_not_2_tor : yk ≠ W.toAffine.negY xk yk) :
    (W_smooth W).pointValuation P
      ((translateSlope_xy W xk yk - algebraMap F KE (W.toAffine.slope P.x xk P.y yk)) *
      ...
      · calc (W_smooth W).pointValuation P
            ...
            = (W_smooth W).pointValuation P
              ((slope_K + (W.map (algebraMap F KE)).a₁) *
                (addX_K - algebraMap F KE addX_F)) := Valuation.map_neg _ _
          _ < 1 :=
            pointValuation_slope_add_a₁_mul_translateX_diff_lt_one_of_doubling W P xk yk
              h_eq_x h_eq_y h_not_2_tor
      · exact pointValuation_slope_mul_x_gen_diff_lt_one_of_doubling W P xk yk
          h_eq_x h_eq_y h_not_2_tor
    · exact pointValuation_slope_diff_mul_alg_addX_diff_lt_one_of_doubling W P xk yk
        h_eq_x h_eq_y h_not_2_tor
  · exact pointValuation_y_gen_sub_alg_y_self_lt_one W P
