...
theorem ordAtInfty_addPullbackNumerator_reduced_frobenius_eq
  ...
  -- Setup: the integer-level inequalities, named so linarith can use them.
  have h_q : (1 : ℤ) < (Fintype.card K : ℤ) := by exact_mod_cast hq
  have h_int_t1 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ -2 * (Fintype.card K : ℤ) := by linarith
  have h_int_t2 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ (0 : ℤ) := by linarith
  have h_int_t3 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ -3 * (Fintype.card K : ℤ) := by linarith
  have h_int_t5 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ -2 - 3 * (Fintype.card K : ℤ) := by linarith
  have h_int_t6 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ -4 - 2 * (Fintype.card K : ℤ) := by linarith
  have h_int_t7 : (-3 - 3 * (Fintype.card K : ℤ) : ℤ) ≤ -2 - 2 * (Fintype.card K : ℤ) := by linarith
  have h_int_dom : (-2 - 4 * (Fintype.card K : ℤ) : ℤ) <
      -3 - 3 * (Fintype.card K : ℤ) := by linarith
  -- The seven "rest" terms each have ord ≥ -3 - 3q.
  -- Term 1: a₄ · (x_gen + π·x).
  have h_t1 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (algebraMap K KE W.toAffine.a₄ *
          (x_gen W + (frobeniusIsog W).pullback (x_gen W))) := by
    refine ord_algebraMap_mul_ge W W.toAffine.a₄ ?_
    rw [ord_x_gen_add_pi_x_eq W hq]
    exact WithTop.coe_le_coe.mpr h_int_t1
  -- Term 2: 2 · a₆.
  have h_t2 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
    ...
    · rw [ordAtInfty_algebraMap_F_nonzero W ha₆]
      exact WithTop.coe_le_coe.mpr h_int_t2
  -- Term 3: a₃ · (y_gen + π·y).
  have h_t3 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (algebraMap K KE W.toAffine.a₃ *
          (y_gen W + (frobeniusIsog W).pullback (y_gen W))) := by
    refine ord_algebraMap_mul_ge W W.toAffine.a₃ ?_
    rw [ord_y_gen_add_pi_y_eq W hq]
    exact WithTop.coe_le_coe.mpr h_int_t3
  -- Term 4: 2 · y_gen · π·y.
  have h_t4 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
    ...
    exact le_refl _
  -- Term 5: a₁ · (x_gen · π·y + π·x · y_gen).
  have h_t5 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (algebraMap K KE W.toAffine.a₁ *
          (x_gen W * (frobeniusIsog W).pullback (y_gen W) +
           (frobeniusIsog W).pullback (x_gen W) * y_gen W)) := by
    ...
    exact WithTop.coe_le_coe.mpr h_int_t5
  -- Term 6: x_gen² · π·x.
  have h_t6 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (x_gen W ^ 2 * (frobeniusIsog W).pullback (x_gen W)) := by
    rw [ord_x_gen_sq_mul_pi_x_eq W]
    exact WithTop.coe_le_coe.mpr h_int_t6
  -- Term 7: 2 · a₂ · x_gen · π·x.
  have h_t7 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
    ...
    exact WithTop.coe_le_coe.mpr h_int_t7
  -- Chain the 7 bounds into a single bound for the rest-sum.
  have h_rest : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty (
        algebraMap K KE W.toAffine.a₄ *
            (x_gen W + (frobeniusIsog W).pullback (x_gen W))
          + (2 : KE) * algebraMap K KE W.toAffine.a₆
  ...
  -- Algebraic identity: addPullbackNumerator_reduced = dominant + rest.
  have h_eq : addPullbackNumerator_reduced_frobenius W =
      x_gen W * (frobeniusIsog W).pullback (x_gen W) ^ 2 +
      (algebraMap K KE W.toAffine.a₄ *
            (x_gen W + (frobeniusIsog W).pullback (x_gen W))
          + (2 : KE) * algebraMap K KE W.toAffine.a₆
    ...
    ring
  -- ord(T_dom) = -2-4q.
  have h_dom_ord : (W_smooth W).ordAtInfty
      (x_gen W * (frobeniusIsog W).pullback (x_gen W) ^ 2) =
      (((-2 - 4 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) :=
    ord_x_gen_mul_pi_x_sq_eq W
  have h_dom_lt_rest : (W_smooth W).ordAtInfty
      (x_gen W * (frobeniusIsog W).pullback (x_gen W) ^ 2) <
      (W_smooth W).ordAtInfty (
        algebraMap K KE W.toAffine.a₄ *
            (x_gen W + (frobeniusIsog W).pullback (x_gen W))
  ...
  -- Now strict non-arch: ord(T_dom + T_rest) = ord(T_dom).
  exact h_eq.symm ▸ ((W_smooth W).ordAtInfty_add_eq_of_lt h_dom_lt_rest).trans h_dom_ord
...
theorem ordAtInfty_addPullbackNumerator_reduced_negFrobenius_eq
  ...
  have h_t4 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
    ...
    exact le_refl _
  -- Term 5: a₁ · (x_gen · negFrob.pb y + negFrob.pb x · y_gen).
  have h_t5 : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (algebraMap K KE W.toAffine.a₁ *
          (x_gen W * (negFrobeniusIsog W).pullback (y_gen W) +
  ...
  have h_rest : (((-3 - 3 * (Fintype.card K : ℤ)) : ℤ) : WithTop ℤ) ≤
    ...
    have h123456 := ord_add_ge_of_both_ge W h12345 h_t6
    exact ord_add_ge_of_both_ge W h123456 h_t7
  -- Algebraic identity: addPullbackNumerator_reduced = T_dom + T_rest.
...
theorem ordAtInfty_mulByInt_y_eq_of_x
  ...
  obtain ⟨M₂, hM₂⟩ := hM_even
  -- `M = 2·M₂`, so `3M/2 = 3·M₂`; record both `M = M₂ + M₂` and the target.
  have hM_eq : M = M₂ + M₂ := hM₂
  have h_target : (3 * M) / 2 = 3 * M₂ := by omega
  have hX_ord : (W_smooth W).ordAtInfty (mulByInt_x W r) = ((M : ℤ) : WithTop ℤ) := hM
  ...
  have h_eq : mulByInt_y W r ^ 2 +
    ...
    exact h_alg
  -- Compute ord(RHS) = 3M (X³ dominates: 3M < 2M, M, 0 since M < 0).
  ...
  have h_m_le : m ≤ 3 * M₂ := by
    ...
    omega
  -- Step (b): from m ≤ 3M/2 (= 3M₂), Y² strictly dominates LHS, so ord(LHS) = 2m.
  ...
  have h_outer_eq : (W_smooth W).ordAtInfty
    ...
    ((W_smooth W).ordAtInfty_add_eq_of_lt h_a3y_gt').trans h_inner_eq
  rw [h_outer_eq, hY_sq_ord] at h_lhs_ord
  have h_2m : (2 * m : ℤ) = 3 * M := by exact_mod_cast h_lhs_ord
  rw [hm, h_target]
...
theorem addPullbackNumerator_pair_zsmul_frobenius_mulByInt_neg_eq_reduced
  ...
  -- Weierstrass equation for α₁(P) = ((zsmul r π)(x), (zsmul r π)(y)).
  have h_α₁ : (((frobeniusIsog W).zsmul r).pullback (y_gen W)) ^ 2 +
        algebraMap K KE W.toAffine.a₁ *
          ((frobeniusIsog W).zsmul r).pullback (x_gen W) *
          ((frobeniusIsog W).zsmul r).pullback (y_gen W) +
        algebraMap K KE W.toAffine.a₃ *
          ((frobeniusIsog W).zsmul r).pullback (y_gen W) -
        ((((frobeniusIsog W).zsmul r).pullback (x_gen W)) ^ 3 +
         algebraMap K KE W.toAffine.a₂ *
           ((frobeniusIsog W).zsmul r).pullback (x_gen W) ^ 2 +
         algebraMap K KE W.toAffine.a₄ *
           ((frobeniusIsog W).zsmul r).pullback (x_gen W) +
         algebraMap K KE W.toAffine.a₆) = 0 := by
    have h := pullback_equation W ((frobeniusIsog W).zsmul r)
    rw [WeierstrassCurve.Affine.equation_iff'] at h
    have h_a1 : (W_KE W).toAffine.a₁ = algebraMap K KE W.toAffine.a₁ := rfl
    have h_a2 : (W_KE W).toAffine.a₂ = algebraMap K KE W.toAffine.a₂ := rfl
    have h_a3 : (W_KE W).toAffine.a₃ = algebraMap K KE W.toAffine.a₃ := rfl
    ...
    rw [h_a1, h_a2, h_a3, h_a4, h_a6] at h
    exact h
  -- Weierstrass equation for α₂(P) = ((mulByInt -s)(x), (mulByInt -s)(y)).
  have h_α₂ : ((mulByInt W.toAffine (-s)).pullback (y_gen W)) ^ 2 +
        algebraMap K KE W.toAffine.a₁ *
          (mulByInt W.toAffine (-s)).pullback (x_gen W) *
          (mulByInt W.toAffine (-s)).pullback (y_gen W) +
        algebraMap K KE W.toAffine.a₃ *
          (mulByInt W.toAffine (-s)).pullback (y_gen W) -
        (((mulByInt W.toAffine (-s)).pullback (x_gen W)) ^ 3 +
         algebraMap K KE W.toAffine.a₂ *
           (mulByInt W.toAffine (-s)).pullback (x_gen W) ^ 2 +
         algebraMap K KE W.toAffine.a₄ *
           (mulByInt W.toAffine (-s)).pullback (x_gen W) +
         algebraMap K KE W.toAffine.a₆) = 0 := by
    have h := pullback_equation W (mulByInt W.toAffine (-s))
    rw [WeierstrassCurve.Affine.equation_iff'] at h
    have h_a1 : (W_KE W).toAffine.a₁ = algebraMap K KE W.toAffine.a₁ := rfl
    have h_a2 : (W_KE W).toAffine.a₂ = algebraMap K KE W.toAffine.a₂ := rfl
    have h_a3 : (W_KE W).toAffine.a₃ = algebraMap K KE W.toAffine.a₃ := rfl
...
theorem ordAtInfty_addPullbackNumerator_reduced_pair_zsmul_frobenius_mulByInt_neg_eq
  ...
  have h_q : (1 : ℤ) < (Fintype.card K : ℤ) := by
    exact_mod_cast (Fintype.one_lt_card_iff_nontrivial.mpr (inferInstance : Nontrivial K))
  -- Bounds: each non-dominant term has ord ≥ "-3q - 3" (the second-deepest pole).
  have h_int_t1 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ -2 * (Fintype.card K : ℤ) := by linarith
  have h_int_t2 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ (0 : ℤ) := by linarith
  have h_int_t3 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ -3 * (Fintype.card K : ℤ) := by linarith
  have h_int_t5 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ -2 - 3 * (Fintype.card K : ℤ) := by linarith
  have h_int_t6 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ -2 * (Fintype.card K : ℤ) - 4 := by linarith
  have h_int_t7 : (-3 * (Fintype.card K : ℤ) - 3 : ℤ) ≤ -2 * (Fintype.card K : ℤ) - 2 := by linarith
  have h_int_dom : (-4 * (Fintype.card K : ℤ) - 2 : ℤ) <
      -3 * (Fintype.card K : ℤ) - 3 := by linarith
  -- Set abbreviations.
  ...
  -- Helper: a + b in WithTop ℤ for integer-valued ords.
  have h_int_add : ∀ (a b : ℤ),
  ...
  have h_Y_prod : (W_smooth W).ordAtInfty (Y₁ * Y₂) =
    ...
    rw [hY₁_ord, hY₂_ord, h_int_add]
    exact h_int_eq _ _ (by ring)
  -- ord(X₁ · X₂) = -2q + (-2) = -2q - 2.
  ...
  have h_t1 : (((-3 * (Fintype.card K : ℤ) - 3) : ℤ) : WithTop ℤ) ≤
    ...
    refine ord_algebraMap_mul_ge W W.toAffine.a₄ ?_
    rw [h_X_sum]; exact_mod_cast h_int_t1
  -- Term 2: 2 · a₆.  ord ≥ -3q-3.
  ...
  have h_t5 : (((-3 * (Fintype.card K : ℤ) - 3) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty
        (algebraMap K KE W.toAffine.a₁ * (X₁ * Y₂ + X₂ * Y₁)) := by
    refine ord_algebraMap_mul_ge W W.toAffine.a₁ ?_
    refine le_trans ?_ h_cross
    exact_mod_cast h_int_t5
  -- Term 6 (non-dominant): X₁ · X₂². ord = -2q-4 ≥ -3q-3.
  have h_t6 : (((-3 * (Fintype.card K : ℤ) - 3) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty (X₁ * X₂ ^ 2) := by
    rw [h_X1_X2_sq]; exact_mod_cast h_int_t6
  ...
  have h_t7 : (((-3 * (Fintype.card K : ℤ) - 3) : ℤ) : WithTop ℤ) ≤
    ...
    refine ord_two_mul_ge W ?_
    refine ord_algebraMap_mul_ge W W.toAffine.a₂ ?_
    rw [h_X_prod]; exact_mod_cast h_int_t7
  -- Chain the 7 bounds.
  have h_rest : (((-3 * (Fintype.card K : ℤ) - 3) : ℤ) : WithTop ℤ) ≤
      (W_smooth W).ordAtInfty (
        algebraMap K KE W.toAffine.a₄ * (X₁ + X₂)
          + (2 : KE) * algebraMap K KE W.toAffine.a₆
          - algebraMap K KE W.toAffine.a₃ * (Y₁ + Y₂)
          - (2 : KE) * Y₁ * Y₂
          - algebraMap K KE W.toAffine.a₁ * (X₁ * Y₂ + X₂ * Y₁)
          + X₁ * X₂ ^ 2
          + (2 : KE) * algebraMap K KE W.toAffine.a₂ * X₁ * X₂) := by
    have h12 := ord_add_ge_of_both_ge W h_t1 h_t2
    have h123 := ord_sub_ge_of_both_ge W h12 h_t3
    ...
    have h123456 := ord_add_ge_of_both_ge W h12345 h_t6
    exact ord_add_ge_of_both_ge W h123456 h_t7
  -- Algebraic identity: reduced = T_dom + T_rest.
  have h_eq : addPullbackNumerator_reduced_pair_zsmul_frobenius_mulByInt_neg W r s =
      X₁ ^ 2 * X₂ +
      (algebraMap K KE W.toAffine.a₄ * (X₁ + X₂)
        + (2 : KE) * algebraMap K KE W.toAffine.a₆
        - algebraMap K KE W.toAffine.a₃ * (Y₁ + Y₂)
    ...
    ring
  have h_dom_lt_rest : (W_smooth W).ordAtInfty (X₁ ^ 2 * X₂) <
      (W_smooth W).ordAtInfty (
        algebraMap K KE W.toAffine.a₄ * (X₁ + X₂)
          + (2 : KE) * algebraMap K KE W.toAffine.a₆
          - algebraMap K KE W.toAffine.a₃ * (Y₁ + Y₂)
