...
-/
import HasseWeil.Foundation.Curves.Miller
...
/-- **LHS count split (all char)** of the chord-line structural identity: the
count of `M_{Q'}` in the product `span{XClass SR.x} · (M_{SP} · M_{SQ})` factors
as the sum of the three individual counts. Pure `Associates.count_mul`
bookkeeping for the left-hand side of `span_XClass_addSmoothPoint_mul_eq`.
Char-uniform analogue of `count_span_XClass_addSmoothPoint_mul_split`. -/
private theorem count_span_XClass_addSmoothPoint_mul_split_allChar
...
/-- **RHS count split (all char)** of the chord-line structural identity: the
count of `M_{Q'}` in the product `span{YClass linePoly} · M_{SR}` factors as the
sum of the two individual counts. Pure `Associates.count_mul` bookkeeping for the
right-hand side of `span_XClass_addSmoothPoint_mul_eq`. Char-uniform analogue of
`count_span_YClass_maximalIdealAt_addSmoothPoint_split`. -/
private theorem count_span_YClass_maximalIdealAt_addSmoothPoint_split_allChar
    [IsIntegrallyClosed C.CoordinateRing] [DecidableEq F]
    (SP SQ : C.SmoothPoint)
    (hxy : ¬(SP.x = SQ.x ∧ SP.y = C.toAffine.negY SQ.x SQ.y))
    (Q' : C.SmoothPoint) :
...
theorem count_YClass_linePolynomial_eq_allChar
  ...
  have h_count_LHS_split :=
    C.count_span_XClass_addSmoothPoint_mul_split_allChar SP SQ hxy Q'
  have h_count_RHS_split :=
    C.count_span_YClass_maximalIdealAt_addSmoothPoint_split_allChar SP SQ hxy Q'
  have h_struct_count :
      (Associates.mk (C.maximalIdealAt Q')).count
        (Associates.mk (Ideal.span
          ({WeierstrassCurve.Affine.CoordinateRing.XClass C.toAffine
