...
/-- Abstract tower step (stated over general commutative rings to avoid instance-diamond
friction at coordinate-ring types): in a tower `F → R → A`, every polynomial expression in
`x : A` with `F`-coefficients lies in the `R`-subalgebra generated by `x`. -/
private theorem aeval_mem_adjoin_singleton_of_tower {F R A : Type*} [CommRing F] [CommRing R]
    [CommRing A] [Algebra F R] [Algebra F A] [Algebra R A] [IsScalarTower F R A] (x : A)
    (g : Polynomial F) : Polynomial.aeval x g ∈ Algebra.adjoin R {x} := by
  -- the `show … from` pins the base ring: without it `Algebra.subset_adjoin` has to be unified
  -- against `restrictScalars` with a metavariable base ring, which is the expensive `isDefEq`
  have h : Algebra.adjoin F {x} ≤ (Algebra.adjoin R {x}).restrictScalars F :=
    Algebra.adjoin_le_iff.mpr fun w hw ↦
      show w ∈ Algebra.adjoin R {x} from Algebra.subset_adjoin hw
  exact h (Polynomial.aeval_mem_adjoin_singleton F x)

/-- The `{1, Y}` basis spans `F[C₁]` over the `F[C₂]`-subalgebra generated by the coordinate
generator `x₁`: the two `F[X]`-coefficients of an element are polynomials in `x₁` with
`F`-coefficients, hence lie in that subalgebra by
`aeval_mem_adjoin_singleton_of_tower`. -/
private theorem top_le_adjoin_mul_span_basis {C₁ C₂ : SmoothPlaneCurve F}
    [Algebra C₂.CoordinateRing C₁.CoordinateRing]
    [IsScalarTower F C₂.CoordinateRing C₁.CoordinateRing] :
    (⊤ : Submodule C₂.CoordinateRing C₁.CoordinateRing) ≤
      Subalgebra.toSubmodule (Algebra.adjoin C₂.CoordinateRing
          {algebraMap (Polynomial F) C₁.CoordinateRing Polynomial.X}) *
        Submodule.span C₂.CoordinateRing
          ({1, CoordinateRing.mk C₁.toAffine Y} : Set C₁.CoordinateRing) := by
  have hadj : ∀ g : Polynomial F, algebraMap (Polynomial F) C₁.CoordinateRing g ∈
      Algebra.adjoin C₂.CoordinateRing
        {algebraMap (Polynomial F) C₁.CoordinateRing Polynomial.X} := fun g ↦ by
    rw [← aeval_algebraMap_X C₁ g]
    exact aeval_mem_adjoin_singleton_of_tower _ g
  rintro z -
  ...
  exact Submodule.add_mem _
    (Submodule.mul_mem_mul (hadj p) (Submodule.subset_span (Set.mem_insert _ _)))
    (Submodule.mul_mem_mul (hadj q) (Submodule.subset_span (Set.mem_insert_of_mem _ rfl)))
...
theorem algHom_coordinateRing_module_finite
  ...
  exact module_finite_of_adjoin_singleton_mul_span
    (algHom_coordinateRing_isIntegralElem_X ψ hψ)
    (Set.toFinite ({1, CoordinateRing.mk C₁.toAffine Y} : Set C₁.CoordinateRing))
    top_le_adjoin_mul_span_basis
