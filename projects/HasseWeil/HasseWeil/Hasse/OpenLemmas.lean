...
/-! ## HasseOpenLemmaPack — explicit dependency record

To make the final theorem's dependency on all open lemmas explicit at the
Lean level (not just project-plan-level), bundle them into a record. The
final theorem then explicitly consumes the pack, so removing any lemma's
`sorry` triggers a real Lean type-checking obligation.

Per reviewer Q5: `#print axioms` only reports `sorryAx` and does not
certify which open lemmas are actually inputs. The `HasseOpenLemmaPack` makes
this dependency provable. -/

/-- **Bundled record of the open lemmas (reviewer round 2 revised)**.

Reflects the reviewer round 2 (2026-05-15) restructuring:

* L11 (universal `∃!` dual) was REMOVED and replaced by the Miller
  decomposition primitives `l11a..l11f` plus the project's existing
  specific dual existence for Frobenius (L9 = `l9_verschiebung_dual`).
* L12 (Pic⁰ over K) was DROPPED per reviewer recommendation; `l12b`
  alone suffices for the bound discharge.
* NEW `l14_addHom_descent` (`AddHomProperty` descent via reflection) and
  `l15_adjoinRoot_tensor` (`AdjoinRoot` scalar extension) added.

Each field corresponds to one of the open lemma statements above. The
`Sinf`-data-parametric lemmas (L2, L3, L4, L5, L7) bind the call-site data
universally inside the field type itself (parameter `data : Sinf …` or
`Fact (Transcendental …)`), so the codomain is the genuine mathematical
claim — no `True` filler. The carrier universe is taken as the structure
...
structure HasseOpenLemmaPack.{v}
  ...
  /-- L1 (replaced): direct ω-additivity for `addIsog`-built genuine
  addition isogenies. -/
  l1_omega_add : ∀ (α β : Isogeny W.toAffine W.toAffine)
    (hxy : AddNonInversePair (W := W) α β)
    (hinj : Function.Injective (addCoordAlgHomPair (W := W) hxy)),
    omegaPullbackCoeff W (addIsog (W := W) hxy hinj) =
      omegaPullbackCoeff W α + omegaPullbackCoeff W β
  /-- L2 (Bridge B(i)): for every Sinf data on `γ.pullback x_gen`, there exists
  a function from kernel points to prime ideals of the carrier lying over
  `xIdeal`. The `data : Sinf …` parameter is universally-quantified inside
  the field type, since the binder shape is genuinely call-site-data
  parameterised. -/
  l2_bridge_Bi_exists :
    ∀ (data : Sinf.{_, v, _} (k := K)
        (((isogOneSub_negFrobenius W hq).pullback (x_gen W)) :
    ...
    ∃ φ : (isogOneSub_negFrobenius W hq).kernel → Ideal data.carrier,
      ∀ T : (isogOneSub_negFrobenius W hq).kernel,
        (φ T).IsPrime ∧ (φ T).LiesOver (xIdeal (k := K))
  /-- L3 (Bridge B(ii)): for every Sinf data, there exists a bijection
  between the kernel of `γ` and the set of primes of the carrier lying over
  `xIdeal`. Phrased as `Nonempty` of an `Equiv` so the substantive content
  is the typed codomain (no `True` filler). -/
  l3_bridge_Bii [Fintype W.toAffine.Point] :
    ∀ (data : Sinf.{_, v, _} (k := K)
        (((isogOneSub_negFrobenius W hq).pullback (x_gen W)) :
          W.toAffine.FunctionField)),
    letI := data.commRing
    letI := data.algPoly
    Nonempty ((isogOneSub_negFrobenius W hq).kernel ≃
      {P : Ideal data.carrier // P.IsPrime ∧ P.LiesOver (xIdeal (k := K))})
  /-- L4 (Bridge B(iii)): for every Sinf data and every kernel-prime
  function (whose values are primes lying over `xIdeal`), the order at every
  kernel-prime is `-2`. -/
  l4_bridge_Biii :
    ∀ (data : Sinf.{_, v, _} (k := K)
        (((isogOneSub_negFrobenius W hq).pullback (x_gen W)) :
    ...
    ∀ T : (isogOneSub_negFrobenius W hq).kernel,
      data.ordAt (φ T) = (-2 : ℤ)
  /-- L5 (Bridge B(iv)): for every Sinf data and every kernel-prime
  function (whose values are primes lying over `xIdeal`), the inertia
  degree of every kernel-prime over `xIdeal` is `1`. -/
  l5_bridge_Biv :
    ∀ (data : Sinf.{_, v, _} (k := K)
        (((isogOneSub_negFrobenius W hq).pullback (x_gen W)) :
          W.toAffine.FunctionField)),
  ...
  /-- L6 (V.1.3): `sepDegree(1 − π) = #E(F_q)`. -/
  l6_v_1_3 [Fintype W.toAffine.Point] :
    (isogOneSub_negFrobenius W hq).sepDegree = pointCount W.toAffine
  /-- L7 (Sinf typeclass discharge): for every transcendentality `Fact`,
  the function field `K(E)` is separable as an algebra over `FractionRing K[X]`
  under the `LinfAt` construction at `γ.pullback x_gen`. The
  `Fact (Transcendental …)` parameter is universally-quantified inside the
  field type. -/
  l7_sinf_typeclass :
    ∀ (_ : Fact (Transcendental K
      ((isogOneSub_negFrobenius W hq).pullback (x_gen W))⁻¹)),
    Nonempty (Algebra.IsSeparable (FractionRing (Polynomial K))
      (LinfAt (k := K)
        ((isogOneSub_negFrobenius W hq).pullback (x_gen W))))
  /-- L7a (sublemma for L7): `K(E)/K(x)` separability for smooth
  Weierstrass. -/
  l7a_function_field_x_separable :
    Algebra.IsSeparable (FractionRing (Polynomial K))
      W.toAffine.FunctionField
  /-- L8 (degree-QF non-negativity): the Silverman III.6.3 quadratic-form
  positive semidefiniteness (nonnegativity), `0 ≤ q·r² − tr·rs + s²` for all `r, s : ℤ`. This is
  the genuinely-open content (the QF non-negativity that yields `tr² ≤ 4q`);
  it is stated intrinsically on the integer quadratic form so it names no
  isogeny family. (Previously split into `l8_deg_qf` + `l8z_qf_zero_isogeny`
  via the `isogSmulSub` placeholder family; collapsed to this single
  placeholder-free field on 2026-05-28.) -/
  l8_qf_nonneg : ∀ r s : ℤ,
    0 ≤ (Fintype.card K : ℤ) * r ^ 2 -
        isogTrace (frobeniusIsog W) (isogOneSub_negFrobenius W hq) *
          r * s + s ^ 2
  /-- L9 (Verschiebung is dual of Frobenius): `∃ V, IsDualOf V π`. -/
  l9_verschiebung_dual : ∃ V : Isogeny W.toAffine W.toAffine,
    IsDualOf W.toAffine V (frobeniusIsog W)
  /-- L10 (trace formula): `π + V = [tr]` at the *isogeny level* (via
  `addIsog` with non-inverse + injectivity witnesses supplied at the call
  site). The AddMonoidHom-form follows by `addIsog_toAddMonoidHom`. -/
  l10_trace_eq : ∀ V : Isogeny W.toAffine W.toAffine,
    IsDualOf W.toAffine V (frobeniusIsog W) →
    ∀ (hxy : AddNonInversePair (W := W) (frobeniusIsog W) V)
      (hinj : Function.Injective (addCoordAlgHomPair (W := W) hxy)),
    addIsog (W := W) hxy hinj =
      mulByInt W.toAffine
        (isogTrace (frobeniusIsog W)
          (isogOneSub_negFrobenius W hq))
  -- NOTE: L11 (universal `∃!` dual) was REMOVED per reviewer round 2;
  -- the Miller primitives below transitively support L9.
  /-- L11a (Miller): vertical line principal divisor `(P)+(-P)-2(O)`. -/
  l11a_vertical_principal : ∀ P : W.toAffine.Point,
    HasseWeil.Curves.SmoothPlaneCurve.ProjIsPrincipal
      (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K)
      (Finsupp.single P.toProjectiveSmoothPoint (1 : ℤ)
        + Finsupp.single (-P).toProjectiveSmoothPoint 1
        - (2 : ℤ) • Finsupp.single
            (HasseWeil.Curves.ProjectiveSmoothPoint.infinity :
              HasseWeil.Curves.ProjectiveSmoothPoint
                (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K)) 1)
  /-- L11b (Miller): chord/tangent line principal divisor
  `(P)+(Q)+(-(P+Q))-3(O)`. -/
  l11b_line_principal : ∀ P Q : W.toAffine.Point,
    HasseWeil.Curves.SmoothPlaneCurve.ProjIsPrincipal
      (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K)
      (Finsupp.single P.toProjectiveSmoothPoint (1 : ℤ)
        + Finsupp.single Q.toProjectiveSmoothPoint 1
        ...
        - (3 : ℤ) • Finsupp.single
            (HasseWeil.Curves.ProjectiveSmoothPoint.infinity :
              HasseWeil.Curves.ProjectiveSmoothPoint
                (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K)) 1)
  /-- L11c (Miller): Miller relation = `MillerHypothesis W.toAffine`. -/
  l11c_miller_principal : HasseWeil.Curves.MillerHypothesis W.toAffine
  /-- L11d (Miller): degree-zero divisor reduction = `DivZeroReduce W.toAffine`. -/
  l11d_divisor_reduce : HasseWeil.Curves.DivZeroReduce W.toAffine
  /-- L11e (Miller): special uniqueness `(P) - (O)` principal ⟹ P = 0
  = `PointMinusOPrincipalEqZero W.toAffine`. -/
  l11e_point_minus_O : HasseWeil.Curves.PointMinusOPrincipalEqZero W.toAffine
  /-- L11f (Miller, sublemma): coordinate-ring image has no order -1 at
  infinity. -/
  l11f_ord_neg_one : ∀ u : (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K).CoordinateRing,
    u ≠ 0 →
    (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K).ordAtInfty
        (algebraMap _
          (⟨W.toAffine⟩ : HasseWeil.Curves.SmoothPlaneCurve K).FunctionField u) ≠
      ((-1 : ℤ) : WithTop ℤ)
  -- NOTE: L12 (Pic⁰ over K) was DROPPED per reviewer round 2; L12b
  -- alone suffices for the bound.
  /-- L12b: base-change degree descent for isogenies. -/
  l12b_basechange_degree : ∀ α : Isogeny W.toAffine W.toAffine,
    ∃ d : ℤ, d = (α.degree : ℤ)
  /-- L13 (iterated Frobenius for `q = p^r`). -/
  l13_frob_iterate (p r : ℕ) [Fact p.Prime] [CharP K p]
    [Fact (Fintype.card K = p ^ r)] :
    ∀ f : W.toAffine.FunctionField,
      (frobeniusIsog W).pullback f = ((· ^ p)^[r]) f
  /-- L14 (NEW, REVIEWER-CORRECTED v2): `AddHomProperty` descent via
  reflection. Pointwise additivity descends along an injective base-change
  point inclusion bundled as an `AddMonoidHom` (per reviewer round 3:
  drops separate `add_hom₁`/`add_hom₂` predicates by bundling additivity
  into the inclusion's hom-type). -/
  l14_addHom_descent :
    ∀ {F : Type*} [Field F] [DecidableEq F]
      {W₁ W₂ : WeierstrassCurve.Affine F}
      [W₁.IsElliptic] [W₂.IsElliptic]
      ...
      (includeP₁ : W₁.Point →+ W₁'.Point)
      (includeP₂ : W₂.Point →+ W₂'.Point),
      Function.Injective includeP₂ →
      (∀ P : W₁.Point,
        includeP₂ (φ.toPointMap cd P) = φL.toPointMap cdL (includeP₁ P)) →
      φL.AddHomProperty cdL → φ.AddHomProperty cd
  /-- L15 (NEW, REVIEWER-CORRECTED v2): `AdjoinRoot` scalar-extension
  equivalence (Phase G flatness chain). Upgraded to the genuine `AlgEquiv`
  form per reviewer round 3 (2026-05-15). The `B`-algebra structure on the
  tensor product is `Algebra.TensorProduct.rightAlgebra` (deliberately not
  a global instance in mathlib to avoid diamonds; provided here via a
  `letI`). -/
  l15_adjoinRoot_tensor :
    ∀ {A : Type*} [CommRing A] {B : Type*} [CommRing B] [Algebra A B]
      {p : Polynomial A},
      letI : Algebra B (TensorProduct A
          (Polynomial A ⧸ (Ideal.span ({p} : Set (Polynomial A)))) B) :=
        Algebra.TensorProduct.rightAlgebra
      Nonempty
        (TensorProduct A
            (Polynomial A ⧸ (Ideal.span ({p} : Set (Polynomial A)))) B ≃ₐ[B]
          (Polynomial B ⧸
            (Ideal.span
              ({Polynomial.map (algebraMap A B) p} : Set (Polynomial B)))))
