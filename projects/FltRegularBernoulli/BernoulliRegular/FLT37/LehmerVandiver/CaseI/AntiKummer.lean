...
/-- **Abstract tower step**: in a tower `R → S → A`, the `S`-subalgebra generated by `s`, viewed
as an `R`-subalgebra, is contained in any `R`-subalgebra `T` that contains both `s` and the whole
image of `S` in `A`. -/
private theorem adjoin_restrictScalars_le_of_range_le {R S A : Type*} [CommSemiring R]
    [CommSemiring S] [Semiring A] [Algebra R S] [Algebra S A] [Algebra R A]
    [IsScalarTower R S A] {s : Set A} {T : Subalgebra R A}
    (hS : ∀ x : S, algebraMap S A x ∈ T) (hs : s ⊆ (T : Set A)) :
    (Algebra.adjoin S s).restrictScalars R ≤ T := by
  intro x hx
  -- restate the membership at `Algebra.adjoin S s` itself, so that the induction motive is
  -- inferred without having to unfold `restrictScalars` against a metavariable base ring
  replace hx : x ∈ Algebra.adjoin S s := hx
  refine Algebra.adjoin_induction (fun _ hr ↦ hs hr) hS
    (fun _ _ _ _ ha hb ↦ T.add_mem ha hb) (fun _ _ _ _ ha hb ↦ T.mul_mem ha hb) hx
...
theorem antiKummerKplusPoly_adjoin_rootSet_eq_top_of_K_gen
    ...
    Algebra.adjoin (NumberField.maximalRealSubfield K)
        ((antiKummerKplusPoly (p := p) K α₀ hα₀ h_anti).rootSet
          (antiKummerLift (p := p) K α₀ hα₀) : Set _) = ⊤ := by
  rw [eq_top_iff, ← Subalgebra.restrictScalars_top (R := NumberField.maximalRealSubfield K)
        (S := K) (A := antiKummerLift (p := p) K α₀ hα₀),
    ← Polynomial.IsSplittingField.adjoin_rootSet (antiKummerLift (p := p) K α₀ hα₀)
        (Polynomial.X ^ p - Polynomial.C α₀)]
  exact adjoin_restrictScalars_le_of_range_le
    (antiKummerLift_K_image_in_adjoin_of_K_gen (p := p) K α₀ hα₀ h_anti h_K_gen)
    fun _ hr ↦ Algebra.subset_adjoin
      (antiKummerLift_X_pow_sub_C_rootSet_subset (p := p) K α₀ hα₀ h_anti hr)
