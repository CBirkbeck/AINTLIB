...
theorem antiKummerSigmaTildeInvolutive_pow_mul_sigma_pow_eq_one
  ...
  have h_y_eq : g_aut (algebraMap K (antiKummerLift (p := p) K α₀ hα₀) α₀) =
      algebraMap K (antiKummerLift (p := p) K α₀ hα₀) (h α₀) :=
    (AlgEquiv.restrictNormal_commutes g_aut K α₀).symm
  rw [h_y_eq, antiKummerSigmaTildeInvolutive_restricts_K (p := p) K α₀ hα₀ h_anti h_irr h_irr_g
    h_alpha_sq_ne, ← map_mul, alg_equiv_K_alpha_mul_complexConj_eq_one α₀ hα₀ h_anti h, map_one]
...
theorem antiKummerSigmaTildeInvolutive_central
  ...
  rw [antiKummerSigmaTildeInvolutive_apply_root (p := p) K α₀ hα₀ h_anti h_irr h_irr_g,
    ...
    (p := p) hp_odd α₀ hα₀ h_anti h_irr h_irr_g h_alpha_sq_ne g_aut, map_inv₀]

/-- **⟨σ̃⟩ is normal in Gal(L/K⁺)**. -/
...
theorem mkAntiKummerRealSubfieldH94Inputs
    (hp_odd : p ≠ 2)
    (α₀ : K) (hα₀ : α₀ ≠ 0)
    (h_anti : NumberField.IsCMField.complexConj K α₀ = α₀⁻¹)
    (h_irr : Irreducible (Polynomial.X ^ p - Polynomial.C α₀ : Polynomial K))
...
/-- **IsUnramified L⁺/K⁺ from a per-prime ramification-index bound**: if every
finite prime `𝔓` of `𝓞 L⁺` over `𝔭` of `𝓞 K⁺` has ramification index `1`, then
`L⁺/K⁺` is unramified. -/
...
`AntiKummerRealSubfieldH94Inputs` for each case-I datum, together with
`¬ p ∣ hPlus K`, yields `False` on any such datum. -/
