...
-- `groupCohomology.exists_div_of_norm_eq_one` is stated at universe 0, so `K`
-- is restricted to `Type` here (no loss for concrete fields like `CyclotomicField 37 ℚ`).
variable (p : ℕ) [hp : Fact p.Prime]
  (K : Type) [Field K] [NumberField K] [IsCyclotomicExtension {p} ℚ K]
...
theorem algebraMap_intNorm_eq_self_mul_complexConj [IsCMField K] (a : 𝓞 K) :
  ...
  rw [Algebra.algebraMap_intNorm (K := K⁺) (L := K),
    algebraMap_norm_eq_self_mul_complexConj (K := K)]
  push_cast
  rfl
...
/-- **K⁺-relative integer norm of `ζ^m` is `1`** for any `m`. -/
...
theorem map_relNorm_span_singleton_eq_mul_complexConj [IsCMField K] (a : 𝓞 K) :
  ...
  rw [Ideal.relNorm_singleton, Ideal.map_span, Set.image_singleton,
    algebraMap_intNorm_eq_self_mul_complexConj (K := K),
...
/-- **Norm formula on `𝔞` from the equation `(α) = 𝔞^p`:**
`(relNorm 𝔞).map = 𝔞 · σ𝔞`. -/
...
*without* requiring primarity of `α`. -/
  ...
  have h_order_p : ClassGroup.mk0
    ...
    rw [show ClassGroup.mk0
        ...
        ClassGroup.mk0 (⟨Ideal.relNorm (𝓞 K⁺) 𝔞 ^ p, hpow_mem⟩
          : nonZeroDivisors (Ideal (𝓞 K⁺))) by rw [← map_pow]; rfl]
