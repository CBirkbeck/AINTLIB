...
separable-residue-minpoly criterion `isUnramifiedAt_of_Separable_minpoly` (flt-regular's entry point,
the one `KummersLemma.isUnramified` uses).
...
The clean self-contained engine is `isUnramifiedAt_dvr_of_pow_eq_unit`: over an abstract DVR `A` with
`n` a unit, a root `θ` of `X^n - u` (`u : Aˣ`) generates an extension unramified at the maximal ideal
of `A`.
...
Let `A` be a Dedekind domain with fraction field `K`, `L/K` finite separable, `S` the integral closure
of `A` in `L`.  Let `P` be a nonzero prime of `S` lying over the (nonzero) prime `P.under A`, with `n`
a unit in `A ⧸ (P.under A)`.  If `θ : L` generates `L` over `K` (`Algebra.adjoin K {θ} = ⊤`) and
`θ^n = u` for a unit `u : Aˣ`, then `P` is unramified over `A` (mathlib's
...
* `37 ∉ q` (else `37 ∈ P`), so `37` is a unit in `A` and in `κ(q) = A ⧸ q·A`; in particular
  `((37 : A) : A ⧸ m_A) ≠ 0`.
* From `(α) = 𝔟^{37}` we get `v_q(α) = 37·v_q(𝔟) = 37k` for some `k ∈ ℤ`; hence with a `q`-uniformizer
  `ϖ`, `u := α·ϖ^{-37k}` is a `q`-unit (`A`-unit) and the **local generator** `θ' := θ·ϖ^{-k}`
  (where `θ` is the canonical root, `θ^{37} = α`) satisfies `(θ')^{37} = u`.  Since `θ'` differs from
  `θ` by a nonzero `K`-scalar, `Algebra.adjoin K {θ'} = ⊤`.
...
theorem antiKummerLiftRoot_adjoin_K_eq_top
  ...
  set θ := antiKummerLiftRoot (p := p) K α hα with hθ
  ...
  have hr_pow : r ^ p = algebraMap K L α := by
    ...
    simpa using this
  -- `(r·θ⁻¹)^p = 1`.
  have hθ_unit : IsUnit θ := isUnit_iff_ne_zero.mpr hθ_ne
...
theorem caseIIKummerUnramifiedAway37_proven : CaseIIKummerUnramifiedAway37 := by
  ...
  have hγL_ne : (algebraMap (CyclotomicField 37 ℚ) L γ) ^ 37 ≠ 0 :=
    pow_ne_zero 37 ((map_ne_zero_iff _ (FaithfulSMul.algebraMap_injective _ _)).mpr hγ_ne)
  ...
  have hgen : Algebra.adjoin (CyclotomicField 37 ℚ)
    ...
    have hθ_mem : antiKummerLiftRoot (p := 37) (CyclotomicField 37 ℚ) α hα ∈
      ...
      have hθ_eq : antiKummerLiftRoot (p := 37) (CyclotomicField 37 ℚ) α hα =
        ...
        rw [mul_assoc, inv_mul_cancel₀ ((map_ne_zero_iff _
          (FaithfulSMul.algebraMap_injective _ _)).mpr hγ_ne), mul_one]
  ...
  set Pₚ : Ideal Sₚ := P.map (algebraMap (𝓞 L) Sₚ) with hPₚ
