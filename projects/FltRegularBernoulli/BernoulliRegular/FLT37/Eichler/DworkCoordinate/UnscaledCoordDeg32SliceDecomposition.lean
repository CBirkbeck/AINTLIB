...
This file records the **sound** mod-`37²` value of the level-`71` unscaled Dwork-parameter
coordinate `valuedLambdaQuotientDworkCoeffModSq 32 (dworkParameterNormalizedCoordFiniteLogN 71)` —
*piece 2* of `CaseIICor823Level71DworkSpecializedFiniteLog37`
(`CaseIICor823Level71DworkSpecializedReduction.lean`).
...
`valuedLambdaQuotientDworkCoeffModSq_dworkParameterNormalizedCoordFiniteLogN71_eq_sum`, the **sum
over all homogeneous degrees** `d ∈ range (samePrimeFiniteLogCutoff 71) = range 2664` of the
degree-`d` slice's `varpi^{32}` coordinate.  Through the ramification fold `varpi^{36} =
-37·tailUnit` (`dworkParameter_pow_pred_eq_neg_p_mul_tailUnit`), a degree-`d` slice lands on the
`varpi^{32}` coordinate iff `d ≡ 32 (mod 36)`; within mod-`37²` precision **only** `d ∈ {32, 68}`
survive (`d ∈ {104, 140, …}` carry `(varpi^{36})^{≥ 2} = 37^{≥ 2}·(…) = 0 (mod 37²)`).  Hence

  `coordModSq 32 (unscaled) = (deg-`32` slice) + (deg-`68` slice) + (slices that vanish mod 37²)`.

The **deg-`32`** slice is exactly `37·(32!)⁻¹` (`deg32SliceCoordModSq37_eq`, proven).  The
**deg-`68`** slice is `37·c₆₈` with `c₆₈ ∈ ZMod 37` the bare degree-`68` second digit — the Kellner
`α₁` content (`B₆₈/68 ≡ 22 (mod 37)`; cf. `CaseIICor823Level72ColumnScaling` docstring "two genuine
second-order obstructions", and `CaseIICor823Level71NormalizedUnitValue` "degree-`68` … carries the
`α₁`-datum"). Its mod-`37` value is the **proven** `0` (`deg68_slice_varpi32_coordMod37_eq_zero`),
but its second digit `c₆₈` is *not* pinned by the level-`71`/mod-`37²` factorial extraction: that
extraction gives `(68!)·(deg-`68` slice) = (formalSum₆₈ residue)·(-37)`
(`factorial37_deg68_slice_value`), and since `formalSum₆₈ residue ≡ 0 (mod 37)` the right side is
`(37·r₆₈)·(-37) = -37²·r₆₈ = 0 (mod 37²)`, so the relation degenerates to `37·(deg-`68` slice) = 0`,
which pins only `castHom(deg-`68` slice) = 0` and leaves `c₆₈` **free**.  The `-37` ramification
fold annihilates the second digit `r₆₈` of the source.

So the **true** value of piece 2 is `37·((32!)⁻¹ + c₆₈)`, *not* `37·(32!)⁻¹`: piece 2 omits the
degree-`68` second-digit term.  Recovering `c₆₈` from the proven `α₁` requires the unscaled
coordinate to one more `37`-adic digit (mod `37³`, level `≥ 73`) — exactly the precision the proven
low-degree extraction
`valuedLambdaQuotientDworkCoeffModP_factorPow_normalizedHomogeneousDegreeSum_…_of_lt` cannot supply
(it needs `d < p - 1 = 36` so `d!` is a `37`-unit; `d = 68` has `37 ∣ 68!`).
...
degree-`68` slice `37·c₆₈` as a summand (`deg68SliceCoordModSq37_mem_correction`), whose mod-`37`
value is the proven `0` but whose second digit `c₆₈` is the precise irreducible Kellner-`α₁`
residual.  This makes the over-statement **machine-checkable**: piece 2 holds *iff* the proven
correction sum is `0`.

## The precise irreducible residual

`CaseIICor823Level71Unscaled32Coord37` (`def … : Prop`, **not** an axiom): the *sound* form of piece
2 — a nonzero `ρ₀ : ZMod 37` with `coordModSq 32 (unscaled) = 37·(ρ₀.val)` and `ρ₀ = (32!)⁻¹ + c₆₈`.
...
A convenience name for the `varpi^{32}` mod-`37²` coordinate of the degree-`d` homogeneous slice of
the level-`71` unscaled Dwork-parameter normalized finite log, the summand of the proven slice-sum
...
`factorial32_deg32_slice_value_eq_thirtyseven` for the `37`-unit `32!`).  This is the **only** slice
of
...
(`deg68_slice_varpi32_coordMod37_eq_zero`): its first `37`-digit vanishes, so it is `37·c₆₈`.  But
its second digit `c₆₈` — the Kellner `α₁` content — is *not* determined by the level-`71`/mod-`37²`
...
`unscaled32SliceCoord` name (for `K = CyclotomicField 37 ℚ`, the field of the proven deg-`68`
lemma): `castHom (unscaled32SliceCoord 68) = 0`.  Re-export of the proven
`deg68_slice_varpi32_coordMod37_eq_zero`.  So `unscaled32SliceCoord 68 = 37·c₆₈` for a (genuine,
generally nonzero) second digit `c₆₈ ∈ ZMod 37` — the Kellner `α₁` (`B₆₈/68 ≡ 22`) content — which
the
...
(`unscaled32SliceCoord_sixtyeight_castHom_eq_zero`: its mod-`37` value is `0`, so it is `37·c₆₈`),
this
...
general).  The `37·` shape is correct (every contributing slice `d ∈ {32, 68}` has mod-`37`
coordinate `0`); only the second digit `ρ₀` and `ρ₀ ≠ 0` (i.e. `(32!)⁻¹ + c₆₈ ≠ 0`) are the genuine
unknown — the
...
/-- **The sound residual is non-vacuous** (proven): for a *nonzero* witness scalar `ρ₀` both sides
are genuine elements of `ZMod 37²`, and the right side `37·(ρ₀.val)` is itself nonzero whenever `ρ₀
≠ 0`
