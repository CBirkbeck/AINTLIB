...
# the genuine (non-pinned) leading coefficient, and the single irreducible coordinate-bridge piece

This file assembles the level-`71` normalized-unit Dwork coordinate
`W(a) := normalizedUnitCoeff37 a` out of the proven degree-`32` and degree-`68` slice values, the
proven `37·(second-order part)` structure, and the proven cyclotomic column factor, isolating the
**single** genuine remaining `p`-adic-`L` piece — the level-`71` lift of the first-order unit↔Dwork
coordinate bridge — and proving it **discharges** `CaseIICor823Level71SecondOrderPartValue37` (hence
R4 and the FLT37 endpoint).  It imports only; it does **not** modify any existing file.  No `sorry`,
no `axiom`.
...
(`CaseIICor823Level71SecondOrderBridge.lean`) **pins** the second-order leading coefficient `ρ` to
the bare Kellner `α₀`-datum `kellnerLeadingCoeff37 = β₃₂ = B₃₂.num/37 ≡ 3 (mod 37)`.  That pinning
is *unsound*: the genuine Dwork-coordinate leading coefficient is **not** the bare Bernoulli
factor.  The proven degree-`32` slice value (`factorial32_deg32_slice_value_eq_thirtyseven`) is

  `(32! : ZMod 37²)·coordModSq(deg-`32` slice) = 37`,   so
  `coordModSq(deg-`32` slice) = 37·(32!)⁻¹`,

whose mod-`37` **second digit** is `(32!)⁻¹ ≡ 13 (mod 37)`, *not* `3`.  Through the unit↔Dwork
bridge the second-order part is `−(32!)⁻¹·V̄(a)`, i.e. `ρ = −(32!)⁻¹ ≡ 24 (mod 37)` — the value
already *conceded* in the docstring of `CaseIICor823Level71SecondOrderPartValue37` ("the *exact*
...
(`CaseIICor823Level71SecondOrderPartValue37`, the `M ≤ 1` non-degeneracy) needs **only** `ρ ≠ 0`,
*not* a specific numeral, and both `24` and `3` are nonzero in `ZMod 37`.  So this file targets the
**sound** existential residual `CaseIICor823Level71SecondOrderPartValue37` with the **genuine**
nonzero `ρ = deg32SliceSecondDigit37 = −(32!)⁻¹`, rather than the over-pinned (false) `ρ = 3`.
...
`c^{p-1} − 1` (where `c = kummerLogValuedCyclotomicQuotientDenUnit a`) equals the cyclotomic column
factor `(1 − k^{32})` times the level-`71` degree-`32` Dwork-parameter slice coordinate, mod
...
Fermat quotient (`column_pow_pred_modSq_eq`, `k^{37} = k + 37·F(k)`), the first-order Fermat
congruence `c^p ≡ c (mod p)`
(`kummerLogDenUnitPowPredFiniteLog_eq_normalizedQuotientFiniteLog_modP`) being valid only at
precision `p − 1`.  Both the level-`36` unit↔Dwork bridge and the level-`36` unit↔quotient Fermat
lift are *precision-generic mechanisms* whose level-`71` instantiation is exactly this piece; the
`adicCompletionIntegers` whnf wall blocks an automatic level-`71` re-instantiation.
...
(hence `ρ`) and the column factor are **pinned to the proven objects**, and the *only* unknown is
the single per-column coordinate identification `W(a) = (1 − k^{32})·(deg-`32` slice coord)`.  It is
...
(`(32!)·coordModSq = 37`) for the `37`-unit `32!` (`factorial_thirtytwo_isUnit_modSq`).  This pins
the degree-`32` slice's exact second-order value: a *nonzero* `37·(unit)`, whose mod-`37` second
digit is `(32!)⁻¹` — the genuine Dwork-coordinate `α₀`-content (`≢ β₃₂` in general). -/
...
`ZMod 37` this is `−13 ≡ 24`.  This is the **genuine** Dwork-coordinate `α₀`-content — *not* the
bare Bernoulli factor `kellnerLeadingCoeff37 = β₃₂ = 3`, which differs from it by the unit twist
...
since `32!` is a `37`-unit (`37 ∤ 32!`, as `32 < 37`).  This is the genuine `M ≤ 1` non-degeneracy
of the surviving degree-`32` slice, the *sound* witness for the existential residual. -/
...
explicit value of the deg-`32` Dwork-slice second digit negated by the column sign.  Records that
the genuine `ρ` is `24`, **not** the pinned `kellnerLeadingCoeff37 = 3`. -/
...
/-- **The genuine leading coefficient is not the pinned Kellner datum** (proven):
`−(32!)⁻¹ ≠ kellnerLeadingCoeff37 = 3`.  This certifies that
`CaseIICor823Level71UnscaledDworkCoeff37` (which pins `ρ = 3`) over-commits to a *false* numeral,
and that the sound target is the existential `CaseIICor823Level71SecondOrderPartValue37` reached
with the genuine `ρ = 24`. -/
...
forward cancellation `castHom_eq_of_thirtyseven_mul_eq` yields
`castHom((s.val : ZMod 37²)) = castHom X`, i.e. `s = castHom X` (as `castHom ∘ natCast ∘ val = id`
on `ZMod 37`). -/
...
/-! ## 4. The level-`71` unit ↔ Dwork-slice coordinate bridge residual (the single irreducible
piece)
...
factor `(1 − k^{32})` times the level-`71` degree-`32` Dwork-parameter slice coordinate.  This is
the level-`71` (mod `37²`) lift of the proven first-order (mod `37`) unit↔Dwork bridge
`valuedLambdaQuotientDworkCoeffModP_specializedFiniteLog_eq_one_sub_pow_mul_unscaled`, for the
**unit** argument, through the second-order Fermat quotient (§2 of
`CaseIICor823Level71SecondOrderBridge.lean`, `column_pow_pred_modSq_eq`). -/
...
deg-`68`-vanishing-mod-`37`) second-order part to the Teichmüller-Vandermonde.  It is stated at mod
`37` (the order the FLT37 endpoint consumes), so it does **not** over-assert any second-order
...
slice second digit), **non-circular** (its conclusion is the explicit `ρ·V̄` value), and
**non-vacuous** (`caseIICor823Level71UnitDworkCoordBridge37_consequent_inhabited`).  The only
undischarged content is the level-`71` lift of the proven first-order unit↔Dwork bridge
`valuedLambdaQuotientDworkCoeffModP_specializedFiniteLog_eq_one_sub_pow_mul_unscaled` (for the
**unit** argument `c^{p-1} − 1`, through the second-order Fermat quotient
`column_pow_pred_modSq_eq`), blocked from automatic level-`71` re-instantiation by the
`adicCompletionIntegers` whnf wall. -/
...
open BernoulliRegular (CPlusGenerator) in
/-- **The coordinate-bridge residual is non-vacuous** (proven): the pinned leading coefficient is
the genuine nonzero deg-`32` slice second digit `deg32SliceSecondDigit37 = −(32!)⁻¹` (the mod-`37`
...
sign — so the residual is exactly the mod-`37` shadow of the level-`71` coordinate identity, with
the leading coefficient forced to the proven deg-`32` slice second digit `−(32!)⁻¹` (the genuine
`α₀` content, `≢ 3`).  Then the residual discharges `CaseIICor823Level71SecondOrderPartValue37`
...
coordinate-bridge residual `CaseIICor823Level71UnitDworkCoordBridge37`** (proven, axiom-clean given
the genuine residuals + the carried Kellner Prop).

Composes `caseIICor823Level71SecondOrderPartValue37_of_unitDworkCoordBridge` with the proven
endpoint `fermatLastTheoremFor_thirtyseven_of_level71SecondOrderPartValue` — Washington Proposition
8.12 at `i = 32` reduced to the single per-column coordinate identification.  The `37·(...)`
structure of the coordinate, the deg-`68` slice contribution `0`, the **proven** deg-`32` slice
value `37·(32!)⁻¹`, the genuine nonzero leading coefficient `ρ = −(32!)⁻¹` (the *sound* value, not
the over-pinned `3`), the cyclotomic column factor, and the second-order Fermat datum are all in
hand; only the level-`71` unit↔Dwork coordinate bridge remains.  Discharging it leaves FLT37 on R2
(the descent) + Kellner alone. -/
