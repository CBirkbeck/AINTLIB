...
This file uses the `N`-generic structural machinery of
`CaseIICor823Level71NGenericSpecializedLog.lean` (the mod-`37²` cyclotomic column factor, the
mod-`37²` slice-sum decomposition, the `N`-generic unscaled-minus-scaled Dwork difference, the
scaled = cyclotomic image identity) to reduce `CaseIICor823Level71UnitDworkCoordBridge37`
(`CaseIICor823Level71UnitDworkCoordBridge.lean`) to a **strictly smaller** residual that bundles
exactly the genuine analytic kernel.  It imports only; it does **not** modify any existing file.  No
`sorry`, no `axiom`.
...
* the cyclotomic column factor on the level-`71` mod-`37²` coordinate is
  ...
  proven), whose mod-`37` reduction is the rational column value `k`
  (`teichmullerCoeffModSq_castHom`) — so under the `37·` factor the Teichmüller factor collapses to
  the rational `(1 − k³²)`;
...
1. **the level-`71` unit ↔ Dwork-specialized finite-log identity** — that
  ...
  `dworkParameterNormalizedCoordFiniteLogN 71 − scaledDworkParameterNormalizedCoordFiniteLogN k 71`.
   ...
   Teichmüller transport (the Teichmüller difference `τ(k) − k` vanishes only to order `p − 1`, so
   the `p − 2`-precise `_evalₐ_pow_pred` does not lift — this is the genuine second-order
   Fermat-quotient content `v_p(L₃₇(1, ω³²)) = 1`);
...
/-- **The level-`71` Dwork-specialized finite-log residual** (a `def … : Prop`, **not** an axiom —
the single genuine `p`-adic-`L` kernel of `CaseIICor823Level71UnitDworkCoordBridge37` after the
coordinate structure is discharged).  It bundles, for the irregular column index `i = 32` and the
cyclotomic column delta `kummerLogColumnDelta a`:
...
2. the **unscaled degree-`32` coordinate value**:
   `coordModSq 32 (dworkParameterNormalizedCoordFiniteLogN 71) = 37·(32!)⁻¹`.
    ...
    samePrimeFiniteLog (p := 37) (K := CyclotomicField 37 ℚ) 71
      ...
      dworkParameterNormalizedCoordFiniteLogN (p := 37) (K := CyclotomicField 37 ℚ) 71 -
        scaledDworkParameterNormalizedCoordFiniteLogN (p := 37) (K := CyclotomicField 37 ℚ)
          ((kummerLogColumnDelta (p := 37) (by norm_num) a : CyclotomicUnitDelta 37) : ZMod 37)
            71) ∧
...
open BernoulliRegular (CPlusGenerator) in
/-- **The Dwork-specialized residual is non-vacuous** (proven): its piece-`2` value `37·(32!)⁻¹` is
a genuine *nonzero* element of `ZMod 37²` (`37·(unit)`, since `32!` is a `37`-unit), and its
piece-`1` finite-log identity is an equation between genuine level-`71` finite-log quotient elements
over the nonempty column index type.  So the residual is a real statement, not vacuously true; its
piece-`2` value is consistent with the proven `factorial32_deg32_slice_value_eq_thirtyseven`
...
theorem caseIICor823Level71DworkSpecializedFiniteLog37_consequent_inhabited
  ...
  -- `37·(32!)⁻¹ ≠ 0`: `32!` is a `37`-unit, so multiplying `37·(32!)⁻¹ = 0` by `32!` gives
  -- `37 = 0`.
...
For each column `a`, the level-`71` finite-log coordinate `W(a)` of the unit `c^{p-1} − 1` equals,
by the residual's finite-log identity and the proven coordinate structure,
`(1 − τ(k)³²)·(37·(32!)⁻¹)`, which under the `37·` factor (`thirtyseven_mul_eq_of_castHom_eq`)
collapses to `(1 − k³²)·(37·(32!)⁻¹)` since `castHom (τ(k)) = k` (`teichmullerCoeffModSq_castHom`)
and `k = (a + 2 : ZMod 37)`. -/

open BernoulliRegular (CPlusGenerator) in
/-- **The residual yields the mod-`37²` coordinate identity** (proven, axiom-clean given the
residual): `CaseIICor823Level71DworkSpecializedFiniteLog37` implies, for every column `a`,
...
(`valuedLambdaQuotientDworkCoeffModSq_sub`), the scaled coordinate is the Teichmüller factor times
the unscaled (`…_scaledDworkParameterNormalizedCoordFiniteLogN71_eq_smul`), the unscaled degree-`32`
...
unit↔Dwork-slice coordinate bridge).  This is the strictly-smaller-residual reduction: the
level-`71` coordinate structure is discharged from the `N`-generic machinery, leaving only the
single Dwork-
...
/-- **Fermat's Last Theorem for `37`, with `R4` reduced to the level-`71` Dwork-specialized
finite-log residual `CaseIICor823Level71DworkSpecializedFiniteLog37`** (proven, axiom-clean given
...
`fermatLastTheoremFor_thirtyseven_of_unitDworkCoordBridge`.  The level-`71` coordinate structure —
the mod-`37²` cyclotomic column factor, the slice-sum decomposition, the Teichmüller → rational
collapse — is **proven** from the `N`-generic machinery; only the single Dwork-specialized
finite-log identity (the genuine `v_p(L₃₇(1, ω³²)) = 1` second-order Fermat-quotient content) and
the proven-slice degree-`32` coordinate value remain.  Discharging this leaves FLT37 on R2 (the
descent) + Kellner alone. -/
