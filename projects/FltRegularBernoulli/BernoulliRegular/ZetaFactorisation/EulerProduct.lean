...
lemma normalizedFactors_subset_primesOverFinset_of_absNorm_prime_pow
  ...
  have hunder_eq_span_q : Ideal.under ℤ P = Ideal.span {((q : ℕ) : ℤ)} := by
    calc
      Ideal.under ℤ P = Ideal.span {(Ideal.absNorm (Ideal.under ℤ P) : ℤ)} :=
        (Int.ideal_span_absNorm_eq_self (Ideal.under ℤ P)).symm
      _ = Ideal.span {((q : ℕ) : ℤ)} := by rw [hunder_eq_q]
...
lemma idealNormMultiplicity_p_pow_eq_one (k : ℕ) :
  ...
  have hcard : (primesOverFinset K p).card = 1 := by
    rw [primesOverFinset_card_eq_ncard]
    exact ncard_primesOver_at_p_eq_one (p := p) (K := K)
  ...
  haveI : P.IsMaximal := Ideal.isMaximal_of_mem_primesOver hPmem
  haveI : P.LiesOver (Ideal.span {(p : ℤ)}) := by
    simpa [rationalPrimeIdeal] using hPmem.2
  have hbase_ne : rationalPrimeIdeal p ≠ ⊥ := by
    rw [rationalPrimeIdeal, Ne, Ideal.span_singleton_eq_bot]
    exact_mod_cast hp.out.ne_zero
  have hP_ne : P ≠ ⊥ := by
    letI : P.LiesOver (rationalPrimeIdeal p) := hPmem.2
    exact Ideal.ne_bot_of_liesOver_of_ne_bot hbase_ne P
  ...
  letI : Unique {I : NonzeroIdeal K // Ideal.absNorm I.1 = p ^ k} :=
    { default := ⟨⟨P ^ k, pow_ne_zero k hP_ne⟩, by
      ...
      uniq := by
        ...
        have hm_subset : m.toFinset ⊆ primesOverFinset K p := by
          simpa [m] using
            normalizedFactors_subset_primesOverFinset_of_absNorm_prime_pow
              (K := K) (q := ⟨p, hp.out⟩) hI_ne hI_norm
        have hm_eq : m = Multiset.replicate m.card P := by
          apply Multiset.eq_replicate_card.mpr
          intro Q hQ
          have hQ_fin : Q ∈ primesOverFinset K p :=
            hm_subset (Multiset.mem_toFinset.mpr hQ)
          rw [hP] at hQ_fin
          exact Finset.mem_singleton.mp hQ_fin
        have hI_pow : I = P ^ m.card := by
          calc
            I = m.prod := by
              rw [← Ideal.prod_normalizedFactors_eq_self hI_ne]
            _ = (Multiset.replicate m.card P).prod := congrArg Multiset.prod hm_eq
            _ = P ^ m.card := Multiset.prod_replicate m.card P
        have hm : m.card = k := by
          apply Nat.pow_right_injective hp.out.one_lt
          calc
            p ^ m.card = Ideal.absNorm I := by
...
/-- Every prime appearing in the `normalizedFactors` of an ideal whose absolute norm is a
power of the rational prime `q` lies over `q`, i.e. belongs to `primesOverFinset K q`. -/
lemma mem_primesOverFinset_of_mem_normalizedFactors {q : Nat.Primes}
    {I : Ideal (𝓞 K)} (hI_ne : I ≠ ⊥) {k : ℕ}
    (hI_norm : Ideal.absNorm I = (q : ℕ) ^ k) {P : Ideal (𝓞 K)}
    (hP : P ∈ UniqueFactorizationMonoid.normalizedFactors I) :
    P ∈ primesOverFinset K (q : ℕ) :=
...
`primesOverFinset K q`, with `q ≠ p`) is a `Prime` element of the ideal monoid. -/
lemma prime_of_mem_primesOverFinset {q : Nat.Primes} [Fact (q : ℕ).Prime]
    (hq : (q : ℕ) ≠ p) {P : Ideal (𝓞 K)} (hP : P ∈ primesOverFinset K (q : ℕ)) :
    Prime P := by
  have hP_ne : P ≠ ⊥ := by
    intro hP_bot
    have hP_norm :=
      absNorm_eq_q_pow_localResidueDegree_of_mem_primesOverFinset (p := p) (K := K) hq hP
    rw [Ideal.absNorm_eq_zero_iff.mpr hP_bot] at hP_norm
    exact pow_ne_zero _ q.2.ne_zero hP_norm.symm
  exact (Ideal.prime_iff_isPrime hP_ne).2
    ((mem_primesOverFinset_iff (K := K) (ℓ := (q : ℕ))).1 hP).1

/-- For a multiset `m` of ideals of `𝓞 K` all lying over the rational prime `q`
(i.e. `m.toFinset ⊆ primesOverFinset K q`, `q ≠ p`), the absolute norm of the product is
`q ^ (d * m.card)`, where `d = localResidueDegree` is the common local residue degree. -/
lemma absNorm_multiset_prod_eq_of_toFinset_subset {q : Nat.Primes} [Fact (q : ℕ).Prime]
    (hq : (q : ℕ) ≠ p) {m : Multiset (Ideal (𝓞 K))}
    (hm_subset : m.toFinset ⊆ primesOverFinset K (q : ℕ)) :
    Ideal.absNorm m.prod =
      (q : ℕ) ^ (localResidueDegree (p := p) (q : ℕ) hq * m.card) := by
...
/-- Mapping `Subtype.val` over a `Multiset.pmap` that packages each element of `m` together
with a proof recovers the original multiset `m`. -/
lemma map_val_pmap_mk {β : Type*} {r : β → Prop} (m : Multiset β)
    (H : ∀ b ∈ m, r b) :
    (m.pmap (fun b hb ↦ (⟨b, hb⟩ : Subtype r)) H).map Subtype.val = m := by
...
lemma normalizedFactors_card_mul_localResidueDegree_of_absNorm_prime_pow
  ...
  have hnorm_factors : Ideal.absNorm m.prod = (q : ℕ) ^ (d * m.card) := by
    rw [absNorm_multiset_prod_eq_of_toFinset_subset (p := p) (K := K) hq hsubset, hd]
...
lemma idealNormMultiplicity_prime_pow_mul_localResidueDegree_eq_card_sym
  ...
  let toSym : β → Sym α n := fun ⟨⟨I, hI_ne⟩, hI_norm⟩ ↦
    let m := UniqueFactorizationMonoid.normalizedFactors I
    let H : ∀ P, P ∈ m → P ∈ primesOverFinset K (q : ℕ) := fun P hP ↦
      mem_primesOverFinset_of_mem_normalizedFactors (K := K) hI_ne hI_norm hP
    ...
    ⟨Multiset.pmap (fun P hP ↦ (⟨P, hP⟩ : α)) m H, by
      simpa only [Multiset.card_pmap] using hm_card⟩
  let ofSym : Sym α n → β := fun s ↦
    let m : Multiset (Ideal (𝓞 K)) := s.1.map Subtype.val
    have hm_card : m.card = n := by
      change (s.1.map Subtype.val).card = n
    ...
    have hm_prime : ∀ P ∈ m, Prime P := by
      intro P hP
      rcases Multiset.mem_map.1 hP with ⟨Q, hQ, rfl⟩
      exact prime_of_mem_primesOverFinset (p := p) (K := K) hq Q.2
    have hm_prod_ne : m.prod ≠ ⊥ := Multiset.prod_ne_zero_of_prime m hm_prime
    have hm_norm : Ideal.absNorm m.prod = (q : ℕ) ^ (d * n) := by
      rw [absNorm_multiset_prod_eq_of_toFinset_subset (p := p) (K := K) hq hm_subset, ← hd,
        hm_card]
...
lemma dedekind_prime_power_series_eq_localFactor
  ...
  · classical
    ...
    have hf_hasSum : HasSum f (((1 - z)⁻¹) ^ Fintype.card α) := by
      ...
      · intro k hk
        have hk_mult : idealNormMultiplicity K ((q : ℕ) ^ k) ≠ 0 := fun hk_zero ↦
          hk (by simp only [f, hk_zero, Nat.cast_zero, zero_mul])
        have hk_dvd : d ∣ k := by
          by_contra hk_ndvd
          exact hk_mult <|
    ...
    have hcard_finset :
      ...
      rw [primesOverFinset_card_eq_ncard]
      exact ncard_primesOver_eq_localPrimeCount (p := p) (K := K) hq
    have hα_card : Fintype.card α = localPrimeCount (p := p) (q : ℕ) hq := by
      calc
        Fintype.card α = (primesOverFinset K (q : ℕ)).card :=
          Fintype.card_of_finset' (primesOverFinset K (q : ℕ)) fun P ↦ by rfl
...
theorem LProduct_eq_one_sub_p_neg_s_mul_dedekindZeta_of_one_lt_re {s : ℂ} (hs : 1 < s.re) :
  ...
  rw [h_combine]
  have h_ne : (1 - (p : ℂ) ^ (-s)) ≠ 0 :=
    Complex.one_sub_prime_cpow_ne_zero hp_prime hs
  field_simp
...
theorem dedekindZeta_eq_riemannZeta_mul_nontrivialLProduct_of_one_lt_re {s : ℂ}
  ...
  have h2 : LProduct (p := p) s =
    ...
    rw [LProduct_eq_trivial_mul_nontrivial p, LFunction_trivial_eq_mul_riemannZeta p hs_ne,
      mul_assoc]
  have hne : (1 - (p : ℂ) ^ (-s)) ≠ 0 :=
    Complex.one_sub_prime_cpow_ne_zero hp.out hs
