...
/-- If `α` is coprime to the rational prime `p` in the singular-recipient sense
...
theorem ringOfIntegersComplexConj_stickelbergerPrincipalGen_mul_self_of_eq_sigma_neg_one
  ...
  have hval_sum :
    ...
    rw [show ((-a : CyclotomicUnitDelta p) : ZMod p) = -(a : ZMod p) from rfl,
      ZMod.neg_val, if_neg ha_ne]
