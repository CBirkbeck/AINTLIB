...
omit [IsScalarTower ℤ (𝓞 K) (𝓞 R')] in
/-- The integer ideal under the descent prime is the ideal generated by `ℓ`
(both are maximal, and each contains the other). -/
private theorem descentPrime_under_int_eq_span_ell :
    S.toConcreteStickelbergerSetup.descentPrime.under ℤ = Ideal.span ({(ℓ : ℤ)} : Set ℤ) := by
