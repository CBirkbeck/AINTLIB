...
omit [IsScalarTower ℤ (𝓞 K) (𝓞 R')] in
/-- Flexible API: the integer ideal under the flexible descent prime is the
ideal generated by `ℓ` (both are maximal, and each contains the other). -/
private theorem descentPrime_under_int_eq_span_ell :
    S.concrete.descentPrime.under ℤ = Ideal.span ({(ℓ : ℤ)} : Set ℤ) := by
  ...
  have h_under_ne :
    ...
    exact (by exact_mod_cast (Fact.out : ℓ.Prime).ne_zero : (ℓ : ℤ) ≠ 0)
      h_ell_in_under
  have : (S.concrete.descentPrime.under ℤ).IsPrime :=
    Ideal.IsPrime.under ℤ (P := S.concrete.descentPrime)
  have : (S.concrete.descentPrime.under ℤ).IsMaximal :=
...
theorem stickelbergerSupportInOrbit_of_descentGaussSum
  ...
  have := S.concrete.descentPrime_isPrime
  have h_descent_under := S.descentPrime_under_int_eq_span_ell
