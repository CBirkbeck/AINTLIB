...
theorem quotientFractionEvalPrimeCompl_self (N : ℕ) (s : F.Q.primeCompl) :
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N (s : 𝓞 R') s =
      1 := by
  apply
    (F.toConcreteStickelbergerSetup.quotient_mk_isUnit_primeCompl N s).mul_left_inj.mp
  rw [mul_comm (F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N
      (s : 𝓞 R') s),
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_den_mul]
  simp

theorem quotientFractionEvalPrimeCompl_mul_den_cancel (N : ℕ) (x : 𝓞 R')
    (s t : F.Q.primeCompl) :
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N (x * (t : 𝓞 R'))
        (s * t) =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N x s := by
  calc
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N (x * (t : 𝓞 R'))
        (s * t)
        = F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N x s *
          ...
          F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_mul
            N x (t : 𝓞 R') s t
    _ = F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N x s := by
...
theorem finiteLogNatDivEval_natCast_mul_eq_mk {N n s : ℕ} (hn : n ≠ 0)
  ...
  calc
    Ideal.Quotient.mk (F.Q ^ (N + 1)) ((n : ℕ) : 𝓞 R') *
      ...
      Ideal.Quotient.mk (F.Q ^ (N + 1)) ((ℓ : 𝓞 R') ^ m) *
          (Ideal.Quotient.mk (F.Q ^ (N + 1)) (c : 𝓞 R') *
            F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y dc) := by
    ...
    _ =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N
        ((c : 𝓞 R') * ((d : 𝓞 R') * z)) dc := by
        rw [← F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_one
          N (c : 𝓞 R')]
        rw [← F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_mul]
        simp [dc]
    _ =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N
        ((dc : 𝓞 R') * z) dc := by
...
theorem finiteLogNatDivEval_eq_of_spec {N n s : ℕ} (hn : n ≠ 0)
    ...
    F.finiteLogNatDivEval N n s hn z hz =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y
  ...
  rw [finiteLogNatDivEval]
  change
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y₀ s₀ -
        F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y s₁ = 0
  have hsub :=
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_sub N y₀ y s₀ s₁
  rw [← hsub]
  exact
    F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_eq_zero_of_mem
      N (s₀ * s₁) hnum
...
theorem finiteLogNatDivEval_zero {N n s : ℕ} (hn : n ≠ 0)
  ...
  calc
    F.finiteLogNatDivEval N n s hn 0 hzero
        =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N 0
      ...
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y₁₂
        ...
        congrArg
          (fun den : F.Q.primeCompl =>
            F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y₁₂ den)
          (Subtype.ext (by
            simp [d₁c, d₂c, d₁d₂c, c, mul_assoc, mul_left_comm, mul_comm]))
    _ =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y₁ d₁c +
        F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y₂ d₂c := by
        rw [← F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_add]
    _ =
      F.finiteLogNatDivEval N n s hn z₁ hz₁ +
        F.finiteLogNatDivEval N n s hn z₂ hz₂ := by
...
theorem finiteLogNatDivEval_mul_denominator_right {N n m s : ℕ} (hn : n ≠ 0)
  ...
  calc
    F.finiteLogNatDivEval N (n * m) s hnm ((m : 𝓞 R') * z) hmz
        =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N (y * (cm : 𝓞 R'))
        (d * cnm) :=
    ...
    _ =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y (d * cn) :=
...
theorem finiteLogNatDivEval_neg {N n s : ℕ} (hn : n ≠ 0) {z : 𝓞 R'}
  ...
  calc
    ...
    _ = -F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y
        (d * F.ordComplPrimeCompl hn) := by
        rw [F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_neg]
...
theorem finiteLogNatDivEval_mul_left {N n s : ℕ} (hn : n ≠ 0) (r : 𝓞 R')
  ...
  calc
    F.finiteLogNatDivEval N n s hn (r * z) hrz
        =
      F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N (r * y)
        (d * F.ordComplPrimeCompl hn) :=
