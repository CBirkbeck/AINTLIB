    ...
    Ideal.Quotient.mk (F.Q ^ (N + 1)) ((n : ℕ) : 𝓞 R') *
      ...
      Ideal.Quotient.mk (F.Q ^ (N + 1)) ((ℓ : 𝓞 R') ^ m) *
          (Ideal.Quotient.mk (F.Q ^ (N + 1)) (c : 𝓞 R') *
            F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl N y s) := by
...
theorem finiteLogTermCore_arg_zero (N n : ℕ) (h0 : (0 : 𝓞 R') ∈ F.Q) :
  ...
  exact F.toConcreteStickelbergerSetup.quotientFractionEvalPrimeCompl_eq_zero_of_mem
