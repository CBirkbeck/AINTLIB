...
public import BernoulliRegular.Reflection.ResidueSymbol.Furtwaengler.DworkFactorization.FlexibleFiniteArtinHasseHomogeneous.HomogeneousExpCoordPoly
...
theorem artinHasseExp_trunc_eval_inverse_trunc_eval_eq_one_add_pi
    ...
    (PowerSeries.trunc (N + 1) Eps).eval₂ (RingHom.id A)
      ...
      1 + πbar := by
  exact
    F.concrete.artinHasseExp_trunc_eval_inverse_trunc_eval_eq_one_add_pi
      N
...
theorem quotient_mk_finiteArtinHasseExp_inverseParameter_eq_one_add_pi
  ...
  rw [finiteArtinHasseExp]
  exact
    F.concrete.quotient_mk_dworkThetaTrunc_artinHasseAtTo_approx_one_eq_one_add_pi N
...
theorem finiteLog_finiteArtinHasseExpCoord_inverseParameter_eq_finiteLog_pi
    (N : ℕ)
    (hδ : F.concrete.artinHasseDworkParameterApproxTo N ∈ F.Q) :
    F.finiteLog N
        (F.finiteArtinHasseExpCoord N
          (F.concrete.artinHasseDworkParameterApproxTo N))
...
theorem finiteArtinHasseLog_inverseParameter_eq_finiteLog_pi
    ...
    F.finiteArtinHasseLog N
        (F.concrete.artinHasseDworkParameterApproxTo N) hδ =
...
theorem finiteArtinHasseLog_inverseParameter_ell_nsmul_eq_zero
    ...
    ℓ • F.finiteArtinHasseLog N
        (F.concrete.artinHasseDworkParameterApproxTo N) hδ = 0 := by
...
theorem finiteArtinHasseLog_inverseParameter_ell_nsmul_eq_zero'
    ...
    ℓ • F.finiteArtinHasseLog N
        (F.concrete.artinHasseDworkParameterApproxTo N)
        (F.concrete.artinHasseDworkParameterApproxTo_mem_Q N) = 0 :=
