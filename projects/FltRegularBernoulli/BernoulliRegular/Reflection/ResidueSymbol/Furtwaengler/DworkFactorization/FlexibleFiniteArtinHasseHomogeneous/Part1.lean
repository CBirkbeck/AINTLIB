...
noncomputable def finiteArtinHasseExpCoordPoly (N : ℕ) (x : 𝓞 R') :
  ...
  ∑ n ∈ Finset.range N,
    Polynomial.monomial (n + 1)
      (F.concrete.dworkCoeffArtinHasseAtTo x N (n + 1))
...
theorem finiteArtinHasseExpCoordPoly_coeff_mem_Q_pow
  ...
  · subst d
    ...
    exact F.concrete.dworkCoeffArtinHasseAtTo_mem_Q_pow hx N (n + 1)
...
theorem finiteArtinHasseExpCoordPoly_coeff_eq_of_pos_le
    ...
    (F.finiteArtinHasseExpCoordPoly N x).coeff d =
      F.concrete.dworkCoeffArtinHasseAtTo x N d := by
  classical
  ...
  calc
    (∑ n ∈ Finset.range N,
      ...
      ((Polynomial.monomial ((d - 1) + 1))
        (F.concrete.dworkCoeffArtinHasseAtTo x N
          ((d - 1) + 1))).coeff d := by
    ...
    _ = F.concrete.dworkCoeffArtinHasseAtTo x N d := by
...
theorem finiteArtinHasseExpCoordPoly_coeff_sub_coeff_mem_Q_pow
  ...
  · rw [if_pos hdN]
    ...
    · have hdM : d ≤ M := hdN.trans hNM
      ...
      have hNcoeff :
          (F.finiteArtinHasseExpCoordPoly N x).coeff d =
            S.dworkCoeffArtinHasseAtTo x N d := by
        simpa [S] using F.finiteArtinHasseExpCoordPoly_coeff_eq_of_pos_le N d x hd0 hdN
      have hMcoeff :
          (F.finiteArtinHasseExpCoordPoly M x).coeff d =
            S.dworkCoeffArtinHasseAtTo x M d := by
        simpa [S] using F.finiteArtinHasseExpCoordPoly_coeff_eq_of_pos_le M d x hd0 hdM
      have hN :
          (c.den : 𝓞 R') * S.dworkCoeffArtinHasseAtTo x N d -
              (c.num : 𝓞 R') * x ^ d ∈ F.Q ^ (N + 1 + d) :=
        S.dworkCoeffArtinHasseAtTo_den_mul_sub_num_gamma_pow_mem_Q_pow_succ_add hx N d
      have hMhigh :
          (c.den : 𝓞 R') * S.dworkCoeffArtinHasseAtTo x M d -
              (c.num : 𝓞 R') * x ^ d ∈ F.Q ^ (M + 1 + d) :=
        S.dworkCoeffArtinHasseAtTo_den_mul_sub_num_gamma_pow_mem_Q_pow_succ_add hx M d
      have hM :
          (c.den : 𝓞 R') * S.dworkCoeffArtinHasseAtTo x M d -
              (c.num : 𝓞 R') * x ^ d ∈ F.Q ^ (N + 1 + d) :=
      ...
      have hden_not_mem : (c.den : 𝓞 R') ∉ F.Q :=
        S.natCast_not_mem_Q_of_coprime_ell
          (show c.den.Coprime ℓ from artinHasseExpSeries_coeff_isRIntegral ℓ d)
...
theorem quotient_mk_finiteArtinHasseExpCoordPoly_coeff_eq
  ...
  let Eps : PowerSeries A := hE.mapTo (F.rIntegralRatToQuotient N)
  by_cases hd0 : d = 0
  · subst d
    have hconst :
    ...
    · have hdmem : d - 1 ∈ Finset.range N :=
          ...
          (∑ n ∈ Finset.range N,
            ...
            ((Polynomial.monomial ((d - 1) + 1))
              ...
              have hne_degree : n + 1 ≠ d := fun h =>
                hne (by omega)
              simp [Polynomial.coeff_monomial, hne_degree]
          _ = S.dworkCoeffArtinHasseAtTo x N d := by
      ...
      have hcoeff_E :
        ...
        have hcoeff_Eps :
          ...
          change (PowerSeries.coeff (R := A) d)
              (hE.mapTo (F.rIntegralRatToQuotient N)) =
            F.rIntegralRatToQuotient N
              (⟨(PowerSeries.coeff (R := ℚ) d) (artinHasseExpSeries ℓ), hE d⟩ :
      ...
      calc
        Ideal.Quotient.mk QN ((F.finiteArtinHasseExpCoordPoly N x).coeff d)
            =
          Ideal.Quotient.mk QN (S.dworkCoeffArtinHasseAtTo x N d) := by
            rw [hcoeff_poly]
        _ =
          F.rIntegralRatToQuotient N
              (⟨(PowerSeries.coeff (R := ℚ) d) (artinHasseExpSeries ℓ), hE d⟩ :
            ...
            S.quotient_mk_dworkCoeffArtinHasseAtTo_eq_rIntegralRatToQuotient_mul_gamma_pow x N d
        _ =
      ...
      PowerSeries.rescale (Ideal.Quotient.mk (F.Q ^ (N + 1)) x)
        ((((show DieudonneDwork.IsRIntegralPS ℓ (artinHasseExpSeries ℓ) from
            fun m => artinHasseExpSeries_coeff_isRIntegral ℓ m).mapTo
              (F.rIntegralRatToQuotient N)) - 1)) := by
  ext d
    ...
    (PowerSeries.coeff (R := 𝓞 R' ⧸ F.Q ^ (N + 1)) d)
      ...
      Ideal.Quotient.mk (F.Q ^ (N + 1)) (x ^ d) *
        (PowerSeries.coeff (R := 𝓞 R' ⧸ F.Q ^ (N + 1)) d)
          (((((show DieudonneDwork.IsRIntegralPS ℓ (artinHasseExpSeries ℓ) from
              fun m => artinHasseExpSeries_coeff_isRIntegral ℓ m).mapTo
                (F.rIntegralRatToQuotient N)) - 1) ^ n)) := by
  let A : Type w := 𝓞 R' ⧸ F.Q ^ (N + 1)
  let QN : Ideal (𝓞 R') := F.Q ^ (N + 1)
          ...
          (PowerSeries.coeff (R := 𝓞 R' ⧸ F.Q ^ (N + 1)) d)
            (((((show DieudonneDwork.IsRIntegralPS ℓ (artinHasseExpSeries ℓ) from
                fun m => artinHasseExpSeries_coeff_isRIntegral ℓ m).mapTo
                  (F.rIntegralRatToQuotient N)) - 1) ^ n)) := by
  let S := F.concrete
  let φ := F.rIntegralRatToQuotient N
  let hE : DieudonneDwork.IsRIntegralPS ℓ (artinHasseExpSeries ℓ) :=
