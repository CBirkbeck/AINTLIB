...
theorem kummerLogColumnIndex_two_le (hp_three : 3 ≤ p) (a : Fin (kummerLogRank p)) :
    2 ≤ kummerLogColumnIndex (p := p) hp_three a :=
  CPlusGeneratorIndex_two_le (p := p) hp_three a

theorem kummerLogColumnIndex_le_half (hp_three : 3 ≤ p) (a : Fin (kummerLogRank p)) :
    kummerLogColumnIndex (p := p) hp_three a ≤ (p - 1) / 2 :=
  CPlusGeneratorIndex_le_half (p := p) hp_three a

theorem kummerLogColumnIndex_lt_p (hp_three : 3 ≤ p) (a : Fin (kummerLogRank p)) :
...
theorem zmod_natCast_eq_of_lt {a b p : ℕ} (ha : a < p) (hb : b < p)
    (h : (a : ZMod p) = (b : ZMod p)) :
    a = b := by
  rwa [ZMod.natCast_eq_natCast_iff', Nat.mod_eq_of_lt ha, Nat.mod_eq_of_lt hb] at h
...
theorem kummerLogColumnIndex_add_ne_zero (hp_three : 3 ≤ p) (a b : Fin (kummerLogRank p)) :
...
theorem teichmullerEvenNode_ne_one (hp_three : 3 ≤ p) (a : Fin (kummerLogRank p)) :
  ...
  · have hA1 : A = 1 :=
      zmod_natCast_eq_of_lt
        (kummerLogColumnIndex_lt_p (p := p) hp_three a)
        (by omega)
        (by simpa using sub_eq_zero.mp hsub)
    have hA_two := kummerLogColumnIndex_two_le (p := p) hp_three a
    omega
  · have hAp1 :
        ((A + 1 : ℕ) : ZMod p) = 0 := by
...
theorem transpose_vandermondeTeichmullerEvenSubOneMatrix_eq_diagonal_mul_geomEvaluationMatrix
  ...
  ext a j
  simp only [transpose_apply, vandermondeTeichmullerEvenSubOneMatrix, diagonal_mul,
    geomEvaluationMatrix, geomPolynomial_eval]
  simpa [mul_comm, mul_left_comm, mul_assoc] using
    (geom_sum_mul (teichmullerEvenNode (p := p) hp_three a) ((j : ℕ) + 1)).symm

theorem vandermonde_teichmuller_even_sub_one_det_ne_zero (hp_three : 3 ≤ p) :
  ...
  exact mul_ne_zero
    (Finset.prod_ne_zero_iff.mpr fun a _ =>
      sub_ne_zero.mpr (teichmullerEvenNode_ne_one (p := p) hp_three a))
