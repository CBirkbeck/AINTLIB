...
/-- The Dirichlet congruence `ℓ ≡ 1 (mod p^n)` already forces `ℓ ≠ p`, so any prime `ℓ`
satisfying it is a Thaine auxiliary. (If `ℓ = p` then `p^n ∣ p - 1`, impossible for `n ≥ 1`.) -/
private theorem isThaineAuxiliary_of_cast_eq_one {p n ℓ : ℕ} (hp : p.Prime) (hn : 1 ≤ n)
    (hℓ_prime : ℓ.Prime) [NeZero (p ^ n)]
    (hℓ_cast : (ℓ : ZMod (p ^ n)) = 1) :
    IsThaineAuxiliary p n ℓ := by
  constructor
  · intro h_eq
    rw [h_eq] at hℓ_cast
    have h_sub_zero : ((p - 1 : ℕ) : ZMod (p ^ n)) = 0 := by
      rw [Nat.cast_sub hp.pos, Nat.cast_one, hℓ_cast, sub_self]
  ...
  · have h_sub_zero : ((ℓ - 1 : ℕ) : ZMod (p ^ n)) = 0 := by
      rw [Nat.cast_sub hℓ_prime.one_lt.le, Nat.cast_one, hℓ_cast, sub_self]
...
p n ℓ` is infinite, by Dirichlet density on the congruence `ℓ ≡ 1 (mod p^n)`. -/
theorem infinite_setOf_thaineAuxiliary
    (p : ℕ) (hp : p.Prime) (n : ℕ) (hn : 1 ≤ n) :
    {ℓ : ℕ | ℓ.Prime ∧ IsThaineAuxiliary p n ℓ}.Infinite := by
  have : NeZero (p ^ n) := ⟨(Nat.pow_pos hp.pos).ne'⟩
  refine (Nat.infinite_setOf_prime_and_eq_mod (q := p ^ n) (a := 1) isUnit_one).mono ?_
  rintro ℓ ⟨hℓ_prime, hℓ_cast⟩
  exact ⟨hℓ_prime, isThaineAuxiliary_of_cast_eq_one hp hn hℓ_prime hℓ_cast⟩

/-- **Generic Thaine-auxiliary existence via Dirichlet density**
(unconditional): for any prime `p` and any `n ≥ 1` there is a prime `ℓ ≠ p`
with `p^n ∣ ℓ - 1`. Immediate from `infinite_setOf_thaineAuxiliary`. -/
theorem thaineAuxiliaryExistence_of_prime
    (p : ℕ) (hp : p.Prime) (n : ℕ) (hn : 1 ≤ n) :
    ThaineAuxiliaryExistence p n :=
  let ⟨ℓ, hℓ⟩ := (infinite_setOf_thaineAuxiliary p hp n hn).nonempty
  ⟨ℓ, hℓ.1, hℓ.2⟩
