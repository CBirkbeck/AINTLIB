...
/-! ### The pulled torsion points, and the abstract closure transport -/

/-- The `T'`-point of `E` over `τ ≫ g` obtained by pulling back a `T`-point `c` of `E[N]`
over `g` along `τ`. -/
noncomputable def torsionPointPull {T' : Scheme.{u}} (c : T ⟶ E.torsion N)
    (hc : c ≫ E.torsionπ N = g) (τ : T' ⟶ T) : E.Point (τ ≫ g) :=
  ⟨τ ≫ c ≫ E.torsionι N, by rw [Category.assoc, Category.assoc, E.torsionι_π, hc]⟩

@[simp]
theorem torsionPointPull_coe {T' : Scheme.{u}} (c : T ⟶ E.torsion N)
    (hc : c ≫ E.torsionπ N = g) (τ : T' ⟶ T) :
    ((E.torsionPointPull N g c hc τ : E.Point (τ ≫ g)) : T' ⟶ E.E)
      = τ ≫ c ≫ E.torsionι N :=
  rfl

/-- A pulled torsion point is killed by `N`: `torsionι ≫ [N] = torsionπ ≫ 0`. -/
theorem zsmul_torsionPointPull_eq_zero {T' : Scheme.{u}} (c : T ⟶ E.torsion N)
    (hc : c ≫ E.torsionπ N = g) (τ : T' ⟶ T) :
    (N : ℤ) • E.torsionPointPull N g c hc τ = 0 := by
...
/-- The base-change dictionary sends the pulled tautological section to the pulled torsion
point: both have carrier `τ ≫ c ≫ torsionι`. -/
theorem baseChangeEquiv_pull_torsionMapSection {T' : Scheme.{u}} (c : T ⟶ E.torsion N)
    (hc : c ≫ E.torsionπ N = g) (τ : T' ⟶ T) :
    Point.baseChangeEquiv E g τ
        (Point.pull (E.baseChange g) τ (E.torsionMapSection N g c hc))
      = E.torsionPointPull N g c hc τ := by
  refine Subtype.ext ?_
  show (τ ≫ ((E.torsionMapSection N g c hc : (E.baseChange g).Point (𝟙 T)) :
      T ⟶ (E.baseChange g).E)) ≫ pullback.fst E.π g = τ ≫ c ≫ E.torsionι N
  rw [Category.assoc, E.torsionMapSection_fst N g c hc]

/-- The pulled tautological section is killed by `N`. -/
theorem zsmul_pull_torsionMapSection_eq_zero {T' : Scheme.{u}} (c : T ⟶ E.torsion N)
    (hc : c ≫ E.torsionπ N = g) (τ : T' ⟶ T) :
    (N : ℤ) • Point.pull (E.baseChange g) τ (E.torsionMapSection N g c hc) = 0 := by
  rw [← Point.pull_zsmul, E.torsionMapSection_killed N g c hc, Point.pull_zero]

/-- The geometric fibre of `E[N]` has exactly `N²` points (T-B6, packaged as a cardinality). -/
theorem natCard_torsionBy_point_eq_sq [IsAlgClosed k] (t : Spec (CommRingCat.of k) ⟶ S)
    (hNk : (N : k) ≠ 0) :
    Nat.card ↥(Submodule.torsionBy ℤ (E.Point t) (N : ℤ)) = N ^ 2 := by
...
/-- **The closure transport, on abstract groups.** For an additive isomorphism `φ : A ≃+ B`
and a pair `P, Q` of `n`-killed elements of `A`, "every `n`-killed element of `A` lies in the
subgroup generated by `P, Q`" is the same statement as "the `n`-torsion submodule of `B` is
generated by the transported pair".  No schemes occur, so the group instances are local
variables and nothing is unfolded while this is checked. -/
private theorem forall_zsmul_eq_zero_mem_closure_pair_iff {A B : Type u} [AddCommGroup A]
    [AddCommGroup B] (φ : A ≃+ B) {n : ℤ} {P Q : A}
    (P' Q' : ↥(Submodule.torsionBy ℤ B n)) (hP' : (P' : B) = φ P) (hQ' : (Q' : B) = φ Q) :
    (∀ x : A, n • x = 0 → x ∈ AddSubgroup.closure ({P, Q} : Set A))
      ↔ ∀ z : ↥(Submodule.torsionBy ℤ B n),
          z ∈ AddSubgroup.closure ({P', Q'} : Set ↥(Submodule.torsionBy ℤ B n)) := by
  have hsub : ∀ z : ↥(Submodule.torsionBy ℤ B n),
      z ∈ AddSubgroup.closure ({P', Q'} : Set ↥(Submodule.torsionBy ℤ B n))
        ↔ (z : B) ∈ AddSubgroup.closure ({φ P, φ Q} : Set B) := by
    intro z
    have himg : ({φ P, φ Q} : Set B)
        = (Submodule.torsionBy ℤ B n).subtype.toAddMonoidHom '' {P', Q'} := by
      rw [Set.image_pair]
      exact congrArg₂ (fun u v => ({u, v} : Set B)) hP'.symm hQ'.symm
    rw [himg]
    exact (injective_mem_closure_image_iff _ Subtype.val_injective _ _).symm
  constructor
  · intro hcl z
    rw [hsub]
    have hz : n • φ.symm (z : B) = 0 := by
      rw [← map_zsmul φ.symm, (Submodule.mem_torsionBy_iff _ _).mp z.2, map_zero]
    have himg := addMonoidHom_mem_closure_image φ.toAddMonoidHom (hcl _ hz)
    rw [Set.image_pair] at himg
    simpa using himg
  · intro hcl x hx
    have hφx : φ x ∈ Submodule.torsionBy ℤ B n :=
      (Submodule.mem_torsionBy_iff _ _).mpr (by rw [← map_zsmul φ, hx, map_zero])
    have himg := addMonoidHom_mem_closure_image φ.symm.toAddMonoidHom
      ((hsub ⟨φ x, hφx⟩).mp (hcl ⟨φ x, hφx⟩))
    rw [Set.image_pair] at himg
    simpa using himg

/-- A geometric point of `T` above a prescribed point: the algebraic closure of its residue
field. -/
private theorem exists_algClosed_point_over (t : ↥T) :
    ∃ (k' : Type u) (_ : Field k') (_ : IsAlgClosed k') (τ : Spec (CommRingCat.of k') ⟶ T)
      (pt : ↥(Spec (CommRingCat.of k'))), τ.base pt = t := by
  obtain ⟨pt⟩ : Nonempty ↥(Spec (CommRingCat.of (AlgebraicClosure (T.residueField t)))) :=
    inferInstance
  refine ⟨AlgebraicClosure (T.residueField t), inferInstance, inferInstance,
    Spec.map (CommRingCat.ofHom (algebraMap (T.residueField t)
      (AlgebraicClosure (T.residueField t)))) ≫ T.fromSpecResidueField t, pt, ?_⟩
  have hmem : (Spec.map (CommRingCat.ofHom (algebraMap (T.residueField t)
      (AlgebraicClosure (T.residueField t)))) ≫ T.fromSpecResidueField t).base pt
        ∈ Set.range (T.fromSpecResidueField t).base :=
    ⟨(Spec.map (CommRingCat.ofHom (algebraMap (T.residueField t)
      (AlgebraicClosure (T.residueField t))))).base pt, rfl⟩
  rwa [Scheme.range_fromSpecResidueField] at hmem

/-- **The single-geometric-point form of the master iff.** At a geometric point `τ` of `T`,
the lifted pair lands in the full-level set exactly when the two pulled sections generate
all `N`-torsion of `E ×_S T` over `τ`: the counting criterion
`pair_generates_iff_combos_ne_zero` with `#E[N](k̄) = N²`, read through the zero test
`comp_combination_mem_zeroSection_iff` and the abstract transport above. -/
private theorem mem_fullLevelSet_iff_forall_zsmul_eq_zero_mem_closure [IsAlgClosed k]
    (h : NIsInvertible S N) (a b : T ⟶ E.torsion N)
    (ha : a ≫ E.torsionπ N = g) (hb : b ≫ E.torsionπ N = g)
    (τ : Spec (CommRingCat.of k) ⟶ T) (pt : ↥(Spec (CommRingCat.of k))) :
    (pullback.lift a b (ha.trans hb.symm)).base (τ.base pt) ∈ E.fullLevelSet N
      ↔ ∀ x : (E.baseChange g).Point τ, (N : ℤ) • x = 0 →
          x ∈ AddSubgroup.closure
            ({Point.pull (E.baseChange g) τ (E.torsionMapSection N g a ha),
              Point.pull (E.baseChange g) τ (E.torsionMapSection N g b hb)} :
              Set ((E.baseChange g).Point τ)) := by
  classical
  have hNk : (N : k) ≠ 0 := (nIsInvertible_spec_iff k N).mp (h.of_hom (τ ≫ g))
  obtain ⟨P', hP'⟩ : ∃ P' : ↥(Submodule.torsionBy ℤ (E.Point (τ ≫ g)) (N : ℤ)),
      (P' : E.Point (τ ≫ g)) = E.torsionPointPull N g a ha τ :=
    ⟨⟨_, (Submodule.mem_torsionBy_iff _ _).mpr (E.zsmul_torsionPointPull_eq_zero N g a ha τ)⟩,
      rfl⟩
  obtain ⟨Q', hQ'⟩ : ∃ Q' : ↥(Submodule.torsionBy ℤ (E.Point (τ ≫ g)) (N : ℤ)),
      (Q' : E.Point (τ ≫ g)) = E.torsionPointPull N g b hb τ :=
    ⟨⟨_, (Submodule.mem_torsionBy_iff _ _).mpr (E.zsmul_torsionPointPull_eq_zero N g b hb τ)⟩,
      rfl⟩
  -- the combination bridge: membership in the set = no vanishing nonzero combination
  have hbridge : (pullback.lift a b (ha.trans hb.symm)).base (τ.base pt) ∈ E.fullLevelSet N
      ↔ ∀ v : ZMod N × ZMod N, v ≠ 0 →
          (v.1.val : ℤ) • P' + (v.2.val : ℤ) • Q' ≠ 0 := by
    have hcomp : ∀ v : ZMod N × ZMod N,
        (τ ≫ pullback.lift a b (ha.trans hb.symm) ≫ E.combinationHom N v).base pt
          = (E.combinationHom N v).base
            ((pullback.lift a b (ha.trans hb.symm)).base (τ.base pt)) := by
      intro v
      simp
    have hzero : ∀ v : ZMod N × ZMod N,
        ((v.1.val : ℤ) • P' + (v.2.val : ℤ) • Q' = 0)
          ↔ (v.1.val : ℤ) • E.torsionPointPull N g a ha τ
              + (v.2.val : ℤ) • E.torsionPointPull N g b hb τ = 0 := by
      intro v
      rw [← Submodule.coe_eq_zero, Submodule.coe_add, Submodule.coe_smul,
        Submodule.coe_smul, hP', hQ']
    rw [fullLevelSet]
    constructor
    · intro hin v hv h0
      refine Set.mem_iInter₂.mp hin v hv ?_
      rw [← hcomp v]
      exact (E.comp_combination_mem_zeroSection_iff N g h a b ha hb τ pt v).mpr
        ((hzero v).mp h0)
    · intro hv
      refine Set.mem_iInter₂.mpr fun v hvne => ?_
      rw [Set.mem_preimage, Set.mem_compl_iff]
      intro hmem
      refine hv v hvne ((hzero v).mpr ?_)
      exact (E.comp_combination_mem_zeroSection_iff N g h a b ha hb τ pt v).mp
        (by rw [hcomp v]; exact hmem)
  rw [hbridge, pair_generates_iff_combos_ne_zero (N := N)
      (E.natCard_torsionBy_point_eq_sq N (τ ≫ g) hNk)
      (fun z => Submodule.smul_torsionBy _ z) P' Q']
  exact (forall_zsmul_eq_zero_mem_closure_pair_iff (Point.baseChangeEquiv E g τ) P' Q'
    (hP'.trans (E.baseChangeEquiv_pull_torsionMapSection N g a ha τ).symm)
    (hQ'.trans (E.baseChangeEquiv_pull_torsionMapSection N g b hb τ).symm)).symm
...
structure. Both directions are the single-geometric-point statement
`mem_fullLevelSet_iff_forall_zsmul_eq_zero_mem_closure`: forward at the geometric points
supplied by the naive condition, backward at the residue-closure point over an arbitrary
`t : T` (`exists_algClosed_point_over`). -/
theorem forall_mem_fullLevelSet_iff_isNaiveFullLevel (h : NIsInvertible S N)
  ...
  constructor
  · refine fun hset => ⟨⟨E.torsionMapSection_killed N g a ha,
      E.torsionMapSection_killed N g b hb⟩, fun k' _ _ τ x hx => ?_⟩
    obtain ⟨pt⟩ : Nonempty ↥(Spec (CommRingCat.of k')) := inferInstance
    exact (E.mem_fullLevelSet_iff_forall_zsmul_eq_zero_mem_closure N g h a b ha hb τ pt).mp
      (hset _) x hx
  · intro hnaive t
    obtain ⟨k', fk, ak, τ, pt, hpt⟩ := exists_algClosed_point_over (T := T) t
    letI := fk
    letI := ak
    rw [← hpt]
    exact (E.mem_fullLevelSet_iff_forall_zsmul_eq_zero_mem_closure N g h a b ha hb τ pt).mpr
      (hnaive.2 k' τ)
