...
private theorem sectionsIdealAux_quotient_prod {R A : Type u} [CommRing R] [CommRing A]
...
private theorem sectionsIdealAux_free_quotient {R A : Type u} [CommRing R] [CommRing A]
  ...
  | m + 1, f, hf, σ, hσ => by
    ...
    obtain ⟨e₁⟩ := sectionsIdealAux_quotient_prod (f 0) (hf 0) (σ 0) (hσ 0)
      (∏ i : Fin m, f i.succ)
...
private theorem officialAux_exists_mem_fibre_principal (R A : Type u) [CommRing R] [CommRing A]
  ...
  haveI : Module.Finite κ (κ ⊗[R] (A ⧸ I)) := inferInstance
...
private theorem officialAux_fibre_nzd (R A : Type u) [CommRing R] [CommRing A] [Algebra R A]
  ...
  haveI : Module.Finite R (A ⧸ I) := hfin
...
private theorem officialAux_away_nzd (R A : Type u) [CommRing R] [CommRing A] [Algebra R A]
    [Algebra.IsStandardSmoothOfRelativeDimension 1 R A]
    (I : Ideal A) (hflat : Module.Flat R (A ⧸ I)) (hfin : Module.Finite R (A ⧸ I))
    (f : A) (r : A)
    (hspan : I.map (algebraMap A (Localization.Away r))
        = Ideal.span {algebraMap A (Localization.Away r) f}) :
...
point `c` in the support of a relative effective Cartier divisor `D` on a smooth separated
relative curve there is a standard-smooth affine chart `V₀ ∋ c` (over an affine `U₀`) on
which the coordinate ring `Γ(C,V₀)/I` of `D ∩ V₀` is a **finite** `Γ(S,U₀)`-module.

This is the one genuinely chart-sensitive input: finiteness of `D → S` is
Zariski-local-at-target only (`IsFinite = affineAnd RingHom.Finite`), *not* source-local, so
`Γ(C,V₀)/I` need **not** be finite on an arbitrary chart — e.g. for the section `V(tx-1) ⊆
𝔸¹` the divisor "escapes to infinity" and `Γ(C,V₀)/I ≅ R[t,t⁻¹]` is a non-finite
localization. A valid chart must therefore capture the whole (finite, by `IsFinite`) `D`-fibre
over `π c` — the isolation-of-a-fibre-point construction of KM 1.2.3 (mirrored, for the
special case of a *sum of sections*, by `sectionsIdealAux_exists_chart`/`exists_groupChart` +
the disjoint-pieces gluing above). Flatness, finite presentation and `I.FG` are, by contrast,
