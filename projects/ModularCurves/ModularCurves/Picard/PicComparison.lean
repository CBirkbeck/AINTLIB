...
This is what turns a statement about *discrepancies* (`𝒪(D_Q) ⊗ 𝒪(D_{Q'})` versus
`𝒪(D_{Q+Q'}) ⊗ 𝒪(D_0)`, both tensored with a common invertible factor) back into a
statement about the two sides themselves. -/
theorem nonempty_iso_of_tensorObj_right_cancel {A B C : X.Modules} (hC : IsInvertible C)
    (e : Nonempty (tensorObj A C ≅ tensorObj B C)) : Nonempty (A ≅ B) := by
  letI := Modules.monoidalCategory X
  letI := Modules.symmetricCategory X
  ...
  obtain ⟨u, hu⟩ := hC.isUnit_toSkeleton
  rw [← hu] at hmul
  first
  | exact (Units.mul_left_inj u).mp hmul
  | exact (Units.mul_right_inj u).mp hmul
