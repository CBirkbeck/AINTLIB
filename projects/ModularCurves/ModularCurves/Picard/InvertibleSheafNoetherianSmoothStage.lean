...
This file combines smooth affine-intersection descent with properness descent over the
...
private theorem exists_later_proper_smooth_model_with_invertible_base_change_iso
...
theorem IsInvertible.exists_noetherianStageModelOfFinitePresentationProperSmoothBaseChangeIso_of_isProper
  ...
  obtain ⟨J, hJ, U, hcover, hU, M, hopenM, hpushM, hsepM, cM,
      hfpM, hqcM, hsepπM, hsmoothM, hcM, hbaseM⟩ :=
    hN.exists_finiteStageModelOfFinitePresentationSeparatedSmoothBaseChangeIso_of_isProper
      (π := π) H
  letI : Finite J := hJ
  let A := Γ(S, (⊤ : S.Opens))
