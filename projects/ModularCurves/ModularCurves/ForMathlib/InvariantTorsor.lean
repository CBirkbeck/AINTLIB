...
/-- Freeness implies the Chase–Harrison–Rosenberg condition: for every prime `p` and
`g ≠ 1`, some displacement `g • a - a` lies outside `p`. This follows by applying
freeness to the quotient map `A →ₐ[R] A ⧸ p`. -/
