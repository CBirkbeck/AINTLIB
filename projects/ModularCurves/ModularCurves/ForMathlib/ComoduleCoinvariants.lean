...
`B^{coρ} = {b : ρ b = b ⊗ 1}` are the functions on the affine chart of the quotient `E/G`; the affine
quotient map is `Spec B ⟶ Spec B^{coρ}` (cf. `ForMathlib/SpecEqualizer.lean`). Absent from mathlib
(only representation-coinvariants exist, the wrong object), so built here as an `R`-subalgebra via the
`AlgHom.equalizer` of `ρ` and the "trivial co-action" `b ↦ b ⊗ 1` (`Algebra.TensorProduct.includeLeft`).

The remaining crux — `B` is *faithfully flat* over `B^{coρ}` (the torsor / Hopf-Galois property),
whence `Spec B → Spec B^{coρ}` is the affine coequalizer via `isRegularEpi_of_flat_of_surjective_of_isAffine`
— is deferred (multi-week, mathlib-gap-filling).
-/
