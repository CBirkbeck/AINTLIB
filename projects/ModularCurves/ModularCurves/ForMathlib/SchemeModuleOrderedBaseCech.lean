...
@[simp]
theorem OrderedCechIndex.delete_delete {ι : Type u} [Preorder ι] {n : ℕ}
...
theorem isEmpty_orderedCechIndex_of_card_le
...
@[simp]
theorem orderedBaseCechComplex_X
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [LinearOrder ι] (U : ι → X.Opens) (n : ℕ) :
...
noncomputable abbrev orderedBaseCechLinearDifferential
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [LinearOrder ι] (U : ι → X.Opens) (n : ℕ) :=
  (orderedBaseCechDifferential π M U n).hom

theorem orderedBaseCechLinearDifferential_comp
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [LinearOrder ι] (U : ι → X.Opens) (n : ℕ) :
...
/-- Ordered Cech terms are flat when all intersection factors are flat. -/
theorem orderedBaseCechTerm_flat_of_factors
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
  ...
  letI : ∀ i : OrderedCechIndex ι n,
      Module.Flat Γ(S, (⊤ : S.Opens))
        (baseCechFactor π M U n i.1) := fun i => hflat i.1
  letI : Finite (Fin (n + 1) → ι) := Pi.finite
  letI : Finite (OrderedCechIndex ι n) := Subtype.finite
  exact Module.Flat.pi

/-- Ordered Cech objects are flat when all intersection factors are flat. -/
theorem orderedBaseCechObject_flat_of_factors
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [Fintype ι] [LinearOrder ι]
    (U : ι → X.Opens) (n : ℕ)
    (hflat : ∀ i : Fin (n + 1) → ι,
      Module.Flat Γ(S, (⊤ : S.Opens)) (baseCechFactor π M U n i)) :
    Module.Flat Γ(S, (⊤ : S.Opens)) (orderedBaseCechObject π M U n) := by
  letI : Module.Flat Γ(S, (⊤ : S.Opens))
      (orderedBaseCechTerm π M U n) :=
    orderedBaseCechTerm_flat_of_factors π M U n hflat
  exact Module.Flat.of_linearEquiv
    (orderedBaseCechObjectIsoPi π M U n).toLinearEquiv

/-- Every term of the ordered base-linear Cech complex is flat when the cover
consists of affine opens carrying trivializations. -/
theorem orderedBaseCechObject_flat_of_trivializingCover
    {X S : Scheme.{u}} (π : X ⟶ S) [IsAffine S] [Flat π]
    [X.IsSeparated] (M : X.Modules)
    {ι : Type u} [Fintype ι] [LinearOrder ι]
...
theorem orderedBaseCechTerm_subsingleton_of_card_le
    ...
    Subsingleton (orderedBaseCechTerm π M U n) := by
  letI : IsEmpty (OrderedCechIndex ι n) :=
    isEmpty_orderedCechIndex_of_card_le ι n h
  infer_instance

/-- Ordered Cech objects vanish above the cardinality of the cover. -/
theorem orderedBaseCechObject_isZero_of_card_le
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [Fintype ι] [LinearOrder ι]
    (U : ι → X.Opens) (n : ℕ) (h : Fintype.card ι ≤ n) :
    IsZero (orderedBaseCechObject π M U n) := by
  letI : Subsingleton (orderedBaseCechTerm π M U n) :=
    orderedBaseCechTerm_subsingleton_of_card_le π M U n h
...
theorem orderedBaseCechObject_subsingleton_of_card_le
    {X S : Scheme.{u}} (π : X ⟶ S) (M : X.Modules)
    {ι : Type u} [Fintype ι] [LinearOrder ι]
    (U : ι → X.Opens) (n : ℕ) (h : Fintype.card ι ≤ n) :
    Subsingleton (orderedBaseCechObject π M U n) :=
  ModuleCat.subsingleton_of_isZero
    (orderedBaseCechObject_isZero_of_card_le π M U n h)
