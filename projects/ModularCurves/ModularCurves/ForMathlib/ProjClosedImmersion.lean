...
/-- The `⊆` direction of `ker_away_map_quotientGradingHom`: the chartwise kernel is at
most `(F/sᵈ)`. Given `z = p/sⁿ` in the kernel, some `s`-power carries `p` into `I = (F)`;
comparing degree-`(k+n)` homogeneous components factors `p` through `F` (or kills it when
the degree is too small), placing `z` in `span {F/sᵈ}`. -/
