...
/-! ## [T-FIT] Ideal of `r × r` minors (Fitting-type) of a map of finite free modules

Stacks 00N1 / 07ZA.  ABSENT from mathlib (only `Matrix` cofactor/adjugate minors exist).  Intended
definition: the ideal generated by all `r × r` minors of the matrix of `φ` in the standard bases;
base-independent, compatible with base change / localisation, with vanishing locus
`{𝔮 : rank_{κ(𝔮)}(φ ⊗ κ(𝔮)) < r}`. -/

/-- Ideal of `r × r` minors of `φ : Sᵃ → Sᵇ` (Stacks 00N1 `I_r(φ)`, Fitting-type). -/
...
expansion).  A basic property feeding the McCoy-rank bookkeeping in [T-BE]. -/
...
/-! ## [T-GRADE] grade / depth `≥ k` and its openness

No packaged `grade`/`depth` exists in mathlib (`RingTheory/Regular` has only
`IsRegular`/`IsWeaklyRegular`; `RingTheory/Regular/Depth.lean` is a deprecated stub).  Two handles:

* Native to Stacks 00N1 / 00RB — "`I` contains a regular sequence of length `k`" (00LF), whose
  persistence under localisation / flat base change is `IsWeaklyRegular.of_flat_of_isBaseChange`.
* Strategic alternative — `grade(I,S) ≥ k ⟺ Extⁱ_S(S/I,S) = 0 ∀ i < k` (0AUJ/00LE), making
  `{𝔮 : grade_𝔮 ≥ k}` OPEN via `Module.support_eq_zeroLocus` (each `Extⁱ` is a finite module with
  CLOSED support).  Stated Ext-free below; the Ext bridge is prose in the decomposition doc. -/
...
/-- [T-GRADE.open] Openness of the grade locus — the Ext-support payoff: `{𝔮 : grade(I_𝔮) ≥ k}` is
OPEN.  Via Ext this is the complement of `⋃_{i<k} Supp Extⁱ_S(S/I,S)` (each closed by
`Module.support_eq_zeroLocus`); it is the leaner replacement for Stacks 10.129.2 inside [T-RB]. -/
