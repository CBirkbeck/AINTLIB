...
private theorem exists_map_eq_of_coeff_mem (R : Type u) [CommRing R] (R₀ : Subalgebra ℤ R)
    (n : ℕ) (p : MvPolynomial (Fin n) R) (hp : ∀ m ∈ p.support, p.coeff m ∈ R₀) :
    ∃ p₀ : MvPolynomial (Fin n) R₀, MvPolynomial.map (algebraMap R₀ R) p₀ = p := by
  classical
  refine ⟨∑ m ∈ p.support.attach, monomial m.1 (⟨p.coeff m.1, hp m.1 m.2⟩ : R₀), ?_⟩
  rw [map_sum]
  conv_rhs => rw [MvPolynomial.as_sum p]
  rw [← Finset.sum_attach p.support (fun m => monomial m (p.coeff m))]
...
private theorem exists_noetherianSubalgebra_map_image (R : Type u) [CommRing R] (n : ℕ)
  ...
  set T : Finset R := G.biUnion (fun g => g.support.image (fun m => g.coeff m)) with hT
  have hTmem : ∀ g ∈ G, ∀ m ∈ g.support, g.coeff m ∈ T := by
    intro g hg m hm
  ...
  have hTR₀ : ∀ g ∈ G, ∀ m ∈ g.support, g.coeff m ∈ R₀ := by
    intro g hg m hm
    ...
    { app A := CommRingCat.ofHom
      ...
      naturality {A B} h := by
        ...
        exact congrArg (fun φ : (A.1 ⊗[R₀] A₀) →ₐ[R₀] (R ⊗[R₀] A₀) =>
          CommRingCat.ofHom (R := A.1 ⊗[R₀] A₀) (S := R ⊗[R₀] A₀) φ.toRingHom) key }
