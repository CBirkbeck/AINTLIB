...
(`torsionπ_isFinite_of_nIsInvertible`, `MulByHomQuasiFinite.lean`) instead of the sorried
...
/-- **(L-BC, proven-finiteness variant — Y1-CLOSER L3)** `formallyUnramified_torsionπ` with
the finiteness input supplied by `torsionπ_isFinite_of_nIsInvertible`. Proof body as in
