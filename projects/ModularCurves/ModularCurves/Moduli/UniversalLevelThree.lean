...
section OpaqueProjModelE3Top

/- Same device as `OpaqueProjModelE3` above (and as `e4Top_zero` in UniversalLevelFour):
the leaf is `projModel (universalE3 R)` over `E3ModuliRing R`, itself an Away-localization
of `MvPolynomial (Fin 2) R ⧸ e3Rel`, sitting under `eqToHom (congrArg projModel _)`. -/
set_option allowUnsafeReducibility true in
attribute [local irreducible] ModularCurves.projModel
...
theorem e3Top_pulled (h3 : IsUnit (3 : Γ(X.base, ⊤))) :
  ...
  have hle : w.V.1 ≤ φ.baseHom ⁻¹ᵁ
      (⟨⊤, isAffineOpen_top _⟩ : (Spec (CommRingCat.of
        (E3ModuliRing R))).affineOpens).1 := fun x _ => trivial
  ...
  -- `universalE3` coefficient `whnf` is paid exactly once — no `maxHeartbeats` bump).
  ...
  rw [taut_universalSide (universalE3 R) φ.baseHom φ.top φ.isPullback φ.zero_w hle]
  have hσ : (sectionsMapLE φ.baseHom hle).comp
      ((Scheme.ΓSpecIso (CommRingCat.of (E3ModuliRing R))).inv.hom) =
      ((X.base.presheaf.map (homOfLE (le_top : w.V.1 ≤ ⊤)).op).hom).comp
        (e3ClassifyingRingHom X _ hD' h3) := by
    rw [sectionsMapLE_congr_hom (e3ClassifyingMap_pulled φ hL h3).symm hle]
    exact sectionsMapLE_e3ClassifyingMap hD' h3 w.V (fun x _ => trivial)
  rw [projModelBaseChange_congr_hom hσ (universalE3 R)]
  conv_lhs => rw [e3Piece]
  rfl
