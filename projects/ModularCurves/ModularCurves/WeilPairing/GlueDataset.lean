  ...
  set e : ∀ a : ↥(pullback E.π t) ⊕ ↥(pullback E.π t), M.over (W a) ≅
    ...
    fun a => match a with
      ...
      | .inr i => SheafOfModules.restrictOverTrivialization
          (pullback E.π t).ringCatSheaf M (W₀ i) (e₀ i)
          (Over.mk (homOfLE (inf_le_left : W₀ i ⊓ Zc ≤ W₀ i))) with hedef
  -- the per-chart comparison units, computed once per chart
  set Ach : ∀ c : ↥(pullback E.π t), Γ(pullback E.π t, W₀ c)ˣ := fun c =>
    trivializationTransitionUnit (W₀ c) (e₀ c)
      (Scheme.Modules.overTrivializationOfRestrictIso M (W₀ c)
        (restrictIsoOfPullbackIso M (W₀ c)
          (nativeTensorIdealTriv M J₁ J₂ e_dict (W₀ c) (f₁ c) (f₂ c) (hfmem₁ c)
            (isIso_idealGenHom_of_principal J₁ ⟨W₀ c, (V c).2⟩ (f₁ c)
              (hspan₁ c) (hnzd₁ c) (hfmem₁ c))
            (hfmem₂ c)
            (isIso_idealGenHom_of_principal J₂ ⟨W₀ c, (V c).2⟩ (f₂ c)
    ...
    | inl i =>
        ...
        | inl j =>
            ...
            exact (congrArg (sectionEval z
              ((W₀ i ⊓ (pullback.snd E.π t) ⁻¹ᵁ (z ⁻¹ᵁ W₀ i)) ⊓
                (W₀ j ⊓ (pullback.snd E.π t) ⁻¹ᵁ (z ⁻¹ᵁ W₀ j)))) hco).trans hkey
  · -- every piece sits inside its chart
    intro a
    cases a with
    | inl i => exact inf_le_left
    | inr i => exact inf_le_left
  · -- the per-chart dressed transitions (stage 2)
    intro a b hWch hne
    cases a with
    | inl i => cases b with
      | inl j =>
