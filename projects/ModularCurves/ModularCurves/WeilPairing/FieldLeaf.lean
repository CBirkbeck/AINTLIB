...
(stated abstractly so nothing chart-specific is ever unfolded). -/
theorem submonoid_map_primeCompl_of_ringEquiv {R S : Type*} [CommRing R] [CommRing S]
  ...
  · intro hx
    exact ⟨e x, fun hmem' => hx (by simpa using (hmem (e x)).mp hmem'), by simp⟩

/-- **(RP-3 transport, step 2a)** The instantiation at the chart identification. -/
theorem primeCompl_map_zChartMaximalIdeal {K : Type u} [Field K] [DecidableEq K]
    (W : WeierstrassCurve K) [W.IsElliptic]
    (P : (⟨W⟩ : SmoothPlaneCurve K).SmoothPoint) :
...
noncomputable def zChartLocalizationEquiv {K : Type u} [Field K] [DecidableEq K]
    (W : WeierstrassCurve K) [W.IsElliptic]
    (P : (⟨W⟩ : SmoothPlaneCurve K).SmoothPoint) :
    haveI := (zChartMaximalIdeal_isMaximal W P).isPrime
