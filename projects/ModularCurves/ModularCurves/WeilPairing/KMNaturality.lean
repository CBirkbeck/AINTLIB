...
/-- Transport of `sectionEval` along an equality of the section morphism (the opens are
propositionally equal, so this needs a `subst`). -/
private theorem sectionEval_congr_morphism {X Y : Scheme.{u}} {P P' : Y ⟶ X} (h : P = P')
    (U : X.Opens) (u : Γ(X, U)ˣ) :
    Scheme.resUnit (le_of_eq (congrArg (· ⁻¹ᵁ U) h.symm)) (sectionEval P U u) =
      sectionEval P' U u := by
  subst h
  refine Units.ext ?_
  rw [Scheme.resUnit_val]
  exact Scheme.resLE_rfl _ _

/-- **([TSE-BCSWAP])** The torsion-splitting value is invariant under the collapse iso:
the doubled-base-change evaluation on the pulled dataset equals the `E/t` evaluation on
the original dataset, at corresponding points. -/
theorem torsionSplittingEval_bcSwap {S : Scheme.{u}} (E : EllipticCurve S)
    {T : Scheme.{u}} (t : T ⟶ S)
    (hsm : SmoothOfRelativeDimension 1 E.π) [IsSeparated E.π]
    (hsmBC : SmoothOfRelativeDimension 1 ((E.baseChange t).π))
    ...
    (hA : letI := Scheme.Modules.monoidalCategory (pullback E.π t)
      (kappa E hsm t Q).val = toSkeleton A)
    {J : Type*} (W : J → (pullback E.π t).Opens) (hW : iSup W = ⊤)
    (e : ∀ i, A.over (W i) ≅
      _root_.SheafOfModules.unit ((pullback E.π t).ringCatSheaf.over (W i)))
    (hnorm : ∀ i j, transitionUnitOfCover A W e i j ∈
      sectionUnits (Scheme.Modules.baseChangeZero E.π E.zero E.zero_π t) (W i ⊓ W j))
    ...
    (hPswt : (⟨Pt.1 ≫ (bcSwapIso E t).inv, hPsw⟩ :
      ((E.baseChange t).baseChange (𝟙 T)).Point (𝟙 T)) ∈
      torsionPoints (E.baseChange t) (𝟙 T) N) :
    torsionSplittingEval (E.baseChange t) hsmBC (𝟙 T) N
        (⟨Q.1 ≫ (bcSwapIso E t).inv, hQsw⟩ :
      ...
      torsionSplittingEval E hsm t N Q hQ A hA W hW e hnorm Pt hPt := by
  obtain ⟨h, hn, hsplit⟩ := exists_normalized_transitionUnit_eq_mul_inv_of_mem_torsionPoints
    E hsm t N Q hQ A hA W hW e hnorm
  have hres := resUnit_torsionSplittingEval E hsm t N Q hQ A hA W hW e hnorm Pt hPt
    h hn hsplit
  have hle : ∀ i, mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i) ≤
      (bcSwapIso E t).hom ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i) :=
    fun i => le_of_eq (mulByN_preimage_bcSwapIso E t N (W i))
  have hn' : ∀ i, Scheme.resUnit (hle i)
      (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W i)).hom.toMonoidHom (h i)) ∈
      sectionUnits (Scheme.Modules.baseChangeZero ((E.baseChange t).π)
          ((E.baseChange t).zero) ((E.baseChange t).zero_π) (𝟙 T))
        (mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i)) := by
    intro i
    rw [mem_sectionUnits_iff, sectionEval_resUnit,
      show sectionEval (Scheme.Modules.baseChangeZero ((E.baseChange t).π)
        ((E.baseChange t).zero) ((E.baseChange t).zero_π) (𝟙 T))
        ((bcSwapIso E t).hom ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i))
        (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W i)).hom.toMonoidHom (h i)) = 1 from
      mem_sectionUnits_pullback (baseChangeZero_comp_bcSwapIso E t)
        (mulByN E t N ⁻¹ᵁ W i) (hn i), map_one]
  have hsplit' : ∀ i j,
      Units.map ((mulByN (E.baseChange t) (𝟙 T) N).app
          ((bcSwapIso E t).hom ⁻¹ᵁ W i ⊓ (bcSwapIso E t).hom ⁻¹ᵁ W j)).hom.toMonoidHom
        (transitionUnitOfCover ((Scheme.Modules.pullback (bcSwapIso E t).hom).obj A)
          (fun i => (bcSwapIso E t).hom ⁻¹ᵁ W i)
          (fun i => Scheme.Modules.localPullbackTrivializationT (bcSwapIso E t).hom A (W i) (e i)) i j) =
      Scheme.resUnit inf_le_left (Scheme.resUnit (hle i)
        (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W i)).hom.toMonoidHom (h i))) *
        (Scheme.resUnit inf_le_right (Scheme.resUnit (hle j)
          (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W j)).hom.toMonoidHom (h j))))⁻¹ := by
    intro i j
    refine ((congrArg (Units.map ((mulByN (E.baseChange t) (𝟙 T) N).app
        ((bcSwapIso E t).hom ⁻¹ᵁ W i ⊓ (bcSwapIso E t).hom ⁻¹ᵁ W j)).hom.toMonoidHom)
        (transitionUnitOfCover_localPullback ((bcSwapIso E t).hom) A W e i j)).trans ?_)
    refine ((unitsMap_app_mulByN_bcSwapIso E t N (W i ⊓ W j)
        (transitionUnitOfCover A W e i j)).trans ?_)
    refine ((congrArg (Scheme.resUnit _) (congrArg
        (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ (W i ⊓ W j))).hom.toMonoidHom)
        (hsplit i j))).trans ?_)
    refine ((congrArg (Scheme.resUnit _) (map_mul _ _ _)).trans ?_)
    refine ((map_mul _ _ _).trans ?_)
    have hVi : mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i) ⊓
        mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W j) ≤
        (bcSwapIso E t).hom ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i ⊓ mulByN E t N ⁻¹ᵁ W j) :=
      le_of_eq (by exact mulByN_preimage_bcSwapIso E t N (W i ⊓ W j))
    refine congrArg₂ (· * ·) ?_ ?_
    · exact EllipticCurve.resUnit_unitsMap_app_resUnit ((bcSwapIso E t).hom)
        (inf_le_left : mulByN E t N ⁻¹ᵁ W i ⊓ mulByN E t N ⁻¹ᵁ W j ≤ mulByN E t N ⁻¹ᵁ W i)
        hVi (le_of_eq (mulByN_preimage_bcSwapIso E t N (W i))) inf_le_left (h i)
    · refine ((congrArg (Scheme.resUnit _) (map_inv _ _)).trans ?_)
      refine ((map_inv _ _).trans ?_)
      refine congrArg (·⁻¹) ?_
      exact EllipticCurve.resUnit_unitsMap_app_resUnit ((bcSwapIso E t).hom)
        (inf_le_right : mulByN E t N ⁻¹ᵁ W i ⊓ mulByN E t N ⁻¹ᵁ W j ≤ mulByN E t N ⁻¹ᵁ W j)
        hVi (le_of_eq (mulByN_preimage_bcSwapIso E t N (W j))) inf_le_right (h j)
  have hC : ∀ i, Scheme.resUnit
      (le_top : ((⟨Pt.1 ≫ (bcSwapIso E t).inv, hPsw⟩ :
          ((E.baseChange t).baseChange (𝟙 T)).Point (𝟙 T)).1 :
          T ⟶ pullback ((E.baseChange t).π) (𝟙 T)) ⁻¹ᵁ
        (mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i)) ≤ ⊤)
      (torsionSplittingEval E hsm t N Q hQ A hA W hW e hnorm Pt hPt) =
      sectionEval ((⟨Pt.1 ≫ (bcSwapIso E t).inv, hPsw⟩ :
          ((E.baseChange t).baseChange (𝟙 T)).Point (𝟙 T)).1 :
          T ⟶ pullback ((E.baseChange t).π) (𝟙 T))
        (mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i))
        (Scheme.resUnit (hle i)
          (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W i)).hom.toMonoidHom (h i))) := by
    intro i
    have hcomp : (Pt.1 ≫ (bcSwapIso E t).inv) ≫ (bcSwapIso E t).hom = Pt.1 :=
      (Category.assoc _ _ _).trans
        ((congrArg (fun m => Pt.1 ≫ m) (bcSwapIso E t).inv_hom_id).trans
          (Category.comp_id _))
    refine Eq.symm ?_
    -- the two typed transports: onto `Pt.1`'s open, then onto the top
    have hmicro := sectionEval_congr_morphism hcomp (mulByN E t N ⁻¹ᵁ W i) (h i)
    have hLE : ((Pt.1 ≫ (bcSwapIso E t).inv) ⁻¹ᵁ
        ((bcSwapIso E t).hom ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i))) ≤
        ((Pt.1 ≫ (bcSwapIso E t).inv) ≫ (bcSwapIso E t).hom) ⁻¹ᵁ
          (mulByN E t N ⁻¹ᵁ W i) := le_of_eq rfl
    refine ((sectionEval_resUnit _ (hle i) _).trans ?_)
    refine ((congrArg (Scheme.resUnit
        ((Pt.1 ≫ (bcSwapIso E t).inv).preimage_mono (hle i)))
      (sectionEval_pullback ((bcSwapIso E t).hom) (Pt.1 ≫ (bcSwapIso E t).inv)
        (mulByN E t N ⁻¹ᵁ W i) (h i))).trans ?_)
    have houter : ((Pt.1 ≫ (bcSwapIso E t).inv) ⁻¹ᵁ
        (mulByN (E.baseChange t) (𝟙 T) N ⁻¹ᵁ ((bcSwapIso E t).hom ⁻¹ᵁ W i))) ≤
        Pt.1 ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i) :=
      ((Pt.1 ≫ (bcSwapIso E t).inv).preimage_mono (hle i)).trans
        (le_of_eq (congrArg (· ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i)) hcomp))
    refine (((Scheme.resUnit_resUnit houter
      (le_of_eq (congrArg (· ⁻¹ᵁ (mulByN E t N ⁻¹ᵁ W i)) hcomp.symm)) _).symm.trans
      (congrArg (Scheme.resUnit houter) hmicro)).trans ?_)
    refine ((congrArg (Scheme.resUnit houter) (hres i).symm).trans ?_)
    exact Scheme.resUnit_resUnit _ _ _
  exact (eq_torsionSplittingEval (E.baseChange t) hsmBC (𝟙 T) N _ hQswt
    ((Scheme.Modules.pullback (bcSwapIso E t).hom).obj A)
    (hM_bcSwap E t hsm hsmBC Q hQsw A hA)
    (fun i => (bcSwapIso E t).hom ⁻¹ᵁ W i)
    (((bcSwapIso E t).hom).iSup_preimage_eq_top hW)
    (fun i => Scheme.Modules.localPullbackTrivializationT (bcSwapIso E t).hom A (W i) (e i))
    (hnorm_bcSwap E t A W e hnorm) _ hPswt
    (fun i => Scheme.resUnit (hle i)
      (Units.map (((bcSwapIso E t).hom).app (mulByN E t N ⁻¹ᵁ W i)).hom.toMonoidHom (h i)))
    hn' hsplit' hC).symm
...
/-- **([SECTIONCLS-BCSWAP-GEN])** Section-class transport along the general collapse. -/
theorem sectionCls_bcSwapGen
    (hsm : SmoothOfRelativeDimension 1 E.π) [IsSeparated E.π]
    (hsmBC : SmoothOfRelativeDimension 1 ((E.baseChange t).π))
    [IsSeparated ((E.baseChange t).π)]
    (P : T' ⟶ pullback E.π (g ≫ t)) (hP : P ≫ pullback.snd E.π (g ≫ t) = 𝟙 T')
    (hPsw : (P ≫ (bcSwapGenIso E t g).inv) ≫
      pullback.snd ((E.baseChange t).π) g = 𝟙 T') :
    Scheme.Pic.map (bcSwapGenIso E t g).hom (sectionCls E hsm (g ≫ t) P hP) =
      sectionCls (E.baseChange t) hsmBC g (P ≫ (bcSwapGenIso E t g).inv) hPsw := by
  letI := Scheme.Modules.monoidalCategory (pullback E.π (g ≫ t))
  letI := Scheme.Modules.monoidalCategory (pullback ((E.baseChange t).π) g)
  haveI hsepE : IsSeparated (pullback.snd E.π (g ≫ t)) :=
    MorphismProperty.pullback_snd (P := @IsSeparated) E.π (g ≫ t) ‹_›
  haveI hsepB : IsSeparated (pullback.snd ((E.baseChange t).π) g) :=
    MorphismProperty.pullback_snd (P := @IsSeparated) _ _ ‹_›
  have hsmE : SmoothOfRelativeDimension 1 (pullback.snd E.π (g ≫ t)) :=
    haveI := smoothOfRelativeDimension_isStableUnderBaseChange (n := 1)
    MorphismProperty.pullback_snd (P := @SmoothOfRelativeDimension 1) E.π (g ≫ t) hsm
  have hsmB : SmoothOfRelativeDimension 1 (pullback.snd ((E.baseChange t).π) g) :=
    haveI := smoothOfRelativeDimension_isStableUnderBaseChange (n := 1)
    MorphismProperty.pullback_snd (P := @SmoothOfRelativeDimension 1) _ _ hsmBC
  haveI hciP : IsClosedImmersion P := by
    refine MorphismProperty.of_postcomp (W := @IsClosedImmersion)
      (W' := @IsSeparated) _ (pullback.snd E.π (g ≫ t)) hsepE ?_
    rw [hP]
    infer_instance
  haveI hciPsw : IsClosedImmersion (P ≫ (bcSwapGenIso E t g).inv) := by
    refine MorphismProperty.of_postcomp (W := @IsClosedImmersion)
      (W' := @IsSeparated) _ (pullback.snd ((E.baseChange t).π) g) hsepB ?_
    rw [hPsw]
    infer_instance
  have hkercollapse : (Scheme.Hom.ker P).comap (bcSwapGenIso E t g).hom =
      Scheme.Hom.ker (P ≫ (bcSwapGenIso E t g).inv) :=
    EllipticCurve.ker_comap_iso (bcSwapGenIso E t g) P
  have hJ : ∀ c : ↥(pullback E.π (g ≫ t)),
      ∃ V : (pullback E.π (g ≫ t)).affineOpens, c ∈ V.1 ∧
      ∃ f : Γ(pullback E.π (g ≫ t), V.1),
        (Scheme.Hom.ker P).ideal V = Ideal.span {f} ∧
          f ∈ nonZeroDivisors Γ(pullback E.π (g ≫ t), V.1) :=
    (RelEffCartierDiv.sectionDivisor_isOfficial hsmE P hP).locallyPrincipal
  have hJ' : ∀ c : ↥(pullback ((E.baseChange t).π) g),
      ∃ V : (pullback ((E.baseChange t).π) g).affineOpens, c ∈ V.1 ∧
      ∃ f : Γ(pullback ((E.baseChange t).π) g, V.1),
        ((Scheme.Hom.ker P).comap (bcSwapGenIso E t g).hom).ideal V = Ideal.span {f} ∧
          f ∈ nonZeroDivisors Γ(pullback ((E.baseChange t).π) g, V.1) := by
    rw [hkercollapse]
    exact (RelEffCartierDiv.sectionDivisor_isOfficial hsmB _ hPsw).locallyPrincipal
  obtain ⟨eiso⟩ := nonempty_pullback_idealModule (bcSwapGenIso E t g).hom
    (Scheme.Hom.ker P) hJ hJ'
  have hcore : Scheme.Pic.map (bcSwapGenIso E t g).hom
      (((RelEffCartierDiv.sectionDivisor (pullback.snd E.π (g ≫ t))
          P hP).isInvertible_idealModule
        (RelEffCartierDiv.sectionDivisor_isOfficial hsmE P hP)).isUnit_toSkeleton.unit) =
      ((RelEffCartierDiv.sectionDivisor (pullback.snd ((E.baseChange t).π) g)
          (P ≫ (bcSwapGenIso E t g).inv) hPsw).isInvertible_idealModule
        (RelEffCartierDiv.sectionDivisor_isOfficial hsmB _ hPsw)).isUnit_toSkeleton.unit := by
    refine Units.ext ?_
    refine (Scheme.Pic.map_val (bcSwapGenIso E t g).hom _).trans ?_
    refine (congrArg (Scheme.Modules.pullback (bcSwapGenIso E t g).hom).mapSkeleton.obj
      (IsUnit.unit_spec _)).trans ?_
    refine (Functor.mapSkeleton_obj_toSkeleton _ _).trans ?_
    refine (toSkeleton_eq_toSkeleton_iff.mpr
      ⟨eiso ≪≫ eqToIso (congrArg Scheme.Modules.idealModule hkercollapse)⟩).trans ?_
    exact (IsUnit.unit_spec _).symm
  refine Eq.trans (map_inv (Scheme.Pic.map (bcSwapGenIso E t g).hom) _) ?_
  exact congrArg (·⁻¹) hcore

/-- **([ZEROCLS-BCSWAP-GEN])** Zero-class transport along the general collapse. -/
theorem zeroCls_bcSwapGen
    (hsm : SmoothOfRelativeDimension 1 E.π) [IsSeparated E.π]
    (hsmBC : SmoothOfRelativeDimension 1 ((E.baseChange t).π))
    [IsSeparated ((E.baseChange t).π)] :
    Scheme.Pic.map (bcSwapGenIso E t g).hom (zeroCls E hsm (g ≫ t)) =
      zeroCls (E.baseChange t) hsmBC g := by
  have hz := baseChangeZero_comp_bcSwapGenIso E t g
  have hswap : Scheme.Modules.baseChangeZero E.π E.zero E.zero_π (g ≫ t) ≫
      (bcSwapGenIso E t g).inv =
      Scheme.Modules.baseChangeZero ((E.baseChange t).π) ((E.baseChange t).zero)
        ((E.baseChange t).zero_π) g :=
    (congrArg (fun m => m ≫ (bcSwapGenIso E t g).inv) hz.symm).trans
      ((Category.assoc _ _ _).trans
        ((congrArg (fun m => Scheme.Modules.baseChangeZero ((E.baseChange t).π)
            ((E.baseChange t).zero) ((E.baseChange t).zero_π) g ≫ m)
          (bcSwapGenIso E t g).hom_inv_id).trans (Category.comp_id _)))
  have hzsw : (Scheme.Modules.baseChangeZero E.π E.zero E.zero_π (g ≫ t) ≫
      (bcSwapGenIso E t g).inv) ≫ pullback.snd ((E.baseChange t).π) g = 𝟙 T' :=
    (congrArg (fun m => m ≫ pullback.snd ((E.baseChange t).π) g) hswap).trans
      (Scheme.Modules.baseChangeZero_snd _ _ _ _)
  calc Scheme.Pic.map (bcSwapGenIso E t g).hom (zeroCls E hsm (g ≫ t))
      = sectionCls (E.baseChange t) hsmBC g
          (Scheme.Modules.baseChangeZero E.π E.zero E.zero_π (g ≫ t) ≫
            (bcSwapGenIso E t g).inv) hzsw :=
        sectionCls_bcSwapGen E t g hsm hsmBC _
          (Scheme.Modules.baseChangeZero_snd E.π E.zero E.zero_π (g ≫ t)) hzsw
...
/-- **([UMAP-SQ-BCSWAP-GEN])** The `Units.map ∘ app` collapse along the general square. -/
private theorem unitsMap_app_mulByN_bcSwapGenIso (N : ℕ)
    (V : (pullback E.π (g ≫ t)).Opens) (a : Γ(pullback E.π (g ≫ t), V)ˣ) :
    Units.map ((mulByN (E.baseChange t) g N).app ((bcSwapGenIso E t g).hom ⁻¹ᵁ V)).hom.toMonoidHom
        (Units.map (((bcSwapGenIso E t g).hom).app V).hom.toMonoidHom a) =
      Scheme.resUnit (le_of_eq (mulByN_preimage_bcSwapGenIso E t g N V))
        (Units.map (((bcSwapGenIso E t g).hom).app (mulByN E (g ≫ t) N ⁻¹ᵁ V)).hom.toMonoidHom
          (Units.map ((mulByN E (g ≫ t) N).app V).hom.toMonoidHom a)) := by
  refine ((congrArg (Units.map ((mulByN (E.baseChange t) g N).app
      ((bcSwapGenIso E t g).hom ⁻¹ᵁ V)).hom.toMonoidHom)
    (map_app_eq_unitPullback ((bcSwapGenIso E t g).hom) V a)).trans ?_)
  refine ((map_app_eq_unitPullback (mulByN (E.baseChange t) g N) ((bcSwapGenIso E t g).hom ⁻¹ᵁ V)
    (unitPullback ((bcSwapGenIso E t g).hom) V ((bcSwapGenIso E t g).hom ⁻¹ᵁ V) le_rfl a)).trans ?_)
  refine ((unitPullback_unitPullback (mulByN (E.baseChange t) g N) ((bcSwapGenIso E t g).hom)
    le_rfl le_rfl a).trans ?_)
  refine ((unitPullback_congr (mulByN_comp_bcSwapGenIso E t g N) V _ _
    (le_of_eq (mulByN_preimage_bcSwapGenIso E t g N V)) a).trans ?_)
  refine Eq.trans ?_ (congrArg (Scheme.resUnit (le_of_eq
      (mulByN_preimage_bcSwapGenIso E t g N V)))
    ((congrArg (Units.map (((bcSwapGenIso E t g).hom).app (mulByN E (g ≫ t) N ⁻¹ᵁ V)).hom.toMonoidHom)
      (map_app_eq_unitPullback (mulByN E (g ≫ t) N) V a)).trans
     ((map_app_eq_unitPullback ((bcSwapGenIso E t g).hom) (mulByN E (g ≫ t) N ⁻¹ᵁ V)
        (unitPullback (mulByN E (g ≫ t) N) V (mulByN E (g ≫ t) N ⁻¹ᵁ V) le_rfl a)).trans
      (unitPullback_unitPullback ((bcSwapGenIso E t g).hom) (mulByN E (g ≫ t) N) le_rfl le_rfl a))).symm)
  exact (resUnit_unitPullback (((bcSwapGenIso E t g).hom) ≫ mulByN E (g ≫ t) N)
