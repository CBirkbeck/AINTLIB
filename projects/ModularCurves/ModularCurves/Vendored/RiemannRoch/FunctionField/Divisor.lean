...
theorem finitePrincipalDegree_eq_intDegree_norm (x : Kˣ) :
  ...
  have hchart :
    ...
    rw [← hnormn, ← hnormd]
    simpa +unfoldPartialApp only [FractionalIdeal.weightedDegree_apply, finiteIdealWeight,
...
theorem infinitePrincipalDegree_eq_neg_intDegree_norm (x : Kˣ) :
  ...
  have hchart :
    ...
    have hdeg' := hdeg
    simp +unfoldPartialApp only [FractionalIdeal.weightedDegree_apply,
      infiniteIdealWeight] at hdeg'
