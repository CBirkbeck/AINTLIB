...
section OpaqueValuationSubring
-- v4.33 bump: without the 4M-heartbeat budget the `refine ⟨_, ?_⟩` below whnf-explodes
-- through `valuationSubringAtPrime` over `integralClosure (inftyValuationSubring k) K`.
-- Opacity, not budget: the two defs only touch the subring through its SetLike interface
-- and `valuationSubringAtPrime_eq_valuationSubring`.
set_option allowUnsafeReducibility true in
attribute [local irreducible] IsDedekindDomain.HeightOneSpectrum.valuationSubringAtPrime
...
noncomputable def infiniteAdeleLocalResidueToSubring (D : DivisorA k K)
  ...
  have hmem : v.valuation K (π ^ n * (a.val : AdeleSpace k K).val (Sum.inr v)) ≤ 1 := by
    ...
    have hf : v.valuation K ((a.val : AdeleSpace k K).val (Sum.inr v)) ≤ WithZero.exp n := by
      ...
      have hDv : (D + Finsupp.single (Sum.inr v) 1 : DivisorA k K) (Sum.inr v) = n := by
...
theorem finrank_map_comap_mkQ_eq_quotient {D' : DivisorA k K}
  ...
  have hmap : st.map (eT.symm : ↥t →ₗ[k] ↥qt) = qpInQt := by
    ext x
    simp only [Submodule.mem_map_equiv, Submodule.mem_comap, Submodule.subtype_apply,
      st, qpInQt, qp, qt, eT]
    rfl
