...
|------|--------|------|-----|
| `preview-9781400881710_A26691730.pdf` | **Katz–Mazur**, *Arithmetic Moduli of Elliptic Curves* (AM-108, 1985) — **PREVIEW ONLY: frontmatter + Introduction + Ch. 1 §§1.1–1.9 (book pp. i–39)** | THE primary reference. Ch. 1 (relative Cartier divisors, full sets of sections, **exact order N**, §1.4 with proofs) fully usable for verbatim-quote decomposition. **Ch. 2 (group law, [N], pairings), Ch. 3–4 (moduli problems, representability) NOT INCLUDED — full text needed** | P0 |
...
1. **Katz–Mazur, FULL TEXT** — blocking verbatim-quote fidelity for Ch. 2 (§2.1 group
   structure, §2.3 structure of [N], §2.8 pairings), Ch. 3 (Γ-structures), Ch. 4
   (formalism + 4.7). The plan's WS-C and parts of WS-A/E are PENDING-SOURCE until then.
