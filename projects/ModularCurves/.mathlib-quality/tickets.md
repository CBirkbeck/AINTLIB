...
- [T-D22] section-of-smooth-rel-curve ⟹ locally principal nzd ideal (HB-REGIMM;
  ...
  - **[T-D22+T-D23] Status**: in_progress · **Claimed**: beastmode-D2 (stream-D
    ...
    local rings) only if the chosen route needs it. Execute [Box-1] first
    (input stated as hypothesis, immediately consumable), then [Box-2]. [T-D24] finrank additivity in SES of finite free modules (local + glue).
