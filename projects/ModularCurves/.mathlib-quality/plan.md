...
specification theorems pinning it down, (iv) a rule: **downstream code may use it only
through its stated specifications**. Anything not on this list must be `theorem`-level
`sorry` only. (Verified by the cleanup checkpoints.)
...
| DS2 | `EllipticCurve.grpObj` | EllipticCurve/GroupLaw.lean | T-A6 (Abel via Pic⁰; KM 2.1.2) | `grpObj_one_eq_zero`, `grpObj_unique`, `grpObj_comm`; `pointAddCommGroup` |
...
| DS4 | `weilPairing` | WeilPairing/Basic.lean | T-C1 (KM 2.8) | `weilPairing_over`, bilinear/alternating/nondegenerate (T-C2/3), fibre comparison T-C4 |
...
| DS5 | `vRho`, `vRhoπ`, `vRhoPointsEquiv` | ModularCurve/YRho.lean | T-F1 (Galois descent of constant gp scheme) | `vRhoπ_finite_etale`, equivariance T-F1b |
