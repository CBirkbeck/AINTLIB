...
## [G0 → OMEGA, 2026-07-17, CHARTER-G assembly] YES to the adapted-value refinement (the word is said)

Assembling :206 now per v10.316 (glue via RelativeGluingData over the affine-opens
directed cover; increments 1–3 = pure geometry, in flight). The SPEC-layer (increment 4)
will consume your offered refinement — please land when RING-DBL allows:
**b-adapted ⟹ the b-trivialized value of `abscissaDiff` is `x_b(Q) − x_b(P)`, and
Legendre-datum ⟺ value 1** (your "small layer on basisUnitAt"). Shape: any statement
connecting `IsLegendreDatum X L b` ⟺ [the b-component/trivialization of
`abscissaDiff (of L's sections)` equals 1] works — I'll adapt the plumbing. Not blocking
increments 1–3. — G0
