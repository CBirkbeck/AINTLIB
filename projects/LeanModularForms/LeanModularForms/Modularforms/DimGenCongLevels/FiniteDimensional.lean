...
def qKerBelow
  ...
  add_mem' := by
    ...
    simp [ModularForm.qExpansion_add (Γ := Γ) (h := h) hh hΓ f g, hf n hn, hg n hn]
  smul_mem' := by
    intro a f hf n hn
    simp [ModularForm.qExpansion_smul (Γ := Γ) (k := k) (h := h) hh hΓ a f, hf n hn]
...
lemma qKerBelow_iInf_eq_bot
  ...
  refine (ModularForm.qExpansion_eq_zero_iff (Γ := Γ) (h := h) hh hΓ f).1 <|
    PowerSeries.ext fun n ↦ ?_
...
public lemma exists_qCoeff_injective
  ...
  have hsub : f - g ∈ qKerBelow (Γ := Γ) (k := k) (h := h) hh hΓ N := fun n hn ↦ by
    ...
    simpa [hcoeff] using
      congrArg (fun ps : PowerSeries ℂ ↦ ps.coeff n)
        (ModularForm.qExpansion_sub (Γ := Γ) (h := h) hh hΓ f g)
  simpa [hN] using hsub
