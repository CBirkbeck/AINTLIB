...
This file proves the main uniform step bound for dyadic PV convergence by combining the remainder
analysis, gamma bounds, and singular annulus bound into a single epsilon-independent estimate.
...
private lemma annulus_inv_indicator_intervalIntegrable {γ : ℝ → ℂ}
    {a b t₀ ε₁ ε₂ : ℝ}
    {L : ℂ}
    (hab : a < b) (hγ_meas : Measurable γ) (hε₂_pos : 0 < ε₂) (hL_pos : 0 < ‖L‖)
    (h_lower : ∀ t ∈ Set.Icc a b, ε₂ < ‖γ t - γ t₀‖ →
      ‖γ t - γ t₀‖ ≤ ε₁ → ε₂ / (2 * ‖L‖) < |t - t₀|) :
    IntervalIntegrable
      (fun t ↦
        if ε₂ < ‖γ t - γ t₀‖ ∧ ‖γ t - γ t₀‖ ≤ ε₁ then
          (↑(t - t₀) : ℂ)⁻¹
        else 0)
      volume a b := by
    ...
    calc |t - t₀|⁻¹
        ≤ (ε₂ / (2 * ‖L‖))⁻¹ :=
          inv_anti₀ (by positivity)
            (le_of_lt (h_lower t (Set.Ioc_subset_Icc_self ht) hcond.1 hcond.2))
      _ = 2 * ‖L‖ / ε₂ := by rw [inv_div]
  · rw [if_neg hcond, norm_zero]
    positivity

private lemma annulus_cutoff_intervalIntegrable {f : ℝ → ℂ} {g : ℝ → ℝ}
    {a b ε₁ ε₂ : ℝ}
    (hε : ε₂ ≤ ε₁)
...
lemma pv_step_bound_ratio_two_uniform {γ : ℝ → ℂ} {a b t₀ : ℝ} {L : ℂ} (hab : a < b)
    (hat₀ : t₀ ∈ Set.Ioo a b) (hγ_C2 : ContDiffAt ℝ 2 γ t₀)
  ...
  have h_localize : ∀ t ∈ Set.Icc a b, ‖γ t - γ t₀‖ ≤ ε₁ →
      |t - t₀| < min δ₀ δ₁ := by
    intro t ht hγ_le
  ...
  have h_sing_lower : ∀ t ∈ Set.Icc a b, ε₂ < ‖γ t - γ t₀‖ →
      ‖γ t - γ t₀‖ ≤ ε₁ → ε₂ / (2 * ‖L‖) < |t - t₀| := by
  ...
  have h_sing_int :=
    annulus_inv_indicator_intervalIntegrable hab hγ_meas hε₂_pos (norm_pos_iff.mpr hL)
      h_sing_lower
  ...
  calc ‖(∫ t in a..b,
      ...
      ≤ ‖∫ t in a..b,
          ...
          ‖∫ t in a..b,
              if ε₂ < ‖γ t - γ t₀‖ ∧ ‖γ t - γ t₀‖ ≤ ε₁ then r t
              else 0‖ :=
        norm_add_le _ _
