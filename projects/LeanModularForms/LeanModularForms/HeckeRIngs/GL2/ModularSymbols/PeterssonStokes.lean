...
/-! ## L1 — the exact-2-form / planar-divergence decomposition

The genuine analytic foundation: the holomorphic-times-antiholomorphic product `h(z)·conj(a(z))`,
which is the coefficient of the Petersson area `2`-form `f·conj(g)·y^{k-2} dx∧dy` once the primitive
structure is inserted, is an exact `2`-form `d(H·conj(a) dz̄)`.  Concretely, writing the `1`-form
`H·conj(a) dz̄ = A dx + B dy` with `A = H·conj(a)`, `B = -i·H·conj(a)`, its exterior derivative is
`(∂ₓB − ∂_yA) dx∧dy = -2i·h·conj(a) dx∧dy`.

We package this as a planar **divergence** identity (matching mathlib's
`integral2_divergence_prod_of_hasFDerivAt`, which integrates `∂ₓF + ∂_yG`): with `F = B`,
`G = -A`, one has `∂ₓF + ∂_yG = -2i·h·conj(a)`.  The two derivatives are produced by
`HasDerivAt.complexToReal_fderiv` for the holomorphic factor and `Complex.conjCLE` for the
antiholomorphic one. -/
...
/-! ## L3 (rectangle case) — Stokes on a rectangle in `ℍ`

The planar divergence theorem `MeasureTheory.integral2_divergence_prod_of_hasFDerivAt` turns the
area integral of the exact `2`-form over a **rectangle** `[x₀, x₁] × [y₀, y₁] ⊂ ℍ` into a boundary
integral.  This is the rectangle special case of region-Stokes; it is the engine that, glued over the
`fd`-tile and telescoped across cosets, would yield the paired-edge boundary integral.  We record it
here for the exact-form integrand `h·conj(a)` built from a holomorphic `H` (primitive, `H' = h`) and
an antiholomorphic factor `conj(a)`.

The output is the signed sum of the four edge integrals of the `1`-form `H·conj(a) dz̄`. -/
...
/-! ## Specialisation to the period form (the holomorphic factor of the Petersson `2`-form)

The holomorphic factor of the Petersson `2`-form is the period form `periodForm g P = g·evalSym1 P`,
which has a global primitive `Fp` on the open upper half-plane by `Complex.isExactOn_upperHalf`
(`differentiableOn_periodForm`).  We record the primitive and specialise the rectangle Stokes lemma
to it: over any rectangle `[x₀,x₁]×[y₀,y₁] ⊂ ℍ`, the area integral of `-2i·periodForm g P·conj(a)`
(for an antiholomorphic factor `conj(a)`) is the signed sum of the four edge integrals of the
`1`-form `Fp·conj(a) dz̄`.  This is the form in which the geodesic-edge boundary integrals of
`FundamentalDomainBoundary.lean` are produced, once the rectangle is replaced by the `fd`-tile (the
residual region-Stokes step). -/
...
/-! ## Type-I region foundations — the FTC-in-y + Fubini half of curvilinear region-Stokes

The `fd`-tile is **not** a rectangle, so `rectangle_stokes_periodForm` does not apply directly.  But
the (capped) `fd`-tile is a **Type-I region** — the set between the graphs `y = √(1−x²)` (the
unit-circle arc, below) and `y = H` (the cap, above) over `x ∈ (−½, ½)`.  For such a region the area
integral of a planar divergence reduces, *without any missing mathlib theorem*, to boundary integrals
via FTC-in-`y` + Fubini.  We establish that founded reduction here.

These three lemmas are the sorry-free analytic backbone of the curvilinear region-Stokes step:

* `setIntegral_complex_eq_regionBetween` — the `ℂ`-area integral over the capped `fd`-set equals an
  `ℝ²`-area integral over the Type-I region `regionBetween √(1−x²) H (Ioo (−½) ½)` (via the
  measure-preserving `Complex.measurableEquivRealProd`);
* `regionBetween_integral_eq_iterated` — Fubini: the `ℝ²`-area integral over the region equals the
  iterated integral `∫ₓ ∫_{φx}^{ψx}`;
* `integral_regionBetween_partialY` — FTC-in-`y`: the area integral of a function that is `∂_yG`
  inside the region equals `∫ₓ [G(x,ψx) − G(x,φx)] dx`, the *boundary* term.

The `∂_xF` half (the second half of the divergence theorem) is the transposed Type-II statement; for
the arc-strip it requires splitting the `x`-fibres (the region is not `x`-simple below `y = √3/2`),
which is the remaining laborious bookkeeping.  These lemmas show the `∂_yG` half is fully founded. -/
...
/-! ### The `∂_xF` half — FTC-in-`x` over an `x`-simple (Type-II) region

The transposed companion of `integral_regionBetween_partialY`.  An `x`-simple region is, in standard
`(x, y)`-coordinates, the `Prod.swap`-image of a `y`-axis `regionBetween φ ψ t`:
`{(x, y) | y ∈ t ∧ φ y < x < ψ y}`.  Over such a region, the area integral of a function that is
`∂_xF` on each horizontal fibre `[φ y, ψ y]` equals the `t`-integral of the boundary difference
`F(ψ y, y) − F(φ y, y)`.  This is the founded `∂_xF` half of the curvilinear divergence theorem,
obtained from `integral_regionBetween_partialY` by `MeasureTheory.integral_prod_swap`. -/
...
/-! ### Directional derivatives of the boundary potential `Φ = Fp·conj(a)`

The `1`-form `ω = Fp·conj(a) dz̄ = A dx + B dy` has coordinate fields `A = Φ`, `B = -i·Φ` with
`Φ(z) = Fp(z)·conj(a(z))`.  For the region-Stokes reduction we need the `y`- and `x`-directional
derivatives of `Φ` along the chart `z = x + iy`.  The holomorphic factor `Fp` contributes via the
proven `hasDerivAt_periodForm_primitive_vertical`/`_horizontal`; the antiholomorphic factor
`conj ∘ a` contributes the conjugate of `a`'s directional derivative. -/
...
/-! ## Piecewise derivative of the boundary parametrisation `fdBoundaryFun H`

The contour integral over `fdBoundaryFun H` is `∫₀¹ ω(γ t)·conj(γ'(t)) dt`.  We compute `γ' = deriv
(fdBoundaryFun H)` on the open interior of each of the five segments by transferring `HasDerivAt`
across the locally-constant branch selection via `Filter.EventuallyEq.hasDerivAt_iff` (each open
sub-interval is a neighbourhood on which `fdBoundaryFun H` equals the single active branch). -/
...
The contour integral of the `1`-form `ω = W dz̄` over `fdBoundaryFun H` is
`∫₀¹ W(γ t)·conj(γ'(t)) dt`.  We split it at the four breakpoints `1/5, 2/5, 3/5, 4/5` and, on each
segment, replace `deriv (fdBoundaryFun H)` by the explicit segment derivative (they agree on the open
interior, hence a.e.).  The key reusable engine is the per-segment lemma below: on a sub-interval
`[a, b] ⊆ [0, 1]` where `deriv (fdBoundaryFun H)` agrees with a function `d` continuous on `[a, b]`,
the contour integrand is interval-integrable and equals `∫ W(γ t)·conj(d t)`. -/
...
/-! ## The `fd`-region Type-I data: the arc `φ`, the cap `ψ`, and basic facts

For the curvilinear region-Stokes assembly we fix the Type-I description of the (capped) `fd`-region:
`φ x = √(1−x²)` (the unit-circle arc, below), `ψ x = H` (the cap, above), over the open base
`s = Ioo (−½) ½`.  The arc value lies in `(√3/2, 1)` for `|x| < ½`, hence below `H` and strictly
positive; this is what places every region fibre inside the open upper half-plane. -/
...
/-! ## L3/L4 — region-Stokes on the `fd`-tile (the residual analytic wall)

This is the genuine research-scale long pole.  The rectangle Stokes lemma `rectangle_stokes_periodForm`
turns the area integral over a **rectangle** into a boundary integral.  The `Γ₁(N)` fundamental domain
`Gamma1_fundDomain_PSL N` (`PeterssonLevelN.lean`) is, however, a coset tiling by `SL₂(ℤ)`-translates
of the *standard* domain `fdo`, whose boundary is **not** a rectangle: it is the `5`-segment closed
contour `fdBoundaryFun` (`ForMathlib/FDBoundary.lean`) with **two unit-circle geodesic arcs** (Seg 2,
Seg 3) besides the three straight edges.

To run Stokes on the `fd`-tile one needs region-Stokes for this non-rectangular curvilinear region.
The two *off-the-shelf* mathlib region tools fall short:

* `MeasureTheory.integral2_divergence_prod_of_hasFDerivAt` (the planar divergence theorem) is
  **rectangle-only** — it integrates over `[[a₁,b₁]] ×ˢ [[a₂,b₂]]`.  It cannot directly take the
  curvilinear `fd`-tile.

* `MeasureTheory.curveIntegral_add_curveIntegral_eq_of_hasFDerivWithinAt` (the Poincaré route in
  `CurveIntegral/Poincare.lean`) handles a general region via a `C²` homotopy, but it proves the
  curve integral of a **closed** `1`-form (`dω = 0`) is invariant — i.e. it is the *path-independence*
  statement.  The Petersson `1`-form `ω = Fp·conj(a) dz̄` is **exact, not closed**: its differential
  `dω = -2i·periodForm g P·conj(a) dx∧dy` is the (nonzero) area integrand we are integrating.  So the
  closed-form Poincaré machinery does not apply.

**However, the curvilinear region-Stokes step is FOUNDED, not blocked by a missing theorem.**  The
(capped) `fd`-tile is a **Type-I region** — between the graphs `φ(x) = √(1−x²)` (the arc, below) and
`ψ(x) = H` (the cap, above) over `x ∈ (−½, ½)` — and Green/divergence on a Type-I region reduces to
FTC + Fubini, both of which mathlib has.  The `∂_yG` half of that reduction is now **proven
sorry-free** in this file: `setIntegral_complex_eq_regionBetween` (the `ℂ ↔ ℝ²` bridge),
`fdSet_image_eq_regionBetween` (the `fd`-set *is* the Type-I region), `regionBetween_integral_eq_iterated`
(Fubini), and `integral_regionBetween_partialY` (FTC-in-`y`, giving the boundary term
`∫ₓ [G(x,ψx) − G(x,φx)]`).  What remains is *laborious but founded* bookkeeping, with **no missing
mathlib lemma**:

  (i)   the `∂_xF` half — the transposed Type-II statement, which for the arc-strip needs splitting
        the `x`-fibres (the region is not `x`-simple below `y = √3/2`, splitting into two intervals);
  (ii)  the boundary assembly — matching the four Type-I boundary pieces (top `y=H`, bottom arc,
        two vertical sides) to the five segments of `fdBoundaryFun`, including the **arc
        reparametrisation** angle ↔ `x` (`intervalIntegral.integral_comp_mul_deriv` is the tool) and
        the piecewise `deriv (fdBoundaryFun H)` computation;
  (iii) the clockwise-contour ↔ counterclockwise-Green orientation sign;
  (iv)  the Manin↔Siegel model bridge to `FundamentalDomainBoundary.lean` (a separate downstream
        step, off this file's critical path).

Each is mechanical analysis/measure-theory work on the proven backbone above; none is a research gap.

We isolate the wall as the single named interface `tile_stokes_fd` below: region-Stokes for the
`fdo`-tile, stating that the area integral of the exact `2`-form `-2i·periodForm g P·conj(a)` over the
tile equals the contour integral of the `1`-form `Fp·conj(a) dz̄` over the `5`-segment boundary
`fdBoundaryFun H` (capped at height `H`).  `tile_stokes_fd` itself is now **proven** by founded
reduction to the single residual `region_stokes_eq_neg_contour` (whose contour side is in turn reduced
to the five explicit segment integrals by the proven `contour_eq_five_segments`); everything above
(`holAntihol_eq_divergence`, `rectangle_stokes_holAntihol`, `rectangle_stokes_periodForm`,
`exists_primitive_periodForm`, `contour_eq_five_segments`, `region_yhalf`, `continuousOn_gField_aux`)
is sorry-free and axiom-clean.

**Feasibility verdict** (see the report): the `fd`-tile + coset-telescoping route is mathematically
sound (Shimura §8.2 carries it out classically), and — with the Type-I foundations above — it is now
known to be **founded in mathlib, no missing theorem**, but it is a *long* formalization (laborious,
not research-scale).  The remaining work is the `∂_xF` Type-II half (with the arc-strip `x`-fibre
split), the four-piece boundary assembly with the arc reparametrisation angle ↔ `x`, the piecewise
`deriv (fdBoundaryFun H)` computation, the orientation sign, and the (downstream) Manin↔Siegel model
bridge.  The L1 exact-form decomposition + rectangle Stokes + the Type-I `∂_yG` reduction (this file's
sorry-free core) are the foundation; the wall is now bounded bookkeeping, not a conceptual gap. -/

/-! ### The two region-Stokes halves, specialised to the period potential `Φ = Fp·conj(a)`

We instantiate the founded half-lemmas `integral_regionBetween_partialY` (Type-I, `∂_yG`) and
`integral_regionBetweenX_partialX` (Type-II, `∂_xF`) at the concrete vector field of the Petersson
exact `2`-form.  Writing `Φ(z) = Fp(z)·conj(a(z))`, the fields are `G(x,y) = -Φ(x+iy)` and
`F(x,y) = -i·Φ(x+iy)`, whose `y`- resp. `x`-partial derivatives are supplied by
`hasDerivAt_Phi_vertical`/`hasDerivAt_Phi_horizontal` (built on the proven primitive- and
antiholomorphic-factor derivatives).  The integrability of each half's indicator is taken as a
hypothesis (discharged downstream from continuity on the compact region closure `⊂ ℍ`). -/
...
/-! ### Decomposition of the geometric edge↔segment matching

We split `region_xhalf_arc_segments` into the two genuine sub-lemmas

* `region_xhalf_typeII` (the `∂_xF` Type-II reduction: split `R` at `x = 0` into two `x`-simple
  pieces and apply the proven `integral_regionBetweenX_partialX`), and
* `arc_reparam_segments` (the unit-circle arc reparametrisation `x = cos θ`, `y = sin θ` matching the
  inner Type-II `dy`-terms + the arc `dx`-term to the two arc segments `Seg 2, Seg 3`),

together with three straight-edge match lemmas `seg1_eq`/`seg4_eq`/`seg5_eq` (affine
reparametrisations of the three straight segments). -/
...
/-! ## The binomial weight bridge (deliverable 3, step 1) — refuting the "genuinely mixed" obstruction

The Petersson area integrand `conj(f z)·g(z)·y^{k-2}` (`y = Im z`) was previously mis-classified as
"genuinely mixed" — not of the holomorphic×antiholomorphic shape `periodForm g P · conj(a)` that
`tile_stokes_fd` consumes.  **That is wrong.**  The binomial expansion of
`(z − z̄)^{k-2} = Σⱼ C(k-2,j) zʲ (−z̄)^{k-2−j}` writes `y^{k-2} = ((z − z̄)/(2i))^{k-2}` as a finite
sum, turning the integrand into

  `(2i)^{-(k-2)} · Σⱼ C(k-2,j)(−1)^{k-2−j} · (g(z)·zʲ) · conj(f(z)·z^{k-2−j})`,

each summand being **exactly** of the shape `periodForm g Pⱼ · conj(periodForm f Q_{k-2-j})` with
`Pⱼ = X₀ʲ X₁^{k-2-j}` the monomial whose `evalSym1` is `zʲ` (so `periodForm g Pⱼ z = g z · zʲ`) and
`periodForm f Q_{k-2-j}` holomorphic (hence C¹).  Thus `tile_stokes_fd` applies *per binomial term*
(`exists_tile_boundary_periodForm_term` below).  No research gap in the decomposition itself. -/
...
/-! ## The concrete fundamental-domain identification (deliverable 3)

The Green / Stokes boundary-period interface, relocated here from `FundamentalDomainBoundary.lean` so
that its proof can consume the per-tile region-Stokes lemma `tile_stokes_fd` proven above. -/

/-! ### The Green / Stokes boundary-period interface (deliverable 3 — isolated analytic core)

For weight `k ≥ 2` and cusp forms `f, g` for `Γ₁(N)`, there is a `Γ₁(N)`-paired boundary `B`
(realising the boundary `∂F` of a fundamental domain `F` as a cycle of `Γ₁(N)`-paired geodesic
edges, per `PairedBoundary`) together with a coefficient `P ∈ Sym^{k-2}(ℤ)` and a complex scalar
`c`, such that the Shimura period pairing
`A(f, g) = (f, g) + (-1)ⁿ (g, f)` (`= petN f g + (-1)^(k-2) • petN g f`, `n = k-2`) equals `c`
times the paired-edge boundary period `rawPairing f (B.boundaryDivisor ⊗ P)`.

(The left-hand side is spelled out as the explicit `petN` combination rather than via
`periodPairingA`, because `periodPairingA` is defined downstream in `PeriodInjective.lean` — the very
file that consumes this leaf.)

This is the *concrete identification* the project's `periodPairingA_eq_boundary_period`
(`PeriodInjective.lean`, Shimura (8.2.22)) requires.  The chain is:

* `petN f g + (-1)ⁿ petN g f` is, by the coset tiling (`petN_eq_nsmul_sum_PSL_tile`), a finite sum of
  area integrals over the `SL₂(ℤ)`-translate tiles `(q.out)⁻¹ • fdo` of the standard domain, of the
  Petersson integrand `conj(f)·g·y^{k-2}` against the planar measure;
* **[DISCHARGED — the `Sym`-weight bridge / "genuinely mixed" refutation]** that integrand is the
  finite binomial sum `(2i)^{-(k-2)} Σⱼ C(k-2,j)(−1)^{k-2−j}·periodForm g (symMon (k-2) j)·conj(period
  Form f (symMon (k-2) (k-2−j)))` — proven pointwise as `petersson_binomial_periodForm`
  (and its abstract core `petersson_binomial_complex`).  Each summand is of `tile_stokes_fd`'s exact
  holomorphic×antiholomorphic shape;
* **[DISCHARGED — per-term region-Stokes applicability]** for each binomial index `j`, the region-
  Stokes lemma `tile_stokes_fd` applies to that summand (`exists_tile_boundary_periodForm_term`),
  rewriting its area integral over the capped standard `fd`-tile into the boundary contour integral
  over the `5`-segment `fdBoundaryFun H`; the antiholomorphic factor `a := periodForm f (symMon …)` is
  holomorphic, discharging `tile_stokes_fd`'s `had`/`hadc` (`periodForm_hasDerivAt_and_continuousOn_
  deriv`);
* **[RESIDUAL — the `sorry` below]** the remaining composite assembling these per-term, per-tile
  capped boundary contours into `c · rawPairing f (B.boundaryDivisor ⊗ P)`: the `μ_hyp ↔ planar`
  measure conversion on each *translated* tile `(q.out)⁻¹ • fdo` (`tile_stokes_fd` is stated for the
  standard tile), the `H → ∞` cap limit (cusp decay kills the top cap), the coset telescoping of the
  `Γ₁(N)`-paired interior geodesic edges (`setIntegral_Gamma1_smul_eq`), and the **Manin↔Siegel model
  change** identifying the surviving outer paired boundary's FTC edge potentials
  (`hasDerivAt_periodForm_primitive_vertical`/`_horizontal`) with the cusp values `cuspValue f P c`
  assembled by `rawPairing f (B.boundaryDivisor ⊗ P)`.

The residual bridges two genuinely *different* geometric models — the Manin ideal-cusp-triangle
tessellation (`PairedBoundary`'s rational-cusp-endpoint edges, where `rawPairing f (∂edge ⊗ P) =
cuspValue f (head) − cuspValue f (tail)` is a geodesic period integral of `f`) and the Siegel
`fd`-translate coset tiling of `Gamma1_fundDomain_PSL` (cusp `∞`, circular-arc boundary).  This
composite — `μ_hyp`/translated-tile glue + `H → ∞` cap limit + coset telescoping + Manin↔Siegel model
change — is the research-scale "long pole" of ES-4, with no mathlib foothold for the model change; it
is the lone residual `sorry`.  Everything around it is proven: the **binomial weight bridge**
(`petersson_binomial_periodForm`, `petersson_binomial_complex`) and **per-term region-Stokes**
(`exists_tile_boundary_periodForm_term`, on top of `tile_stokes_fd`) refute the earlier "genuinely
mixed ⟹ `tile_stokes_fd` inapplicable" mis-classification; the boundary collapse
`two_smul_rawPairing_boundaryDivisor` and the `(2)`-packaging `periodPairingA_eq_boundary_period` are
also proven.  See `projects/LeanModularForms/.mathlib-quality/tickets.md` (Green's-identity
decomposition, leaves L1–L6). -/
...
/-! ## The Manin-symbol `Γ₁(N)`-paired boundary (M4, sorry-FREE)

The concrete `PairedBoundary N` realising the boundary of the `Γ₁(N)` fundamental domain in the Manin
(modular-symbol) model: a `Γ₁(N)`-side-paired cycle whose two edges `{e₀, g₀·reverse(e₀)}` are
interchanged by the fixed-point-free involution `swap` on `Bool`, with side-pairing transformation a
concrete nontrivial `g₀ ∈ Γ₁(N)`.  Its total boundary divisor is the genuine `(1 − g₀)`-cocycle
`∂e₀ − div0Rep g₀ (∂e₀)` (a Manin boundary symbol), *not* `0` — so the area-period identity it feeds
is faithful (no vacuous reverse-pairing). -/
...
/-! ### M0 decomposed: tiling reduction (founded) + the interior-edge wall (isolated)

The lone deep `sorry` of ES-4 (`k ≥ 2`), `maninArea_eq_boundary_period`, is decomposed here into a
**founded tiling/change-of-variables reduction** (`symmArea_eq_symm_stdTile_sum`, sorry-free) and the
**single genuine wall** (`maninFD_interior_edges_cancel`): the interior-edge / side-pairing
telescoping together with the Manin↔Siegel model change.  The decomposition follows Shimura (8.2.22):

* **(step 1, FOUNDED — `symmArea_eq_symm_stdTile_sum`)** the symmetrised area integral over the
  Siegel coset-tiling domain `Gamma1_fundDomain_PSL N = ⋃_q (q.out)⁻¹ • fdo` is, by the proven tiling
  `setIntegral_Gamma1_fundDomain_PSL_eq_sum`, the symmetrised finite sum over the PSL coset space
  `PSL(2,ℤ) ⧸ imageGamma1_PSL N` of the Petersson area integrals over the individual coset tiles
  `(q.out)⁻¹ • fdo`;
* **(steps 2,3,5 — proven reusable infrastructure)** per coset tile, after the measure-preserving
  change-of-variables onto the standard `fdo` (`setIntegral_smul_eq` + `petersson_slash_SL`), the
  slashed integrand is the finite binomial sum of holomorphic×antiholomorphic `periodForm`-products
  (`petersson_binomial_periodForm`); the region-Stokes lemma `tile_stokes_fd` applies per binomial
  term (`exists_tile_boundary_periodForm_term`) turning each tile's area integral into the negated
  `5`-segment boundary contour over `fdBoundaryFun H`; the top cap vanishes as `H → ∞`
  (`tendsto_horizontal_cap`, `periodForm_norm_le`); and the surviving edge contours are FTC primitive
  differences (`hasDerivAt_periodForm_primitive_vertical`/`_horizontal`, `cuspValue_eq_top_sub_botVal`)
  identified with the edge periods `rawPairing f (edge ⊗ P)` (`rawPairing_edgeDivisor_eq_sub`);
* **(step 4, THE WALL — `maninFD_interior_edges_cancel`)** summing those per-tile boundary contours
  over the coset tiling, the **interior edges shared by adjacent tiles cancel** (opposite
  orientations), leaving only the outer cusp edges, which assemble via
  `two_smul_rawPairing_boundaryDivisor` into `rawPairing f ((maninPairedBoundary N).boundaryDivisor ⊗
  P)`.  This is the modular-symbols boundary map (the side-pairing telescoping fused with the
  Manin↔Siegel model change) and has no mathlib analogue. -/
