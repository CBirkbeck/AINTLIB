...
* on every closed sub-interval $`[a_k, a_{k+1}]` between consecutive partition
  members, the (extended) path $`\gamma : [a_k, a_{k+1}] \to E` is of class
  $`C^{1}` and its one-sided within-derivative is nowhere zero:
...
{bpref "closed-pwc1-immersion"}[]. We say that $`\gamma` is *null-homologous in
$`U`* when both

* the image of $`\gamma` lies in $`U`, that is $`\gamma(t) \in U` for every
  $`t \in [0,1]`, and
* for every point $`z \in \mathbb{C} \setminus U`, the generalised winding
  number $`\operatorname{gWN}(\gamma, z)` (see {bpref "generalized-winding-number"}[])
  is zero.
